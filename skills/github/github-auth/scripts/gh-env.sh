#!/usr/bin/env bash
# GitHub environment detection helper for Rayovin Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _rayovin_env="${RAYOVIN_HOME:-$HOME/.rayovin}/.env"; [ -f "$_rayovin_env" ] && grep -q "^GITHUB_TOKEN=" "$_rayovin_env" 2>/dev/null; then
