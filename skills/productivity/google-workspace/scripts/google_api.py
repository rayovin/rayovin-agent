#!/usr/bin/env python3
"""Google Workspace API CLI for Rayovin Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Rayovin-facing JSON contract and falls back to the Python client
...
from _rayovin_home import get_rayovin_home
...
def _gws_binary() -> str | None:
    override = os.getenv("RAYOVIN_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Rayovin Agent")
