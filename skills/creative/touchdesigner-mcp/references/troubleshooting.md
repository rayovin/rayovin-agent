...
## 5. Rayovin Configuration

### Config location

`$RAYOVIN_HOME/config.yaml` (defaults to `~/.rayovin/config.yaml` when `RAYOVIN_HOME` is unset)
...
Restart the Rayovin session for changes to take effect. The MCP connection is
