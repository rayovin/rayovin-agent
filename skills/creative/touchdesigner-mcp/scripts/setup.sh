...
RAYOVIN_HOME_DIR="${RAYOVIN_HOME:-$HOME/.rayovin}"
RAYOVIN_CFG="${RAYOVIN_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Rayovin config has twozero_td MCP entry ──
if [[ ! -f "$RAYOVIN_CFG" ]]; then
    echo -e " ${FAIL} Rayovin config not found at ${RAYOVIN_CFG}"
    manual_steps+=("Create ${RAYOVIN_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$RAYOVIN_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Rayovin config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Rayovin config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$RAYOVIN_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${RAYOVIN_CFG} manually"); }
    manual_steps+=("Restart Rayovin session to pick up config change")
fi
