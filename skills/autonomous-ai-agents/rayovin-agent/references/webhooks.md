...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Rayovin agent runs by POSTing events to a URL.
...
Add to `${RAYOVIN_HOME:-~/.rayovin}/.env`:
```bash
...
All management is via the `rayovin webhook` CLI command:

### Create a subscription
```bash
rayovin webhook subscribe <name> \
...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.rayovin/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.rayovin/scripts/` (path traversal is blocked).

```bash
rayovin webhook subscribe todoist-rayovin \
  --prompt "Task changed: {payload.content}" \
  --script "todoist-rayovin-label.py" \
...
```bash
rayovin webhook remove <name>
```

### Test a subscription
```bash
rayovin webhook test <name>
rayovin webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: PR reviews
```bash
rayovin webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
rayovin webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
rayovin webhook subscribe alerts \
...
- Subscriptions persist to `~/.rayovin/webhook_subscriptions.json`

## How It Works

1. `rayovin webhook subscribe` writes to `~/.rayovin/webhook_subscriptions.json`
...
1. **Is the gateway running?** Check with `systemctl --user status rayovin-gateway` or `ps aux | grep gateway`
2. **Is the webhook server listening?** `curl http://localhost:8644/health` should return `{"status": "ok"}`
3. **Check gateway logs:** `grep webhook ~/.rayovin/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `rayovin webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `rayovin webhook test <name>` to verify the route works.
