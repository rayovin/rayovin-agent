...
rayovin kanban watch --tenant <project-slug>

# Snapshot of the board
rayovin kanban list --tenant <project-slug>
rayovin kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
rayovin kanban stats --tenant <project-slug>

# Visual dashboard (browser)
rayovin dashboard

# Inspect a specific task (includes comments + events)
rayovin kanban show <task-id>

# Follow a single task's event stream
rayovin kanban tail <task-id>
```

Verify available subcommands with `rayovin kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `rayovin kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `rayovin kanban block <id>` then `rayovin kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `rayovin kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `rayovin kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `rayovin profile list` (profile exists?), `rayovin status` (gateway/dispatcher up?), and verify tenant. |
...
rayovin kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
rayovin kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(rayovin kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
rayovin kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
rayovin kanban block <task-id>
rayovin kanban archive <task-id>

# Diagnose what happened
rayovin kanban show <task-id>      # task body, comments, recent events
rayovin kanban tail <task-id>      # follow the live event stream
rayovin kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `rayovin kanban create` for the director
...
while true; do
    clear
    rayovin kanban list --tenant <slug>
    echo "---"
    rayovin kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `rayovin kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `rayovin profile list` and start
