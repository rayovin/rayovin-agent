...
Maps each role archetype to the Rayovin skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public rayovin-agent
...
## Rayovin skills relevant to video production

### Visual / rendering skills (`rayovin-agent/skills/creative/`)
...
### Design / pre-production skills (`rayovin-agent/skills/creative/`)
...
### Audio / media skills (`rayovin-agent/skills/creative/` + `skills/media/`)
...
These are **not** Rayovin skills but external CLIs / APIs that profiles invoke.
...
## Built-in Rayovin tools for media review

These are native Rayovin tools — not invoked via terminal but through their own
...
key is present in `${RAYOVIN_HOME:-~/.rayovin}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Rayovin profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${RAYOVIN_HOME:-~/.rayovin}/.env` → environment variable.
...
When a new Rayovin-public video skill ships:
