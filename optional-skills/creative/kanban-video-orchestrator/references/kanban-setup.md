...
the actual `setup.sh` that creates the project workspace, configures Rayovin
...
2. **Create profiles** — `rayovin profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.rayovin/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `rayovin kanban create` assigned to the director
...
```bash
rayovin profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.rayovin/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.rayovin/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Rayovin
...
Each profile gets a `SOUL.md` at `~/.rayovin/profiles/<name>/SOUL.md` that
...
```bash
rayovin kanban create "Direct production of <video title>" \
    --assignee director \
...
the Rayovin `.env` (`${RAYOVIN_HOME:-$HOME/.rayovin}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _rayovin_env="${RAYOVIN_HOME:-$HOME/.rayovin}/.env"
    if grep -q "^${var}=" "$_rayovin_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_rayovin_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_rayovin_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY rayovin ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY rayovin OPENROUTER_API_KEY || exit 1
# ...
```
