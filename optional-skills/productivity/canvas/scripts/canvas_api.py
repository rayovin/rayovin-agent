#!/usr/bin/env python3
"""Canvas LMS API CLI for Rayovin Agent.
...
def _check_config():
    ...
    if missing:
        rayovin_env = os.path.join(
            os.environ.get("RAYOVIN_HOME", os.path.expanduser("~/.rayovin")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {rayovin_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Rayovin Agent"
    )
