...
The design is **Rayovin-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
rayovin skills install official/security/unbroker
```

Then start a new Rayovin session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Rayovin session:
...
- **PII handling.** Dossiers live under the Rayovin home directory (`0600`, optionally
