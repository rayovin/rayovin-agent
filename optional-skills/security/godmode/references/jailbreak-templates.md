...
## 5. GODMODE FAST — Rayovin 4 405B (Zero Refusal)

**Model:** `rayovin/rayovin-4-405b`
**Strategy:** Rayovin models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Rayovin Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.rayovin/config.yaml`:
