...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("RAYOVIN_HOME", os.path.expanduser("~/.rayovin")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("RAYOVIN_HOME", Path.home() / ".rayovin")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Rayovin config paths
# ═══════════════════════════════════════════════════════════════════

RAYOVIN_HOME = Path(os.getenv("RAYOVIN_HOME", Path.home() / ".rayovin"))
CONFIG_PATH = RAYOVIN_HOME / "config.yaml"
PREFILL_PATH = RAYOVIN_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Rayovin models — already uncensored, just needs clean prompt
    "rayovin": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "rayovin" in model_lower or "nous" in model_lower:
        return "rayovin"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Rayovin config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.rayovin/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Rayovin for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Rayovin for changes to take effect.")
