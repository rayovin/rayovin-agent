...
Usage:
  ...
  <server_name> matches the files in $RAYOVIN_HOME/mcp-tokens/<server>.json etc.
...
def _rayovin_home():
    # Prefer Rayovin' own resolver (profile-safe); fall back to env then ~/.rayovin.
    try:
        from rayovin_constants import get_rayovin_home
        return str(get_rayovin_home())
    except Exception:
        return os.environ.get("RAYOVIN_HOME") or os.path.expanduser("~/.rayovin")


def _tokens_dir():
    return os.path.join(_rayovin_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "rayovin-diag", "version": "1.0"}},
        },
        headers={
