...
Usage (via cron with --no-agent):
    ...
    rayovin cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$RAYOVIN_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Rayovin-Watcher/1.0"})
