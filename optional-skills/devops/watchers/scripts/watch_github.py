...
Usage (via cron with --no-agent):
    ...
    rayovin cron create rayovin-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$RAYOVIN_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name rayovin-issues --repo NousResearch/rayovin-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Rayovin .env file
(``${RAYOVIN_HOME:-~/.rayovin}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Rayovin-Watcher/1.0",
    }
