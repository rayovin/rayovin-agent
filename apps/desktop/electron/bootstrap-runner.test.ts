...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'rayovin-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/rayovin-runner-test',
    sourceRepoRoot: null,
    rayovinHome: '/tmp/rayovin-runner-test',
    logRoot: '/tmp/rayovin-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'rayovin-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'rayovin-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/rayovin-agent',
      rayovinHome: '/tmp/rayovin'
    }),
    ['--dir', '/tmp/rayovin-agent', '--rayovin-home', '/tmp/rayovin', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/rayovin-agent', '--rayovin-home', '/tmp/rayovin', '--branch', 'main']
  )
})
