// Unit tests for the pure Windows `rayovin` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvRayovinCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `rayovin` shim shadows
//      the real rayovin.cmd/rayovin.exe.
//   2. chooseUpdaterArgs() — must gate on haveRealInstall (any real-install
//      signal), not just the rayovin.exe console-script shim, or healthy
//      installs get forced into a destructive --repair.
//   3. resolveVenvRayovinCommand() — must probe the venv python via
//      canImportRayovinCli() before trusting it, or a broken venv gets
...
function makeDeps(overrides: Partial<Parameters<typeof resolveVenvRayovinCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportRayovinCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    rayovinHome: '/fake/rayovin-home',
...
test('resolveVenvRayovinCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvRayovinCommand('/root/venv/Scripts/rayovin.exe', [], deps), null)
})

test('resolveVenvRayovinCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvRayovinCommand('/root/venv/Scripts/rayovin.cmd', [], deps), null)
})

test('resolveVenvRayovinCommand: returns null when the basename is not rayovin/rayovin.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvRayovinCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvRayovinCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvRayovinCommand('/root/venv/bin/rayovin.exe', [], deps), null)
})

test('resolveVenvRayovinCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvRayovinCommand('/root/venv/Scripts/rayovin.exe', [], deps), null)
})

test('resolveVenvRayovinCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportRayovinCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvRayovinCommand('/root/venv/Scripts/rayovin.exe', ['serve'], deps)
...
test('resolveVenvRayovinCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvRayovinCommand('/root/venv/Scripts/rayovin.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'rayovin_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvRayovinCommand: is case-insensitive on rayovin.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvRayovinCommand('/root/venv/Scripts/RAYOVIN.EXE', [], deps))
  assert.ok(resolveVenvRayovinCommand('/root/venv/SCRIPTS/rayovin.exe', [], deps))
})
