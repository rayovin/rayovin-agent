/**
 * windows-rayovin-path.ts
 *
 * Pure, dependency-injected pieces of Windows `rayovin` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvRayovinCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `rayovin` shim shadowed the real
 *      rayovin.cmd/rayovin.exe; the shim then failed the --version probe and
 ...
 *      venv\Scripts\rayovin.exe (the console-script shim, written at the END
 ...
 *   3. resolveVenvRayovinCommand() — unwrapWindowsVenvRayovinCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `rayovin`) must
 * not shadow `rayovin.cmd`/`rayovin.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv rayovin shim, bootstrap-complete
 * marker) — gating on just the rayovin.exe console-script shim alone is the
...
export interface ResolveVenvRayovinCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportRayovinCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `rayovin`/`rayovin.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/rayovin(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m rayovin_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportRayovinCli(). A
 * venv whose update died mid-`pip install` still has python.exe + rayovin.exe
 ...
 *
 * Returns null when `command` is not a venv rayovin shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvRayovinCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvRayovinCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^rayovin(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Rayovin at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Rayovin Python at ${python}`,
    command: python,
