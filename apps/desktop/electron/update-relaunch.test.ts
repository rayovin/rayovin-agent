 ...
 *      internals; RAYOVIN_HOME / RAYOVIN_DESKTOP_* env + cwd preserved) and is
 *      safely shell-quoted.
...
const ROOT = '/home/u/.rayovin/rayovin-agent'
...
test('resolveUnpackedRelease returns the dir for a binary UNDER release/<plat>-unpacked', () => {
  const exec = path.join(UNPACKED, 'rayovin')
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  // AppImage mount
  assert.equal(resolveUnpackedRelease('/tmp/.mount_Rayovin12345/AppRun', ROOT, 'linux'), null)
  // .deb / .rpm system install
  assert.equal(resolveUnpackedRelease('/usr/lib/rayovin/rayovin', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease('/opt/Rayovin/rayovin', ROOT, 'linux'), null)
  // dev electron
  assert.equal(
    resolveUnpackedRelease('/home/u/.rayovin/rayovin-agent/node_modules/electron/dist/electron', ROOT, 'linux'),
    null
  )
  // empty / missing
  assert.equal(resolveUnpackedRelease('', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease(path.join(UNPACKED, 'rayovin'), '', 'linux'), null)
})
...
test('resolveUnpackedRelease is not fooled by a sibling prefix dir', () => {
  ...
  const sneaky = path.join(ROOT, 'apps', 'desktop', 'release', 'linux-unpacked-evil', 'rayovin')
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--lang=en-US',
    'rayovin://open/agent/42', // deep link — keep
    '--profile=work', // app flag — keep
    '--remote-debugging-port=9222' // internal — drop
  ]

  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'rayovin://open/agent/42', '--profile=work'])
  assert.deepEqual(collectRelaunchArgs(undefined), [])
})

test('collectRelaunchEnv preserves RAYOVIN_HOME + RAYOVIN_DESKTOP_* + sandbox opt-out only', () => {
  const env = {
    RAYOVIN_HOME: '/home/u/.rayovin',
    RAYOVIN_DESKTOP_REMOTE_URL: 'http://box:9119',
    RAYOVIN_DESKTOP_REMOTE_TOKEN: 'secret',
    RAYOVIN_DESKTOP_RAYOVIN_ROOT: '/home/u/dev/rayovin',
    RAYOVIN_DESKTOP_APP_NAME: 'RayovinSandbox',
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    pid: 4242,
    execPath: '/home/u/.rayovin/rayovin-agent/apps/desktop/release/linux-unpacked/Rayovin',
    args: ['rayovin://open/agent/42', "--note=it's fine"],
    env: { RAYOVIN_HOME: '/home/u/.rayovin', RAYOVIN_DESKTOP_REMOTE_URL: 'http://box:9119' },
    cwd: '/home/u/work dir'
  })
  ...
  assert.match(script, /export RAYOVIN_HOME='\/home\/u\/\.rayovin'/)
  assert.match(script, /export RAYOVIN_DESKTOP_REMOTE_URL='http:\/\/box:9119'/)
  assert.match(script, /cd '\/home\/u\/work dir'/)
  assert.match(script, /exec '.*\/linux-unpacked\/Rayovin' 'rayovin:\/\/open\/agent\/42' '--note=it'\\''s fine'/)

  // It must be syntactically valid bash (`bash -n`). Write to a temp file and lint.
  const tmp = path.join(os.tmpdir(), `rayovin-relaunch-test-${Date.now()}.sh`)
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  const script = buildRelaunchScript({
    pid: 1,
    execPath: '/opt/Rayovin/Rayovin',
    args: [],
    env: {},
    cwd: ''
  })

  const tmp = path.join(os.tmpdir(), `rayovin-relaunch-test2-${Date.now()}.sh`)
  fs.writeFileSync(tmp, script)
  ...
  assert.match(script, /exec '\/opt\/Rayovin\/Rayovin'\n/)
})
