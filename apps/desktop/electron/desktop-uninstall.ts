 ...
 * the three user-facing uninstall modes to the `rayovin uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m rayovin_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `rayovin_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'rayovin_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Rayovin.app/Contents/MacOS/Rayovin  → …/Rayovin.app
 *   Windows: …\Rayovin\Rayovin.exe                 → …\Rayovin  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Rayovin.app/Contents/MacOS/Rayovin → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Rayovin.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Rayovin.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Rayovin$/i.test(dir) || /[\\/]rayovin-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/rayovin
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import rayovin_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, rayovinHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export RAYOVIN_HOME=${q(rayovinHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import rayovin_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Rayovin install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "RAYOVIN_HOME=${String(rayovinHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
