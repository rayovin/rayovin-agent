 ...
 * Background
 * ----------
 * After `rayovin update` + `rayovin desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything RAYOVIN_DESKTOP_* is preserved
// plus RAYOVIN_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['RAYOVIN_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['RAYOVIN_DESKTOP_']
