...
import { getRayovinConfigRecord, type RayovinConfigRecord, saveRayovinConfig } from '@/rayovin'
...
export interface I18nConfigClient {
  getConfig: () => Promise<RayovinConfigRecord>
  saveConfig: (config: RayovinConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.rayovinDesktop?.api) {
      return Promise.resolve({})
    }

    return getRayovinConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.rayovinDesktop?.api) {
    ...
    return saveRayovinConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: RayovinConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: RayovinConfigRecord, locale: Locale): RayovinConfigRecord {
