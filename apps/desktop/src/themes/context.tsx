...
const SKIN_KEY = 'rayovin-desktop-theme-v2'
const MODE_KEY = 'rayovin-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'rayovin-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'rayovin-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'rayovin-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.rayovinTheme = skinName
  root.dataset.rayovinMode = rendered
  ...
  window.rayovinDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('rayovin-boot-background', chromeBg)
    window.localStorage.setItem('rayovin-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.rayovinThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.rayovinDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active Rayovin backend theme on connect. */
