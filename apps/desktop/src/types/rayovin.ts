...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Rayovin doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in rayovin_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `rayovin model` uses — instead of
...
export interface RayovinConfig {
  agent?: {
...
export type RayovinConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `rayovin model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors rayovin_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Rayovin, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `rayovin tools` active-provider detection). */
  is_active: boolean
}
 ...
 *  not Rayovin); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/rayovin/update/check` — the backend's own update state.
