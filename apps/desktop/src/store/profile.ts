...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/rayovin'
...
import type { ProfileInfo } from '@/types/rayovin'
...
// /api/profiles/active `current`). "default" is the root ~/.rayovin. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'rayovin.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'rayovin.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.rayovinDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new RAYOVIN_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.rayovinDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.rayovinDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'rayovin.desktop.showAllProfiles'
...
// Switch to the default (root ~/.rayovin) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.rayovinDesktop?.touchBackend?.(target).catch(() => undefined)
}
