...
import { getRayovinConfigRecord, saveRayovinConfig } from '@/rayovin'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getRayovinConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveRayovinConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
