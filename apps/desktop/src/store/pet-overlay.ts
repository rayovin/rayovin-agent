 ...
 * app's bounds and stays visible while Rayovin is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'rayovin.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'rayovin.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.rayovinDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.rayovinDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.rayovinDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.rayovinDesktop?.petOverlay?.close()
}
