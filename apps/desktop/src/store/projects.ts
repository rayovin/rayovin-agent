...
import type { RayovinGitBaseBranch, RayovinGitBranch } from '@/global'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/rayovin'
...
const PROJECT_SCOPE_KEY = 'rayovin.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Rayovin gateway is not connected')
  }
...
// repo, returning where Rayovin should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<RayovinGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<RayovinGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.rayovinDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.rayovinDesktop?.writeClipboard?.(path)
  }
}
