...
const { getProfiles } = await import('@/rayovin')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/rayovin/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<RayovinConnection> = {}): RayovinConnection =>
  ({ baseUrl: 'https://rayovin-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as RayovinConnection

const localConn = (over: Partial<RayovinConnection> = {}): RayovinConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as RayovinConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<RayovinConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { rayovinDesktop: { getConnection } })
