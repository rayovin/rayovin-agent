...
import type { RayovinRepoStatus } from '@/global'

import { $repoStatus, refreshRepoStatus } from './coding-status'
import { $currentCwd } from './session'

const sampleStatus: RayovinRepoStatus = {
...
function stubProbe(impl: (cwd: string) => Promise<RayovinRepoStatus | null>) {
  ;(window as unknown as { rayovinDesktop?: unknown }).rayovinDesktop = { git: { repoStatus: impl } }
}
