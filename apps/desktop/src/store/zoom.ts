...
export function setZoomPercent(percent: number): void {
  window.rayovinDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.rayovinDesktop?.zoom) {
  void window.rayovinDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.rayovinDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
