...
const STORAGE_KEY = 'rayovin:native-notifications'
...
// "Backgrounded" = the user isn't on Rayovin. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.rayovinDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.rayovinDesktop
