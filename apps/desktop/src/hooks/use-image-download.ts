...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'rayovin:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.rayovinDesktop?.saveImageFromUrl) {
        if (await window.rayovinDesktop.saveImageFromUrl(src)) {
