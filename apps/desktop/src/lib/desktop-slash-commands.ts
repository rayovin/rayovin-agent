...
/** Why a known Rayovin command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Rayovin profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Rayovin Agent version', surface: exec() },
...
function isKnownRayovinSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Rayovin' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownRayovinSlashCommand(normalized)
}
