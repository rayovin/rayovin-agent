...
declare global {
  interface Window {
    rayovinDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<RayovinConnection>
      ...
      // Rayovin Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new RAYOVIN_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: RayovinApiRequest) => Promise<T>
      notify: (payload: RayovinNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<RayovinReadFileTextResult>
      selectPaths: (options?: RayovinSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<RayovinPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<RayovinPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: RayovinTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<RayovinReadDirResult>
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<RayovinGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<RayovinGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<RayovinGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<RayovinRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: RayovinReviewScope, baseRef?: null | string) => Promise<RayovinReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: RayovinReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<RayovinReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: RayovinTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<RayovinTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: RayovinWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: RayovinPreviewFileChanged) => void) => () => void
...
export interface RayovinTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface RayovinTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `rayovin update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  rayovinRoot?: string
...
export interface RayovinConnection {
  baseUrl: string
...
export interface RayovinTitleBarTheme {
  background: string
  foreground: string
}

export interface RayovinWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Rayovin Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Rayovin Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteToken?: string
  remoteUrl?: string
  // For a 'cloud' connection: the selected Rayovin Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the rayovin gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Rayovin Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface RayovinApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface RayovinNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface RayovinPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface RayovinReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface RayovinPreviewWatch {
  id: string
  path: string
}
...
export interface RayovinGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface RayovinGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
...
// preselect it.
export interface RayovinGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface RayovinRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface RayovinRepoStatus {
  branch: null | string
...
// turn began.
export type RayovinReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface RayovinReviewFile {
  path: string
  added: number
  removed: number
...
export interface RayovinReviewList {
  files: RayovinReviewFile[]
...
export interface RayovinReviewPr {
  url: string
  state: string
  number: number
}
...
export interface RayovinReviewShipInfo {
  ghReady: boolean
  pr: RayovinReviewPr | null
}

export interface RayovinReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface RayovinReadDirResult {
  entries: RayovinReadDirEntry[]
  error?: string
}

export interface RayovinPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface RayovinSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
