...
// bug that drives the REAL useGatewayBoot hook + REAL RayovinGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshRayovinConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { rayovinDesktop?: unknown }).rayovinDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { rayovinDesktop?: unknown }).rayovinDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForRayovin) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startRayovin()'s remote branch awaits waitForRayovin() for 45s before it
    ...
    ;(window as { rayovinDesktop?: unknown }).rayovinDesktop = desktop
    ...
    // After ~45s waitForRayovin gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Rayovin backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
