...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.rayovinDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    window.rayovinDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.rayovinDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.rayovinDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // rayovin:// deep links -> a reviewable /blueprint command in the composer.
  useEffect(() => {
    const unsubscribe = window.rayovinDesktop?.onDeepLink?.(payload => {
    ...
    void window.rayovinDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.rayovinDesktop?.onClosePreviewRequested?.(() => void closeActiveTab())
