...
import type { RayovinReadDirEntry, RayovinReadDirResult } from '@/global'
...
export type ProjectTreeEntry = RayovinReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: RayovinReadDirEntry) {
...
async function filterIgnored(entries: RayovinReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<RayovinReadDirResult> {
  if (!window.rayovinDesktop) {
