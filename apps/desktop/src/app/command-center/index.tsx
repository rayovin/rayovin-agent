...
import { getActionStatus, getLogs, getStatus, getUsageAnalytics, restartGateway, updateRayovin } from '@/rayovin'
import type { ActionStatusResponse, AnalyticsResponse, StatusResponse } from '@/rayovin'
...
export function CommandCenterView({ initialSection, onClose, onDeleteSession, onOpenSession }: CommandCenterViewProps) {
  ...
  // Client-side substring filter over the fetched tail (matches `rayovin logs --search`).
  ...
  const runSystemAction = useCallback(
    async (kind: 'restart' | 'update') => {
      ...
      try {
        const started = kind === 'restart' ? await restartGateway() : await updateRayovin()
                    ...
                    <div className="flex items-start justify-between gap-3 max-[47.5rem]:flex-col max-[47.5rem]:gap-2">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
                          {cc.rayovinActiveSessions(status.version, status.active_sessions)}
                        </div>
                      </div>
