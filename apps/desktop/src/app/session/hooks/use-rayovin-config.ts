...
import { getRayovinConfig, getRayovinConfigDefaults } from '@/rayovin'
...
interface RayovinConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useRayovinConfig({ activeSessionIdRef, refreshProjectBranch }: RayovinConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshRayovinConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getRayovinConfig(), getRayovinConfigDefaults().catch(() => ({}))])
  ...
  return { refreshRayovinConfig, sttEnabled, voiceMaxRecordingSeconds }
}
