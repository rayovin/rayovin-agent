...
import { getRayovinConfig } from '@/rayovin'
import { persistString } from '@/lib/storage'
import { $currentCwd, setCurrentCwd } from '@/store/session'

import { useRayovinConfig } from './use-rayovin-config'

vi.mock('@/rayovin', () => ({
  getRayovinConfig: vi.fn(),
  getRayovinConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'rayovin.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getRayovinConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getRayovinConfig>>)

describe('useRayovinConfig refreshRayovinConfig', () => {
  beforeEach(() => {
  ...
  it('applies terminal.cwd from config even when localStorage has a stale value', async () => {
    ...
    await act(async () => {
      await result.current.refreshRayovinConfig()
    })
