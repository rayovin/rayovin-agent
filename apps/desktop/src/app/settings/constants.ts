...
export const EMPTY_SELECT_VALUE = '__rayovin_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted Rayovin & Nous-trained models',
    docsUrl: 'https://github.com/rayovin/rayovin-agent',
    priority: 0
  },
  {
    prefix: 'OPENROUTER_',
  ...
  { prefix: 'RAYOVIN_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // rayovin_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  timezone: 'Used when Rayovin needs local time context. Blank uses the system timezone.',
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before Rayovin stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Rayovin can read from one file request.',
  approvals: {
    mode: 'How Rayovin handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Rayovin updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
