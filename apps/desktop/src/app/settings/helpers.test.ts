...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: RayovinConfigRecord = { display: { theme: 'mono' } }
  ...
  it('rejects prototype-polluting config paths', () => {
    const config: RayovinConfigRecord = {}
  ...
  describe('providerGroup', () => {
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // RAYOVIN_QWEN_ shares the RAYOVIN_ stem with other integrations.
      expect(providerGroup('RAYOVIN_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
