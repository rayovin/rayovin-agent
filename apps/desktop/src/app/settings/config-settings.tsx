...
import { getElevenLabsVoices, getRayovinConfigSchema, saveRayovinConfig } from '@/rayovin'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, RayovinConfigRecord } from '@/types/rayovin'

import { setRayovinConfigCache, useRayovinConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: RayovinConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<RayovinConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useRayovinConfigRecord()
  ...
  const {
    ...
    queryKey: ['rayovin-config-schema'],
    queryFn: getRayovinConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  const updateConfig = (next: RayovinConfigRecord) => {
