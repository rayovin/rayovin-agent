...
import { invalidateRayovinConfig, setRayovinConfigCache, useRayovinConfigRecord } from '../hooks/use-config-record'
...
// Rayovin' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Rayovin default (medium), shown as Medium.
...
// backend now surfaces the full `rayovin model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in rayovin_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useRayovinConfigRecord()
  const setConfig = setRayovinConfigCache
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `rayovin model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Rayovin runs the flow for you.`}
          </p>
        )}
