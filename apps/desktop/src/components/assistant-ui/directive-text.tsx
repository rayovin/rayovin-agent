...
const RAYOVIN_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type RayovinRefType = (typeof RAYOVIN_REF_TYPES)[number]
...
const ICON_PATHS: Record<RayovinRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as RayovinRefType] ?? ICON_FALLBACK
...
const RAYOVIN_DIRECTIVE_RE = new RegExp(
...
export const rayovinDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(RAYOVIN_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as RayovinRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: RayovinRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...rayovinDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Rayovin directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.rayovinDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.rayovinDesktop?.readFileDataUrl(id)
