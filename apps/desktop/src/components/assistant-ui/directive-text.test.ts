...
import { formatRefValue, rayovinDirectiveFormatter } from './directive-text'
...
describe('rayovinDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = rayovinDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = rayovinDirectiveFormatter.parse('@file:src/main.tsx the entry point')
