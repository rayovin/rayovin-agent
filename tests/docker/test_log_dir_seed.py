"""Runtime smoke test for Docker $RAYOVIN_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the rayovin user after container boot.
...
registered later runs its log service as the dropped rayovin user and
...
def test_logs_gateways_seeded_and_rayovin_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by rayovin after boot."""
    ...
    assert "logs=rayovin" in r.stdout, (
        f"logs/ not owned by rayovin: {r.stdout}"
    )
    assert "gateways=rayovin" in r.stdout, (
        f"logs/gateways/ not owned by rayovin: {r.stdout}"
    )
