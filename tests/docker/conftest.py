...
Override the image with ``RAYOVIN_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("RAYOVIN_TEST_IMAGE", "rayovin-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``RAYOVIN_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("RAYOVIN_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged rayovin user
# ---------------------------------------------------------------------------
#
# Background: every Rayovin runtime path inside the container drops to UID
# 10000 (the ``rayovin`` user) via ``s6-setuidgid rayovin``. ``docker exec``
...
# fails (PermissionError swallowed) for rayovin, so a test that ran as root
...
# helpers below run every probe under ``-u rayovin`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: rayovin).
