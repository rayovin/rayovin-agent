...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_rayovin_env(self):
        """Rayovin' own .env inside RAYOVIN_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: rayovin internal .env is in ~/.rayovin/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.rayovin/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.rayovin/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Rayovin cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        rayovin_home = tmp_path / ".rayovin"
        cache = rayovin_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._rayovin_home_path", return_value=rayovin_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Rayovin cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        rayovin_home = tmp_path / ".rayovin"
        hub = rayovin_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_rayovin_home(self, tmp_path):
        """The env basename guard does not depend on RAYOVIN_HOME resolution."""
        rayovin_home = tmp_path / ".rayovin"
        rayovin_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        rayovin_home = tmp_path / ".rayovin"
        cache = rayovin_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
