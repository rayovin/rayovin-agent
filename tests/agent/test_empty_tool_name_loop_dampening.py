...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated RAYOVIN_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="rayovin_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".rayovin"))
    prev_home = os.environ.get("RAYOVIN_HOME")
    os.environ["RAYOVIN_HOME"] = os.path.join(test_home, ".rayovin")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("rayovin_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["RAYOVIN_HOME"] = prev_home
