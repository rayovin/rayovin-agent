...
path via ``rayovin chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp RAYOVIN_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point RAYOVIN_HOME at a temp dir and turn moa.save_traces on."""
    rayovin_home = tmp_path / ".rayovin"
    rayovin_home.mkdir()
    monkeypatch.setenv("RAYOVIN_HOME", str(rayovin_home))

    # save_moa_turn reads config via rayovin_cli.config.load_config; stub it to
    ...
    return rayovin_home / "moa-traces"
