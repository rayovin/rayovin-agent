...
def test_translate_native_response_surfaces_reasoning_and_tool_calls():
        ...
        "candidates": [
            {
                "content": {
                    "parts": [
                        ...
                        {"functionCall": {"name": "search", "args": {"q": "rayovin"}}},
                    ]
                },
                "finishReason": "STOP",
            }
    ...
    assert json.loads(choice.message.tool_calls[0].function.arguments) == {"q": "rayovin"}
...
def test_max_tokens_none_defaults_to_gemini_output_ceiling():
    ...
    maxOutputTokens is absent, truncating tool calls mid-stream. Rayovin passes
...
def test_x_goog_api_client_header_is_set():
    ...
    assert "rayovin-agent/" in headers["X-Goog-Api-Client"], (
        "rayovin-agent not found in X-Goog-Api-Client header"
    )


def test_x_goog_api_client_header_format():
    """Header value should be 'rayovin-agent/<version>' matching the package version."""
    from agent.gemini_native_adapter import GeminiNativeClient, _RAYOVIN_VERSION

    client = GeminiNativeClient(api_key="fake-key", model="gemini-2.0-flash")
    headers = client._headers()

    expected = f"rayovin-agent/{_RAYOVIN_VERSION}"
    assert headers["X-Goog-Api-Client"] == expected


def test_user_agent_contains_version():
    """User-Agent should include the rayovin-agent version."""
    from agent.gemini_native_adapter import GeminiNativeClient, _RAYOVIN_VERSION

    client = GeminiNativeClient(api_key="fake-key", model="gemini-2.0-flash")
    headers = client._headers()

    assert f"rayovin-agent/{_RAYOVIN_VERSION}" in headers["User-Agent"]


def test_rayovin_version_is_valid():
    """_RAYOVIN_VERSION should be a non-empty string."""
    from agent.gemini_native_adapter import _RAYOVIN_VERSION

    assert isinstance(_RAYOVIN_VERSION, str)
    assert len(_RAYOVIN_VERSION) > 0
    assert _RAYOVIN_VERSION != "0.0.0", (
        "Version should resolve from rayovin_cli.__version__, not the fallback"
    )
