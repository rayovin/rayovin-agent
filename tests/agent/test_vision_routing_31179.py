...
def isolated_home(monkeypatch):
    """Temp RAYOVIN_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="rayovin_test_31179_")
    rayovin_home = os.path.join(test_home, ".rayovin")
    os.makedirs(rayovin_home)
...
def _fresh_modules():
    """Drop cached rayovin modules so each test reloads against current env."""
