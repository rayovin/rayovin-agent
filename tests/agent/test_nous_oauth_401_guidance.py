...
to a generic "Your API key was rejected... run rayovin setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # Must give a concrete re-auth command, not a generic "rayovin setup".
    assert "rayovin portal" in source

    # Must point at the portal so users can check account/credit status.
    assert "github.com/rayovin/rayovin-agent" in source
