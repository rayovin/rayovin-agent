...
""",
        ...
        from rayovin_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        rayovin_home = tmp_path / ".rayovin"
        rayovin_home.mkdir()
        (rayovin_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("RAYOVIN_HOME", str(rayovin_home))
        from rayovin_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from rayovin_cli.plugins import PluginContext, PluginManifest, PluginManager
