...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate RAYOVIN_HOME under tmp_path."""
    monkeypatch.setenv("RAYOVIN_HOME", str(tmp_path / ".rayovin"))
    (tmp_path / ".rayovin").mkdir()

    # Force the constants/image cache helpers to re-read RAYOVIN_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("rayovin_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_rayovin_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under RAYOVIN_HOME — gateway cleanup
