...
class TestToolsetConsistency:
    ...
    def test_rayovin_platforms_share_core_tools(self):
        """All rayovin-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on rayovin-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["rayovin-cli", "rayovin-telegram", "rayovin-discord", "rayovin-whatsapp", "rayovin-slack", "rayovin-signal", "rayovin-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_rayovin_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("rayovin-whatsapp")

    def test_rayovin_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("rayovin-api-server")
