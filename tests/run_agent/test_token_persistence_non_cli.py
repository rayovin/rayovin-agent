...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    rayovin_state = ModuleType("rayovin_state")
    rayovin_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "rayovin_state", rayovin_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Rayovin"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Rayovin"
