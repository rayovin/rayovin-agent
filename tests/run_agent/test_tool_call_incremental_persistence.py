...
def _make_agent():
    rayovin_home = Path(tempfile.mkdtemp(prefix="rayovin-test-home-"))
    (rayovin_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._rayovin_home", rayovin_home),
