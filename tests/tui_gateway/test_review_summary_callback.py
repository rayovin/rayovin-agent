    ...
    with patch.dict(
        "sys.modules",
        {
            "rayovin_constants": MagicMock(
                get_rayovin_home=MagicMock(return_value="/tmp/rayovin_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'rayovin-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'rayovin-release' patched"
    }
