...
binary download are exercised manually by `rayovin secrets bitwarden
...
def rayovin_home(tmp_path, monkeypatch):
    """Point Rayovin at an isolated home directory."""
    home = tmp_path / ".rayovin"
    home.mkdir()
    monkeypatch.setenv("RAYOVIN_HOME", str(home))
    # Some modules cache get_rayovin_home; clear if needed.
    import rayovin_constants
    if hasattr(rayovin_constants, "_RAYOVIN_HOME_CACHE"):
        rayovin_constants._RAYOVIN_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(rayovin_home, monkeypatch):
...
def test_install_bws_happy_path(rayovin_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(rayovin_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(rayovin_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from rayovin_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under rayovin_home/cache/."""
    home = tmp_path / ".rayovin"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
