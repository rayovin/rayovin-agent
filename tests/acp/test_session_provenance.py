...
def test_root_session_no_compression(db):
    ...
    assert prov["currentRayovinSessionId"] == "root1"
    assert prov["rootRayovinSessionId"] == "root1"
    assert prov["parentRayovinSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_rayovin_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentRayovinSessionId"] == "old"
    assert prov["rootRayovinSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousRayovinSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootRayovinSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootRayovinSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_rayovin_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousRayovinSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"rayovin"}
    assert "sessionProvenance" in meta["rayovin"]
    assert meta["rayovin"]["sessionProvenance"]["currentRayovinSessionId"] == "root1"
