...
Uses a throwaway RAYOVIN_HOME so it never touches ~/.rayovin. No real names.
"""
...
def _fresh_home():
    """Point RAYOVIN_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_rayovin_home() at import time)."""
...
# Import AFTER RAYOVIN_HOME is set.
...
# a different get_rayovin_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"RAYOVIN_HOME (throwaway): {HOME}")
