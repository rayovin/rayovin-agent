"""Regression tests for #34107 — Docker UID/GID handling in ensure_rayovin_home.

When Rayovin runs in Docker with ``RAYOVIN_UID=1000`` / ``RAYOVIN_GID=911``,
the entrypoint chowns the top-level ``RAYOVIN_HOME`` once at startup. But
...
The fix is a ``_chown_to_rayovin_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_rayovin_uid_gid
# ---------------------------------------------------------------------------


class TestResolveRayovinUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("RAYOVIN_UID", "1000")
        monkeypatch.setenv("RAYOVIN_GID", "911")
        from rayovin_cli.config import _resolve_rayovin_uid_gid
        uid, gid = _resolve_rayovin_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("RAYOVIN_UID", raising=False)
        monkeypatch.delenv("RAYOVIN_GID", raising=False)
        from rayovin_cli.config import _resolve_rayovin_uid_gid
        uid, gid = _resolve_rayovin_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("RAYOVIN_UID", "not-a-number")
        monkeypatch.setenv("RAYOVIN_GID", "911")
        from rayovin_cli.config import _resolve_rayovin_uid_gid
        uid, gid = _resolve_rayovin_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("RAYOVIN_UID", "")
        monkeypatch.setenv("RAYOVIN_GID", "")
        from rayovin_cli.config import _resolve_rayovin_uid_gid
        uid, gid = _resolve_rayovin_uid_gid()
        assert uid is None
        assert gid is None
...
# ---------------------------------------------------------------------------
# _chown_to_rayovin_uid
# ---------------------------------------------------------------------------


class TestChownToRayovinUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("RAYOVIN_UID", "1000")
        monkeypatch.setenv("RAYOVIN_GID", "911")
        from rayovin_cli import config as cfg

        d = tmp_path / "subdir"
        d.mkdir()

        with patch.object(cfg.os, "chown") as mock_chown:
            cfg._chown_to_rayovin_uid(d)
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_rayovin_uid(d)  # must not raise
