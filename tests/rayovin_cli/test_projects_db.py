"""Tests for the per-profile Projects store (rayovin_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Rayovin Agent", folders=["/tmp/rayovin"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "rayovin-agent"
    assert proj.name == "Rayovin Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/rayovin"
    assert [f.path for f in proj.folders] == ["/tmp/rayovin"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "rayovin-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Rayovin Agent")
    pdb.create_project(conn, name="Rayovin Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["rayovin-agent", "rayovin-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' RAYOVIN_HOME.
...
def test_db_path_under_rayovin_home():
    # Resolves under RAYOVIN_HOME (set by the autouse isolation fixture).
