"""Tests for the top-level `./rayovin` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./rayovin` should use `rayovin_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "rayovin"
    called = []

    fake_main_module = types.ModuleType("rayovin_cli.main")

    def fake_main():
        called.append("rayovin_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "rayovin_cli.main", fake_main_module)
    ...
    assert called == ["rayovin_cli.main"]
