"""Tests for rayovin_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import rayovin_cli.gui_uninstall as gu


def _make_agent(rayovin_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = rayovin_home / "rayovin-agent"
    (agent_root / "rayovin_cli").mkdir(parents=True)
    (agent_root / "rayovin_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(rayovin_home: Path) -> None:
    """Create the source-built GUI artifacts a `rayovin desktop` run produces."""
    desktop = rayovin_home / "rayovin-agent" / "apps" / "desktop"
    ...
    (rayovin_home / "rayovin-agent" / "node_modules").mkdir(parents=True)
    (rayovin_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(rayovin_home: Path) -> None:
    (rayovin_home / "config.yaml").write_text("x: 1\n")
    (rayovin_home / ".env").write_text("KEY=secret\n")
    (rayovin_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    rayovin_home = tmp_path / ".rayovin"
    rayovin_home.mkdir()
    assert gu.agent_is_installed(rayovin_home) is False
    _make_agent(rayovin_home)
    assert gu.agent_is_installed(rayovin_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    rayovin_home = tmp_path / ".rayovin"
    (rayovin_home / "rayovin-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(rayovin_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    rayovin_home = tmp_path / ".rayovin"
    _make_gui_build(rayovin_home)
    artifacts = gu.source_built_gui_artifacts(rayovin_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(rayovin_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(rayovin_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    rayovin_home = tmp_path / ".rayovin"
    agent_root = _make_agent(rayovin_home)
    _make_gui_build(rayovin_home)
    _make_user_data(rayovin_home)
    ...
    removed = gu.uninstall_gui(rayovin_home)
    ...
    assert not (rayovin_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "rayovin_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (rayovin_home / "config.yaml").exists()
    assert (rayovin_home / ".env").exists()
    assert (rayovin_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    rayovin_home = tmp_path / ".rayovin"
    _make_agent(rayovin_home)
    userdata = tmp_path / "Rayovin-userdata"
    userdata.mkdir()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(rayovin_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(rayovin_home)
    ...
    assert summary["rayovin_home"] == str(rayovin_home)
    assert summary["platform"] == sys.platform


def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Rayovin"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Rayovin"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Rayovin"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Rayovin"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    RAYOVIN_HOME with all the destructive externals stubbed out.
    """
    import rayovin_cli.uninstall as uninstall
    ...
    from rayovin_cli import gui_uninstall as gu_mod
...
def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole RAYOVIN_HOME non-interactively."""
    import rayovin_cli.uninstall as uninstall

    rayovin_home = tmp_path / ".rayovin"
    (rayovin_home / "rayovin-agent" / "rayovin_cli").mkdir(parents=True)
    (rayovin_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not rayovin_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m rayovin_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_rayovin_home", lambda: rayovin_home)
    ...
    assert not (desktop / "release").exists()
    assert not (rayovin_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "rayovin_cli").exists()
    assert (rayovin_home / "config.yaml").exists()
