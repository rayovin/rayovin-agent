...
``rayovin_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``rayovin --help``, ``rayovin version``,
``rayovin logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``rayovin --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from rayovin_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["rayovin", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["rayovin", "-w", "--tui"], None),
        (["rayovin", "version"], "version"),
        (["rayovin", "--tui", "chat"], "chat"),
        (["rayovin", "-w", "logs"], "logs"),
        (["rayovin", "chat", "hello world"], "chat"),
        (["rayovin", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["rayovin", "-m", "gpt5", "chat"], "chat"),
        (["rayovin", "--model", "gpt5", "chat", "hi"], "chat"),
        (["rayovin", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["rayovin", "-z", "hello world"], None),
        (["rayovin", "-z", "hello", "chat"], "chat"),
        (["rayovin", "--model=gpt5", "chat"], "chat"),     # inline form
        (["rayovin", "--", "chat"], "chat"),               # -- terminator
        (["rayovin", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["rayovin", "some-plugin-cmd"], "some-plugin-cmd"),
        (["rayovin", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["rayovin"],                          # bare → chat
        ["rayovin", "--help"],                # top-level help
        ["rayovin", "-h"],
        ["rayovin", "version"],               # known built-in
        ["rayovin", "logs"],
        ["rayovin", "gateway", "run"],
        ["rayovin", "--tui"],
        ["rayovin", "-w", "--tui"],
        ["rayovin", "chat", "hi"],
        ["rayovin", "help"],                  # accepted built-in-ish
        ["rayovin", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["rayovin", "meet", "join"],          # potential google_meet plugin
        ["rayovin", "honcho", "status"],      # potential memory plugin
        ["rayovin", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``rayovin help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"rayovin_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
