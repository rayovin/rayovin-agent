...
def rayovin_home(tmp_path, monkeypatch):
    ...
    # /etc/rayovin on the dev/CI box can't influence the test.
    monkeypatch.setenv("RAYOVIN_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(rayovin_home, monkeypatch):
    from rayovin_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(rayovin_home, monkeypatch):
    from rayovin_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(rayovin_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_rayovin_dotenv(rayovin_home=str(home))
