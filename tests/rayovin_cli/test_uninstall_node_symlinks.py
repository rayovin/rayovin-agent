"""Tests for rayovin_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $RAYOVIN_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Rayovin-managed node dir.
"""
...
def _make_rayovin_node(rayovin_home: Path) -> Path:
    """Create a fake $RAYOVIN_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = rayovin_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_rayovin_node(fake_home):
    rayovin_home = fake_home / ".rayovin"
    node_bin = _make_rayovin_node(rayovin_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(rayovin_home)
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    rayovin_home = fake_home / ".rayovin"
    _make_rayovin_node(rayovin_home)
    local_bin = fake_home / ".local" / "bin"
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(rayovin_home) == []


def test_removes_dangling_symlink_into_rayovin_node(fake_home):
    """A link into the Rayovin node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the Rayovin links that exist; ignores the ones that don't."""
    rayovin_home = fake_home / ".rayovin"
    node_bin = _make_rayovin_node(rayovin_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are Rayovin-managed; node is a real user binary.
