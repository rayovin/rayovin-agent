...
the SAME provider universe as ``rayovin model`` (the CLI/TUI picker).
...
def test_every_rayovin_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `rayovin model` universe."""
    ...
    assert not missing, (
        "providers shown in `rayovin model` but not configurable in the desktop "
