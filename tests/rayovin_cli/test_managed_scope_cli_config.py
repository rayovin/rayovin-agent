...
rayovin_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `rayovin config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_rayovin_home = get_rayovin_home()`` at import time (module
    singleton), so monkeypatching RAYOVIN_HOME after import doesn't move it.
    ...
    this only matters for tests that swap RAYOVIN_HOME.)
    """
    import cli

    cli._rayovin_home = home
