...
Before the fix, ``rayovin whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``rayovin gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    rayovin = home / ".rayovin"
    rayovin.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("RAYOVIN_HOME", str(rayovin))
...
def _env_value(rayovin_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from rayovin_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("rayovin_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``rayovin whatsapp`` with an existing paired session and
