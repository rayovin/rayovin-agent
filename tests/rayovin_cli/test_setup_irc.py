"""Tests for IRC gateway configuration via `rayovin setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Rayovin install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Rayovin install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#rayovin")
            monkeypatch.setenv("IRC_NICKNAME", "rayovin-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `rayovin setup gateway` experience with IRC present."""
