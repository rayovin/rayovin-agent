"""Tests for the `rayovin memory reset` CLI command.
...
- Profile-scoped reset (uses RAYOVIN_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake RAYOVIN_HOME with memory files."""
    rayovin_home = tmp_path / ".rayovin"
    memories = rayovin_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("RAYOVIN_HOME", str(rayovin_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nRayovin repo is at ~/.rayovin/rayovin-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `rayovin memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        rayovin_home = tmp_path / ".rayovin"
        (rayovin_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("RAYOVIN_HOME", str(rayovin_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's RAYOVIN_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_rayovin_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
