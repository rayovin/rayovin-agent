...
def container_env(tmp_path, monkeypatch):
    """Set up a fake RAYOVIN_HOME with .container-mode file."""
    rayovin_home = tmp_path / ".rayovin"
    rayovin_home.mkdir()
    monkeypatch.setenv("RAYOVIN_HOME", str(rayovin_home))
    monkeypatch.delenv("RAYOVIN_DEV", raising=False)

    container_mode = rayovin_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=rayovin-agent\n"
        "exec_user=rayovin\n"
        "rayovin_bin=/data/current-package/bin/rayovin\n"
    )
    return rayovin_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("rayovin_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "rayovin-agent"
    assert info["exec_user"] == "rayovin"
    assert info["rayovin_bin"] == "/data/current-package/bin/rayovin"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("rayovin_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_rayovin_dev(container_env, monkeypatch):
    """Returns None when RAYOVIN_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("RAYOVIN_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_rayovin_dev_zero(container_env, monkeypatch):
    """RAYOVIN_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("RAYOVIN_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        rayovin_home = Path(tmpdir) / ".rayovin"
        rayovin_home.mkdir()
        (rayovin_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("rayovin_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_rayovin_home": lambda: rayovin_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("RAYOVIN_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=rayovin-custom\n"
        "exec_user=myuser\n"
        "rayovin_bin=/opt/rayovin/bin/rayovin\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "rayovin-custom"
    assert info["exec_user"] == "myuser"
    assert info["rayovin_bin"] == "/opt/rayovin/bin/rayovin"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from rayovin_cli.main import _exec_in_container
