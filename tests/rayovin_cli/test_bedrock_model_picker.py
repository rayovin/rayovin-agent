...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (RAYOVIN_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via RAYOVIN_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in RAYOVIN_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from rayovin_cli.providers import RAYOVIN_OVERLAYS
        assert "bedrock" in RAYOVIN_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from rayovin_cli.providers import RAYOVIN_OVERLAYS
        assert RAYOVIN_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from rayovin_cli.providers import RAYOVIN_OVERLAYS
        assert RAYOVIN_OVERLAYS["bedrock"].auth_type == "aws_sdk"
