"""Shared fixtures for the rayovin-agent test suite.
...
2. **Isolated RAYOVIN_HOME.** RAYOVIN_HOME points to a per-test tempdir so
   code reading ``~/.rayovin/*`` via ``get_rayovin_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".rayovin"`` instead of the canonical
   ``get_rayovin_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No RAYOVIN_SESSION_* inheritance** — the agent's current gateway
...
# RAYOVIN_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_RAYOVIN_BEHAVIORAL_VARS = frozenset({
    "RAYOVIN_YOLO_MODE",
    "RAYOVIN_INTERACTIVE",
    "RAYOVIN_QUIET",
    "RAYOVIN_TOOL_PROGRESS",
    "RAYOVIN_TOOL_PROGRESS_MODE",
    "RAYOVIN_MAX_ITERATIONS",
    "RAYOVIN_SESSION_PLATFORM",
    "RAYOVIN_SESSION_CHAT_ID",
    "RAYOVIN_SESSION_CHAT_NAME",
    "RAYOVIN_SESSION_THREAD_ID",
    "RAYOVIN_SESSION_SOURCE",
    "RAYOVIN_SESSION_KEY",
    "RAYOVIN_GATEWAY_SESSION",
    "RAYOVIN_CRON_SESSION",
    "_RAYOVIN_GATEWAY",
    "RAYOVIN_PLATFORM",
    "RAYOVIN_MODEL",
    "RAYOVIN_INFERENCE_MODEL",
    "RAYOVIN_INFERENCE_PROVIDER",
    "RAYOVIN_TUI_PROVIDER",
    "RAYOVIN_MANAGED",
    "RAYOVIN_MANAGED_DIR",
    "RAYOVIN_DEV",
    "RAYOVIN_CONTAINER",
    ...
    "RAYOVIN_TIMEZONE",
    "RAYOVIN_REDACT_SECRETS",
    "RAYOVIN_BACKGROUND_NOTIFICATIONS",
    "RAYOVIN_EXEC_ASK",
    "RAYOVIN_HOME_MODE",
    "RAYOVIN_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.rayovin/kanban.db instead of the per-test RAYOVIN_HOME.
    "RAYOVIN_KANBAN_DB",
    "RAYOVIN_KANBAN_BOARD",
    "RAYOVIN_KANBAN_HOME",
    "RAYOVIN_KANBAN_WORKSPACES_ROOT",
    "RAYOVIN_KANBAN_LOGS_ROOT",
    "RAYOVIN_KANBAN_TASK",
    "RAYOVIN_KANBAN_WORKSPACE",
    "RAYOVIN_KANBAN_RUN_ID",
    "RAYOVIN_KANBAN_CLAIM_LOCK",
    "RAYOVIN_KANBAN_DISPATCH_IN_GATEWAY",
    "RAYOVIN_TENANT",
    ...
    "RAYOVIN_DASHBOARD_OAUTH_CLIENT_ID",
    "RAYOVIN_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and RAYOVIN_HOME to per-test tempdirs so code that
    reads ``~/.rayovin/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral RAYOVIN_* vars that could change test semantics.
    for name in _RAYOVIN_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect RAYOVIN_HOME to a per-test tempdir. Code that reads
    #    ``~/.rayovin/*`` via ``get_rayovin_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.rayovin/*`` via
    #    ``Path.home() / ".rayovin"`` instead of ``get_rayovin_home()``
    #    is a bug to fix at the callsite.
    fake_rayovin_home = tmp_path / "rayovin_test"
    fake_rayovin_home.mkdir()
    (fake_rayovin_home / "sessions").mkdir()
    (fake_rayovin_home / "cron").mkdir()
    (fake_rayovin_home / "memories").mkdir()
    (fake_rayovin_home / "skills").mkdir()
    monkeypatch.setenv("RAYOVIN_HOME", str(fake_rayovin_home))
    ...
    #    ~/.rayovin/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import rayovin_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_rayovin_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal rayovin config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``rayovin-gateway`` process
...
# correlated with a ``tests/rayovin_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> rayovin-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart rayovin-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting rayovin/python patterns are also blocked.
    """
    ...
    def _matches_rayovin_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _RAYOVIN_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_rayovin_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch rayovin-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m rayovin_cli.main".
                if (
                    "rayovin" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live rayovin-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting rayovin/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `rayovin update` (or the
        # equivalent `python -m rayovin_cli.main update`).  These commands
        ...
        if "update" in low and (
            # rayovin update / rayovin update --gateway / setsid bash -c ... rayovin update
            ("rayovin" in low and "update" in low.split())
            or
            # python -m rayovin_cli.main update --gateway
            ("rayovin_cli" in low and "update" in low.split())
            or
            # venv/bin/rayovin update  (absolute path variant used in tests)
            (".venv/bin/rayovin" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`rayovin update` against the real checkout, fetching "
