"""Tests for rayovin_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Rayovin entry point
(rayovin, rayovin-agent, rayovin-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Rayovin entry point imports rayovin_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported rayovin_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("rayovin_bootstrap", None)
    import rayovin_bootstrap  # noqa: WPS433
    return rayovin_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Rayovin CLI
...
class TestEntryPointsImportBootstrap:
    """Every Rayovin entry point must import rayovin_bootstrap as its
    ...
    # Entry points that invoke Rayovin as a process.  Each one must
    # import rayovin_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "rayovin_cli/main.py",   # rayovin CLI (console_script)
        "run_agent.py",          # rayovin-agent (console_script)
        "acp_adapter/entry.py",  # rayovin-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import rayovin_bootstrap' and that
        ...
        half-finished ``rayovin update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``rayovin_bootstrap`` as a top-level module) leaves rayovin
        ...
        """
        # Resolve relative to the rayovin-agent repo root.  Tests live
        # at tests/test_rayovin_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # rayovin start even when rayovin_bootstrap hasn't been
        ...
        assert first_import_name == "rayovin_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'rayovin_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import rayovin_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Rayovin's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("RAYOVIN_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["RAYOVIN_PYTHON_SRC_ROOT"] = env
            elif "RAYOVIN_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["RAYOVIN_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/rayovin")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("RAYOVIN_PYTHON_SRC_ROOT", None)
            else:
                os.environ["RAYOVIN_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/rayovin", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Rayovin root.  The guard must relocate Rayovin to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/rayovin"])
        assert result[0] == "/opt/rayovin"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Rayovin root.
        assert result.index("/opt/rayovin") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/rayovin", "/opt/rayovin", ""])
        assert result.count("/opt/rayovin") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["RAYOVIN_PYTHON_SRC_ROOT"] = "/env/rayovin"
            hb.harden_import_path()
            assert sys.path[0] == "/env/rayovin"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("RAYOVIN_PYTHON_SRC_ROOT", None)
