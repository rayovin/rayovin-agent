...
from rayovin_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(rayovin_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <rayovin_home>/plugins/<name> and opt it in."""
    plugin_dir = rayovin_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("RAYOVIN_HOME", str(tmp_path / "rayovin_empty"))
