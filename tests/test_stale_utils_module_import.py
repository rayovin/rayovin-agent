...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.rayovin/rayovin-agent/utils.py)
...
2. ``rayovin update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``rayovin_cli/main.py`` (the ``rayovin update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``rayovin_constants`` after the
