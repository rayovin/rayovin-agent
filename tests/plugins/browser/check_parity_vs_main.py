...
Run from the PR worktree:
    ...
    cd ~/.rayovin/rayovin-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.rayovin/rayovin-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.rayovin/rayovin-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a rayovin-agent checkout"
)
...
# Isolated RAYOVIN_HOME for the config write.
