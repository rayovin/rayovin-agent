...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._rayovin_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(rayovin_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(rayovin_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_rayovin_bin(self):
        """After _INSTALL_FAILED, manual install in RAYOVIN_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        rayovin_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(rayovin_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(rayovin_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._rayovin_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == rayovin_bin
            assert _tirith_mod._resolved_path == rayovin_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# RAYOVIN_HOME isolation
# ---------------------------------------------------------------------------

class TestRayovinHomeIsolation:
    def test_rayovin_bin_dir_respects_rayovin_home(self):
        """_rayovin_bin_dir must use RAYOVIN_HOME, not hardcoded ~/.rayovin."""
        from tools.tirith_security import _rayovin_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"RAYOVIN_HOME": tmpdir}):
            result = _rayovin_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_rayovin_home(self):
        """_failure_marker_path must use RAYOVIN_HOME, not hardcoded ~/.rayovin."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"RAYOVIN_HOME": "/custom/rayovin"}):
            result = _failure_marker_path()
        assert result == "/custom/rayovin/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets RAYOVIN_HOME; verify it's active."""
        rayovin_home = os.getenv("RAYOVIN_HOME")
        assert rayovin_home is not None, "RAYOVIN_HOME should be set by conftest"
        assert "rayovin_test" in rayovin_home, "Should point to test temp dir"

    def test_get_rayovin_home_fallback(self):
        """Without RAYOVIN_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_rayovin_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove RAYOVIN_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".rayovin")
            result = _get_rayovin_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._rayovin_bin_dir",
