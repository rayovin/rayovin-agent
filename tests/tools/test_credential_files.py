...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        rayovin_home.mkdir()
        (rayovin_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"RAYOVIN_HOME": str(rayovin_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(rayovin_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.rayovin/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        rayovin_home = tmp_path / ".rayovin"
        rayovin_home.mkdir()
        (rayovin_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        rayovin_home = tmp_path / ".rayovin"
        rayovin_home.mkdir()
        (rayovin_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        rayovin_home = tmp_path / ".rayovin"
        rayovin_home.mkdir()
        (rayovin_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        rayovin_home = tmp_path / ".rayovin"
        skills_dir = rayovin_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.rayovin/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        rayovin_home = tmp_path / ".rayovin"
        (rayovin_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"RAYOVIN_HOME": str(rayovin_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.rayovin")

        assert mounts[0]["container_path"] == "/home/user/.rayovin/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.rayovin/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.rayovin/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape RAYOVIN_HOME."""
        monkeypatch.setenv("RAYOVIN_HOME", str(tmp_path / ".rayovin"))
        (tmp_path / ".rayovin").mkdir()

        # Create a sensitive file one level above rayovin_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside rayovin_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside RAYOVIN_HOME must still be registered."""
        rayovin_home = tmp_path / ".rayovin"
        rayovin_home.mkdir()
        monkeypatch.setenv("RAYOVIN_HOME", str(rayovin_home))
        (rayovin_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_rayovin_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of RAYOVIN_HOME must be allowed."""
        rayovin_home = tmp_path / ".rayovin"
        rayovin_home.mkdir()
        subdir = rayovin_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside RAYOVIN_HOME pointing outside must be rejected."""
        rayovin_home = tmp_path / ".rayovin"
        rayovin_home.mkdir()
        monkeypatch.setenv("RAYOVIN_HOME", str(rayovin_home))

        # Create a sensitive file outside rayovin_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside rayovin_home pointing outside
        symlink = rayovin_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes RAYOVIN_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, rayovin_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape RAYOVIN_HOME."""
        rayovin_home = tmp_path / ".rayovin"
        rayovin_home.mkdir()
        monkeypatch.setenv("RAYOVIN_HOME", str(rayovin_home))
        ...
        sensitive.write_text("{}")
        self._write_config(rayovin_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(rayovin_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside RAYOVIN_HOME via config must still mount."""
        rayovin_home = tmp_path / ".rayovin"
        rayovin_home.mkdir()
        monkeypatch.setenv("RAYOVIN_HOME", str(rayovin_home))

        (rayovin_home / "oauth.json").write_text("{}")
        self._write_config(rayovin_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        rayovin_home = tmp_path / ".rayovin"
        rayovin_home.mkdir()
        (rayovin_home / "cache" / "documents").mkdir(parents=True)
        (rayovin_home / "cache" / "audio").mkdir(parents=True)
        (rayovin_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("RAYOVIN_HOME", str(rayovin_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.rayovin/cache/documents" in paths
        assert "/root/.rayovin/cache/audio" in paths
        assert "/root/.rayovin/cache/videos" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.rayovin/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_rayovin_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        rayovin_home = tmp_path / ".rayovin"
        rayovin_home.mkdir()
        # Use legacy dir name with content — get_rayovin_dir prefers
        # populated old over new.
        legacy_doc = rayovin_home / "document_cache"
        legacy_img = rayovin_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(rayovin_home / "document_cache") in host_paths
        assert str(rayovin_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.rayovin/cache/documents" in container_paths
        assert "/root/.rayovin/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        rayovin_home = tmp_path / ".rayovin"
        img_dir = rayovin_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.rayovin/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.rayovin")
            == "/home/agent/.rayovin/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        rayovin_home = tmp_path / ".rayovin"
        (rayovin_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("RAYOVIN_HOME", str(rayovin_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(rayovin_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        rayovin_home = tmp_path / ".rayovin"
        doc_dir = rayovin_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        rayovin_home = tmp_path / ".rayovin"
        ss_dir = rayovin_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.rayovin/cache/screenshots/session_abc/screen1.png"
