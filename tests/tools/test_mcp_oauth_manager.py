...
def test_rayovin_provider_subclass_exists():
    """RayovinMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _RAYOVIN_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _RAYOVIN_PROVIDER_CLS is not None
    assert issubclass(_RAYOVIN_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_rayovin_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns RayovinMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _RAYOVIN_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("RAYOVIN_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _RAYOVIN_PROVIDER_CLS is not None
    assert isinstance(provider, _RAYOVIN_PROVIDER_CLS)
    assert provider._rayovin_server_name == "srv"
