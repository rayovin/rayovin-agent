...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad RAYOVIN_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: RAYOVIN_GATEWAY_SESSION set, a bound session
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setattr(A, "_YOLO_MODE_FROZEN", False)
    monkeypatch.setenv("RAYOVIN_CRON_SESSION", "1")
    monkeypatch.delenv("RAYOVIN_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_rayovin_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "RAYOVIN_HOME": "/h", "RAYOVIN_PROFILE": "p",
        "RAYOVIN_CONFIG": "/c.yaml", "RAYOVIN_ENV": "/e",
        # other RAYOVIN_* → dropped (broad prefix removed)
        "RAYOVIN_BASE_URL": "https://x", "RAYOVIN_INTERACTIVE": "1",
        "RAYOVIN_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("RAYOVIN_HOME", "RAYOVIN_PROFILE", "RAYOVIN_CONFIG", "RAYOVIN_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "RAYOVIN_BASE_URL", "RAYOVIN_INTERACTIVE", "RAYOVIN_KANBAN_DB",
...
def test_env_scrub_logs_dropped_rayovin_vars(caplog):
    """Dropping a non-allowlisted, non-secret RAYOVIN_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset RAYOVIN_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "RAYOVIN_HOME": "/h",          # allowlisted → kept, not logged
        "RAYOVIN_BASE_URL": "https://x",   # dropped → logged
        "RAYOVIN_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "RAYOVIN_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "RAYOVIN_HOME" in out and "PATH" in out
    assert "RAYOVIN_BASE_URL" not in out and "RAYOVIN_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "RAYOVIN_BASE_URL" in msgs and "RAYOVIN_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "RAYOVIN_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped RAYOVIN_* vars."""
    import logging
