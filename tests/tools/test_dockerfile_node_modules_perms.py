...
Older images made /opt/rayovin/ui-tui and /opt/rayovin/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV RAYOVIN_TUI_DIR=/opt/rayovin/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R rayovin:rayovin /opt/rayovin/ui-tui" not in text
    assert "chown -R rayovin:rayovin /opt/rayovin/node_modules" not in text
