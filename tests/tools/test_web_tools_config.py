...
class TestFirecrawlClientConfig:
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "github.com/rayovin/rayovin-agent"}):
            with patch("tools.web_tools._read_nous_access_token", return_value="nous-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="https://firecrawl-gateway.github.com/rayovin/rayovin-agent",
                    )
        ...
        with patch.dict(os.environ, {
            ...
            with patch("tools.web_tools._read_nous_access_token", return_value="nous-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="http://firecrawl-gateway.github.com/rayovin/rayovin-agent",
                    )
                    assert result is mock_fc.return_value
    ...
    def test_nous_auth_token_respects_rayovin_home_override(self, tmp_path):
        """Auth lookup should read from RAYOVIN_HOME/auth.json, not ~/.rayovin/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".rayovin").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``rayovin tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestCheckWebApiKey:
    ...
    def test_null_web_section_does_not_crash(self):
        ...
        with patch("rayovin_cli.config.load_config", return_value={"web": None}):
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "rayovin_cli.auth.resolve_nous_access_token",
            _record_refresh,
        )
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from rayovin_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.rayovin/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.rayovin/.env), not bare os.getenv. Parametrized over the four
