...
class TestWriteDenyExactPaths:
    ...
    def test_rayovin_env(self):
        # ``.env`` under the active RAYOVIN_HOME (profile-aware, not just
        # ``~/.rayovin``) must be write-denied. The hermetic test conftest
        # points RAYOVIN_HOME at a tempdir — resolve via get_rayovin_home()
        # to match the denylist.
        from rayovin_constants import get_rayovin_home
        path = str(get_rayovin_home() / ".env")
        assert _is_write_denied(path) is True

    def test_rayovin_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.rayovin/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "rayovin_root"
        ...
        # Sanity check: RAYOVIN_HOME does point to the profile dir, not the root.
        from rayovin_constants import get_rayovin_home, get_default_rayovin_root
        assert get_rayovin_home() == profile_home
        assert get_default_rayovin_root() == root
...
class TestWriteAllowed:
    ...
    def test_rayovin_control_files_requested_writable(self):
        from rayovin_constants import get_rayovin_home
