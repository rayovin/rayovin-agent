"""Regression tests for RAYOVIN_HOME override propagation onto the MCP loop.
...
get_rayovin_home() inside the coroutine, most visibly OAuth token-store
...
def test_oauth_token_paths_follow_override(tmp_path, monkeypatch, mcp_loop):
    """The actual symptom path: RayovinTokenStorage resolving inside the
    ...
    async def token_path():
        ...
        return str(RayovinTokenStorage("probe-srv")._tokens_path())
