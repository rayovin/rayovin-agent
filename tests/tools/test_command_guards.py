...
def _clean_state():
    ...
    saved = {}
    for k in ("RAYOVIN_INTERACTIVE", "RAYOVIN_GATEWAY_SESSION", "RAYOVIN_EXEC_ASK", "RAYOVIN_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["RAYOVIN_INTERACTIVE"] = "1"
        session_key = os.getenv("RAYOVIN_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No RAYOVIN_INTERACTIVE or RAYOVIN_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
