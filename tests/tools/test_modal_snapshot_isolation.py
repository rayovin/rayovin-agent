...
def _restore_tool_modules():
    original_rayovin_home = os.environ.get("RAYOVIN_HOME")
    original_modules = {
        name: module
        ...
        or name == "rayovin_cli"
        or name.startswith("rayovin_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_rayovin_home is None:
            os.environ.pop("RAYOVIN_HOME", None)
        else:
            os.environ["RAYOVIN_HOME"] = original_rayovin_home
        _reset_modules(("tools", "rayovin_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    rayovin_cli = types.ModuleType("rayovin_cli")
    rayovin_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["rayovin_cli"] = rayovin_cli
    rayovin_home = tmp_path / "rayovin-home"
    os.environ["RAYOVIN_HOME"] = str(rayovin_home)
    sys.modules["rayovin_cli.config"] = types.SimpleNamespace(
        get_rayovin_home=lambda: rayovin_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="rayovin-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": rayovin_home / "modal_snapshots.json",
