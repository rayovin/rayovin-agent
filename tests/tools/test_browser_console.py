...
class TestBrowserConsole:
    ...
    def test_allow_unsafe_evaluate_reads_browser_config(self):
        ...
        with patch("rayovin_cli.config.read_raw_config", return_value={"browser": {"allow_unsafe_evaluate": "true"}}):
            assert _allow_unsafe_browser_evaluate() is True
        with patch("rayovin_cli.config.read_raw_config", return_value={"browser": {"allow_unsafe_evaluate": False}}):
...
class TestBrowserConsoleToolsetWiring:
    ...
    def test_in_rayovin_core_tools(self):
        from toolsets import _RAYOVIN_CORE_TOOLS
        assert "browser_console" in _RAYOVIN_CORE_TOOLS
...
class TestBrowserVisionConfig:
    ...
    def test_browser_vision_uses_configured_temperature_and_timeout(self, tmp_path):
        ...
        with (
            patch("rayovin_constants.get_rayovin_dir", return_value=shots_dir),
            ...
            patch("tools.browser_tool._get_vision_model", return_value="test-model"),
            patch("rayovin_cli.config.load_config", return_value={"auxiliary": {"vision": {"temperature": 1, "timeout": 45}}}),
