...
The ``rayovin update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m rayovin_cli.main gateway run`` — image name
    ``pythonw``, not ``rayovin.exe`` — so the ``taskkill /IM rayovin.exe`` guard
    ...
    # The rayovin.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM rayovin.exe' in text
