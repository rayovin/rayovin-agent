"""ACP auth helpers — detect and advertise Rayovin authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "rayovin-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Rayovin runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Rayovin can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Rayovin.
    ...
    not have Rayovin provider credentials configured yet, so Rayovin always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Rayovin using the currently configured "
