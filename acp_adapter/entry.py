"""CLI entry point for the rayovin-agent ACP adapter.

Loads environment variables from ``~/.rayovin/.env``, configures logging
...
try:
    ...
    # shadowing Rayovin's own modules — ``rayovin acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    rayovin_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from RAYOVIN_HOME (default ``~/.rayovin``)."""
    from rayovin_cli.env_loader import load_rayovin_dotenv

    rayovin_home = get_rayovin_home()
    loaded = load_rayovin_dotenv(rayovin_home=rayovin_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", rayovin_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="rayovin-acp",
        description="Run Rayovin Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Rayovin version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Rayovin provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.rayovin/node/ "
...
def _print_version() -> None:
    ...
    print(rayovin_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import RayovinACPAgent  # noqa: F401

    print("Rayovin ACP check OK")


def _run_setup() -> None:
    from rayovin_cli.main import main as rayovin_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "rayovin", "model"]
        rayovin_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``rayovin postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting rayovin-agent ACP adapter")
    ...
    import acp
    from .server import RayovinACPAgent
