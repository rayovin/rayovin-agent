"""Windows UTF-8 bootstrap for Rayovin entry points.
...
should be imported at the very top of every Rayovin entry point
(``rayovin``, ``rayovin-agent``, ``rayovin-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Rayovin modules.

    Rayovin ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Rayovin source root
    ...
    """
    root = src_root or os.environ.get("RAYOVIN_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``RAYOVIN_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("RAYOVIN_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import rayovin_bootstrap``
# (or ``from rayovin_bootstrap import apply_windows_utf8_bootstrap``) at
