# OpenAI-Compatible API Server for Rayovin Agent
...
SSE streaming. By exposing this endpoint, rayovin-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  rayovin-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.rayovin/.env`
2. Run `rayovin gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with rayovin-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns rayovin-agent as a model) |
...
Authorization: Bearer rayovin-api-key-here
...
But rayovin-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "rayovin-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured rayovin-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of rayovin-agent's core prompt
...
Once implemented, rayovin-agent works as a drop-in backend for:
