"""
Dump command for rayovin CLI.

Outputs a compact, plain-text summary of the user's Rayovin setup
...
from rayovin_cli.config import get_rayovin_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.rayovin/.env.
    ...
    exports. ``rayovin debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.rayovin_build_sha`` by the Dockerfile's
    ``RAYOVIN_GIT_SHA`` build-arg (see ``rayovin_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``rayovin dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from rayovin_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(rayovin_home: Path) -> int:
...
def _cron_summary(rayovin_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = rayovin_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"rayovin_home:      {display_rayovin_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.rayovin/.env: a managed
        ...
        # A credential added via `rayovin auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `rayovin auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["rayovin-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(rayovin_home)}")
    lines.append(f"  skills:             {_count_skills(rayovin_home)}")
