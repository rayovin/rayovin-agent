...
the tool registry or launching Rayovin (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``rayovin-cli`` to the non-existent ``rayovin``. ``resolve_toolset('rayovin')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``rayovin-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``rayovin`` instead of ``rayovin-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `rayovin tools` to reconfigure."
        )
    return warnings
