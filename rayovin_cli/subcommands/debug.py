"""``rayovin debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Rayovin Agent. Use 'rayovin debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    rayovin debug share              Upload debug report (asks for confirmation)
    rayovin debug share --yes        Skip confirmation (for scripts/CI)
    rayovin debug share --lines 500  Include more log lines
    rayovin debug share --expire 30  Keep paste for 30 days
    rayovin debug share --local      Print report locally (no upload)
    rayovin debug share --no-redact  Disable upload-time secret redaction
    rayovin debug share --nous       Upload to Nous-internal storage (private)
    rayovin debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'rayovin debug share'",
    )
