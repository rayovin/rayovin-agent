"""
Rayovin Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.rayovin/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Rayovin PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing rayovin-agent or rayovin PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Rayovin Agent" comment and following line
                if '# Rayovin Agent' in line or '# rayovin-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('rayovin' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing rayovin
                if 'rayovin' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the rayovin wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "rayovin",
        Path("/usr/local/bin/rayovin"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains rayovin_cli reference)
                content = wrapper.read_text()
                if 'rayovin_cli' in content or 'rayovin-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(rayovin_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``rayovin`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Rayovin home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (rayovin_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``rayovin gateway run`` processes
    ...
    try:
        from rayovin_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from rayovin_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``RAYOVIN_HOME`` and ``RAYOVIN_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\rayovin\git\cmd``, ``%LOCALAPPDATA%\rayovin\git\bin``,
#      ``%LOCALAPPDATA%\rayovin\git\usr\bin``, ``%LOCALAPPDATA%\rayovin\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\rayovin\git\`` and Node to
#      ``%LOCALAPPDATA%\rayovin\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``rayovin dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\rayovin\gateway-service\`` and sometimes
...
def _rayovin_path_markers(rayovin_home: Path) -> list[str]:
    """Path-entry substrings that identify Rayovin-owned User-PATH entries."""
    root = str(rayovin_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare rayovin-agent install dir.
    markers = [root + "\\rayovin-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if RAYOVIN_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "rayovin".  We don't want to catch
    # unrelated entries like "crayovin-foo" or "ephermeral", so we look for
    # backslash-rayovin as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(rayovin_home: Path) -> list[str]:
    """Strip Rayovin-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _rayovin_path_markers(rayovin_home)
            kept: list[str] = []
...
def remove_rayovin_env_vars_windows() -> list[str]:
    """Delete RAYOVIN_HOME and RAYOVIN_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("RAYOVIN_HOME", "RAYOVIN_GIT_BASH_PATH"):
                try:
...
def _is_default_rayovin_home(rayovin_home: Path) -> bool:
    """Return True when ``rayovin_home`` points at the default (non-profile) root."""
    try:
        from rayovin_constants import get_default_rayovin_root
        return rayovin_home.resolve() == get_default_rayovin_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its RAYOVIN_HOME directory.

    We shell out to ``rayovin -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current RAYOVIN_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m rayovin_cli.main` so we don't depend on a `rayovin`
    #    wrapper that may be half-removed mid-uninstall.
    rayovin_invocation = [_sys.executable, "-m", "rayovin_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                rayovin_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's RAYOVIN_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``rayovin uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$RAYOVIN_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from rayovin_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(rayovin_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Rayovin Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Rayovin Chat GUI installation was found.")
        print(f"  Checked: {rayovin_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Rayovin agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(rayovin_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Rayovin agent at {rayovin_home / 'rayovin-agent'}")
        print(f"  • Your config, sessions, and secrets under {rayovin_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Rayovin agent is still installed. Run 'rayovin' to use the CLI,")
    print("or 'rayovin uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.rayovin/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.rayovin/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie RAYOVIN_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_rayovin_home(rayovin_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.rayovin data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Rayovin Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {rayovin_home / 'config.yaml'}")
    print(f"  Secrets: {rayovin_home / '.env'}")
    print(f"  Data:    {rayovin_home / 'cron/'}, {rayovin_home / 'sessions/'}, {rayovin_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their RAYOVIN_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Rayovin data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Rayovin code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, rayovin_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Rayovin PATH entries from shell configs / Windows User PATH")
    print("  • Rayovin wrapper scripts and Rayovin-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Rayovin config/data: {rayovin_home}")
        if _is_default_rayovin_home(rayovin_home):
        ...
        print(f"  • Keep Rayovin config/data: {rayovin_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``rayovin`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$RAYOVIN_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in rayovin_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\rayovin\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(rayovin_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Rayovin-owned PATH entries in User environment")

        log_info("Removing RAYOVIN_HOME / RAYOVIN_GIT_BASH_PATH User env vars...")
        removed_env = remove_rayovin_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Rayovin-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing rayovin command...")
    ...
    #     (only when they still point into this Rayovin home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Rayovin-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(rayovin_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Rayovin-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(rayovin_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE RAYOVIN_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from rayovin_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(rayovin_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.rayovin/, just remove the rayovin-agent subdir
            if rayovin_home in project_root.parents or project_root.parent == rayovin_home:
    ...
    #     under RAYOVIN_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(rayovin_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(rayovin_home)
    ...
    # 5. Optionally remove ~/.rayovin/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile RAYOVIN_HOME dirs live under
        ...
        try:
            if rayovin_home.exists():
                shutil.rmtree(rayovin_home)
                log_success(f"Removed {rayovin_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {rayovin_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {rayovin_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {rayovin_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://rayovin.github.io/rayovin-agent/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://rayovin.github.io/rayovin-agent/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Rayovin Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m rayovin_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import rayovin_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``rayovin_constants`` + ``rayovin_cli.colors``
    (and lazily ``rayovin_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m rayovin_cli.uninstall")
    parser.add_argument(
        "--mode",
