"""
rayovin fallback — manage the fallback provider chain.
...
Subcommands:
  rayovin fallback [list]   Show the current fallback chain (default when no subcommand)
  rayovin fallback add      Pick provider + model via the same picker as `rayovin model`,
                           then append the selection to the chain
  rayovin fallback remove   Pick an entry to delete from the chain
  rayovin fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.rayovin/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from rayovin_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `rayovin model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  rayovin fallback add")
        print()
        return
    ...
    print("  Docs: https://rayovin.github.io/rayovin-agent/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `rayovin model`, then append the selection to the chain."""
    from rayovin_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `rayovin model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `rayovin fallback list` to view, or `rayovin fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``rayovin fallback [subcommand]``."""
