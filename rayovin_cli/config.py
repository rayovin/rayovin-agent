"""
Configuration management for Rayovin Agent.

Config files are stored in ~/.rayovin/ for easy access:
- ~/.rayovin/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.rayovin/.env         - API keys and secrets

This module provides:
- rayovin config          - Show current configuration
- rayovin config edit     - Open config in editor
- rayovin config set      - Set a specific value
- rayovin config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``rayovin config set`` (which rewrites
    ...
    ``config.yaml`` in place: rayovin never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.rayovin/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``rayovin logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  rayovin config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Rayovin makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Rayovin itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``rayovin update``, the TUI build.
...
#   on every plugin install / ``rayovin update``.
...
# * ``RAYOVIN_HOME`` / ``RAYOVIN_PROFILE`` / ``RAYOVIN_CONFIG`` /
#   ``RAYOVIN_ENV`` — Rayovin runtime location flags. Writing these into
...
#
# IMPORTANT: ``RAYOVIN_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (RAYOVIN_LANGFUSE_PUBLIC_KEY,
# RAYOVIN_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Rayovin runtime location — never via dashboard env writer.
    # NOT a RAYOVIN_* blanket: integration credentials (RAYOVIN_GEMINI_*,
    # RAYOVIN_LANGFUSE_*, RAYOVIN_SPOTIFY_*, ...) ARE allowed.
    "RAYOVIN_HOME", "RAYOVIN_PROFILE", "RAYOVIN_CONFIG", "RAYOVIN_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Rayovin runtime location "
            "(RAYOVIN_HOME, RAYOVIN_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.rayovin/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "RAYOVIN_TOOL_PROGRESS", "RAYOVIN_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `rayovin plugins enable
    # observability/langfuse` or `rayovin tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "RAYOVIN_LANGFUSE_ENV",
    "RAYOVIN_LANGFUSE_RELEASE",
    "RAYOVIN_LANGFUSE_SAMPLE_RATE",
    "RAYOVIN_LANGFUSE_MAX_CHARS",
    "RAYOVIN_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from rayovin_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("RAYOVIN_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_rayovin_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Rayovin is running in package-manager-managed mode.

    Two signals: the RAYOVIN_MANAGED env var (set by the systemd service),
    or a .managed marker file in RAYOVIN_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade rayovin-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``rayovin_cli/`` — i.e. the git checkout for source
    installs, ``/opt/rayovin`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$RAYOVIN_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Rayovin was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$RAYOVIN_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. RAYOVIN_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.rayovin``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$RAYOVIN_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.rayovin:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$RAYOVIN_HOME``, a
    ...
    ``docker`` and ``rayovin update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``rayovin/rayovin-agent`` image bakes a ``docker``
        stamp into ``/opt/rayovin`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $RAYOVIN_HOME.
    try:
    ...
    #    host install whose shared $RAYOVIN_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``rayovin update``.
    try:
        method = (
            (get_rayovin_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``rayovin_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$RAYOVIN_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/rayovin`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Rayovin lives in a ``uv tool`` layout.

    ``uv tool install rayovin-agent`` places the install at
    ``.../uv/tools/rayovin-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``rayovin-agent`` happens to be uv-tool-installed on the machine while
    the *active* Rayovin is a regular pip/venv install, causing
    ``rayovin update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/rayovin-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade rayovin-agent"
    if method == "docker":
        return "docker pull rayovin/rayovin-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade rayovin-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade rayovin-agent"
        return "pip install --upgrade rayovin-agent"
    return "rayovin update"
...
def format_unsupported_install_warning(method: str) -> str:
    ...
    ``rayovin update`` / ``rayovin update --check`` so the wording — and the
...
# Long-form text for ``rayovin update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# rayovin_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Rayovin, it doesn't update
...
✗ ``rayovin update`` doesn't apply inside the Docker container.

Rayovin Agent runs as a published image (rayovin/rayovin-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull rayovin/rayovin-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate rayovin-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm rayovin/rayovin-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/rayovin/rayovin-agent/tags
  • Your config and session history live under ``$RAYOVIN_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``rayovin update`` inside Docker.
...
def format_managed_message(action: str = "modify this Rayovin installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("RAYOVIN_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Rayovin installation is managed by NixOS "
            f"(RAYOVIN_MANAGED={env_hint}).\n"
            "Edit services.rayovin-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Rayovin installation is managed by Homebrew "
            f"(RAYOVIN_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade rayovin-agent"
        )

    return (
        f"Cannot {action}: this Rayovin installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Rayovin."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from RAYOVIN_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, rayovin_bin
    or None if container mode is not active, we're already inside the
    container, or RAYOVIN_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("RAYOVIN_DEV") == "1":
        return None
    ...
    container_mode_file = get_rayovin_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "rayovin-agent")
    exec_user = info.get("exec_user", "rayovin")
    rayovin_bin = info.get("rayovin_bin", "/data/current-package/bin/rayovin")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "rayovin_bin": rayovin_bin,
    }
...
# Re-export from rayovin_constants — canonical definition lives there.
from rayovin_constants import get_rayovin_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_rayovin_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_rayovin_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_rayovin_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the RAYOVIN_UID / RAYOVIN_GID env vars set by Docker deployments.

    Docker containers running Rayovin commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level RAYOVIN_HOME once, but
    subdirectories created at runtime by ``ensure_rayovin_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("RAYOVIN_UID", "").strip()
    gid_str = os.environ.get("RAYOVIN_GID", "").strip()
    try:
...
def _chown_to_rayovin_uid(path) -> None:
    """Chown ``path`` to ``RAYOVIN_UID:RAYOVIN_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_rayovin_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_rayovin_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the rayovin group can
    share state with the gateway service.

    The mode can be overridden via the RAYOVIN_HOME_MODE environment variable
    (e.g. RAYOVIN_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse RAYOVIN_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``RAYOVIN_UID``/``RAYOVIN_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("RAYOVIN_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_rayovin_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Rayovin runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("RAYOVIN_CONTAINER") or os.environ.get("RAYOVIN_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set RAYOVIN_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into RAYOVIN_HOME, upgrading legacy empty templates.
...
def ensure_rayovin_home():
    """Ensure ~/.rayovin directory structure exists with secure permissions.
    ...
    # Named profiles must be created explicitly (e.g. ``rayovin profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old RAYOVIN_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_rayovin_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_rayovin_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"RAYOVIN_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Rayovin-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Rayovin
        ...
        # identity slot (SOUL.md). Empty by default. The RAYOVIN_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Rayovin adds a coding operating brief
    ...
    "terminal": {
        ...
        #             RAYOVIN_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force RAYOVIN_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Rayovin auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Rayovin sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Rayovin
        ...
        # bundled Rayovin image, which drops to the `rayovin` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Rayovin sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Rayovin should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``rayovin chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.rayovin/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: rayovin sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .rayovin.md, or .cursorrules before Rayovin applies
    ...
    # single read_file page exceeds these limits, Rayovin truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Rayovin' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); rayovin =
                                      # Rayovin' compression threshold triggers
    ...
    # See tools/kanban_tools.py and rayovin_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``rayovin dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``rayovin kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``rayovin kanban decompose`` and the kanban
        ...
        # ``rayovin profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `rayovin model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `rayovin` (and `rayovin chat`) launches by default:
        ...
        # REPL and `--tui` (or RAYOVIN_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `rayovin --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `rayovin -c` muscle memory.  Default off so existing
        # users aren't surprised.  RAYOVIN_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `rayovin --tui` drops a one-time hint
        ...
        # can set `path`/`paths` to RAYOVIN_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/rayovin`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `rayovin pets`. Disabled until
        ...
        "pet": {
            ...
            # get_rayovin_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``RAYOVIN_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``RAYOVIN_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``RAYOVIN_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``RAYOVIN_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``RAYOVIN_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/rayovin``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Rayovin appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.rayovin/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.rayovin/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.rayovin/cache/delegation/
    ...
    # response. If not, Rayovin feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Rayovin auto-pauses the goal and
    ...
    "moa": {
        ...
        # <rayovin_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.rayovin/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${RAYOVIN_SKILL_DIR} and ${RAYOVIN_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `rayovin curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `rayovin curator run --consolidate` overrides
        ...
        # normally restored on every `rayovin update`, so pruning them only
        ...
        # skipped), snapshot ~/.rayovin/skills/ into
        # ~/.rayovin/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `rayovin curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for rayovin-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Rayovin Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (RAYOVIN_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Rayovin' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.rayovin/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or RAYOVIN_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `rayovin doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``rayovin_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Rayovin to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $RAYOVIN_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via RAYOVIN_CRON_MAX_PARALLEL env var.
        ...
        # RAYOVIN_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `rayovin -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `rayovin` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `rayovin kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     rayovin-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Rayovin tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.rayovin/logs/.
    ...
    # update model picker lists without shipping a rayovin-agent release.
    ...
    "gateway": {
        ...
        # internal RAYOVIN_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.rayovin/sessions/sessions.json entirely.
        ...
        # the RAYOVIN_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.rayovin.gateway` that defenses 1-2 don't
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.rayovin/.env,
        ...
        # behavior: files must live under the Rayovin cache, under
        ...
        # user. Bridged to RAYOVIN_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Rayovin
        # cache (~/.rayovin/cache/{documents,images,audio,video,screenshots})
        ...
        # to RAYOVIN_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # RAYOVIN_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to RAYOVIN_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.rayovin/state.db.
    ...
    "sessions": {
        ...
        # default of ``rayovin sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.rayovin/sessions/session_{sid}.json`` on every turn
    ...
    # ``rayovin update`` behaviour.
    "updates": {
        # Run a full ``rayovin backup``-style zip of RAYOVIN_HOME before every
        # ``rayovin update``.  Backups land in ``<RAYOVIN_HOME>/backups/`` and
        # can be restored with ``rayovin import <path>``.  Off by default:
        # zipping a large RAYOVIN_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``rayovin update
        ...
        "backup_keep": 5,
        # What `rayovin update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<RAYOVIN_HOME>/lsp/bin/`` on first use.
    ...
    # `rayovin tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.rayovin/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.rayovin/.env (or your shell) and never in config.yaml.
            ...
            # ~/.rayovin/bin/ on first use.  When False you must install
            ...
            # `rayovin secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.rayovin/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Rayovin disables it for our users unless they
        # explicitly opt in here. When false (default), Rayovin sets
    ...
    # Rayovin Desktop (Electron app) launch options. These only affect
    # `rayovin desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the RAYOVIN_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Rayovin mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'rayovin model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Rayovin auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Rayovin, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @rayovin:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. RAYOVIN_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #rayovin)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: rayovin-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'rayovin-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Rayovin API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Rayovin API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Rayovin API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # RAYOVIN_TOOL_PROGRESS and RAYOVIN_TOOL_PROGRESS_MODE are deprecated —
    ...
    "RAYOVIN_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.rayovin.config`` entries, then checks
    ...
    try:
        ...
        # should never break `rayovin update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Rayovin' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Rayovin versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``rayovin_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'rayovin setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Rayovin won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'rayovin doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("RAYOVIN_HOME", "~/.rayovin")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "rayovin update / rayovin -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("RAYOVIN_TOOL_PROGRESS")
            old_mode = get_env_value("RAYOVIN_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from RAYOVIN_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all", "verbose"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from RAYOVIN_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("RAYOVIN_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from RAYOVIN_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$RAYOVIN_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_rayovin_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `rayovin plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `rayovin curator status` has no
    ...
    #   3. Creates `~/.rayovin/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_rayovin_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_rayovin_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `rayovin config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `rayovin config set`"
                    )
    ...
    # behavior set it to true explicitly via `rayovin config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from rayovin_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from rayovin_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: rayovin config set <key> <value>")
    ...
    # `rayovin update`, but only the version bump is persisted.
    ...
    # metadata.rayovin.config in their SKILL.md frontmatter.
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_rayovin_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``rayovin config set``
    ...
    backends) — while display paths (``rayovin status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _rayovin_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.rayovin/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.rayovin/config.yaml.
    ...
    (which change ``RAYOVIN_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/rayovin/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_rayovin_dotenv()
...
#   openai-codex (OAuth — rayovin auth) — OpenAI Codex
#   nous         (OAuth — rayovin auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.rayovin/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Rayovin writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.rayovin/.env.
    ...
    interactive menu render (`rayovin tools`, `rayovin setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `rayovin tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Rayovin env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.rayovin/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.rayovin/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.rayovin/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.rayovin/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Rayovin — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Rayovin vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.rayovin/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.rayovin/.env`` over ``os.environ``.

    Used for Rayovin-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Rayovin Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from rayovin_cli.auth import get_anthropic_key
    ...
    # Warn on stale RAYOVIN_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("RAYOVIN_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale RAYOVIN_MAX_ITERATIONS={_env_ghost} "
                f"(run 'rayovin doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  rayovin config edit     # Edit config file", Colors.DIM))
    print(color("  rayovin config set <key> <value>", Colors.DIM))
    print(color("  rayovin setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `rayovin config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `rayovin config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: rayovin config set <key> <value>")
            print()
            print("Examples:")
            print("  rayovin config set model anthropic/claude-sonnet-4")
            print("  rayovin config set terminal.backend docker")
            print("  rayovin config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'rayovin config migrate' to add them")
        ...
        print("  rayovin config           Show current configuration")
        print("  rayovin config edit      Open config in editor")
        print("  rayovin config set <key> <value>   Set a config value")
        print("  rayovin config check     Check for missing/outdated config")
        print("  rayovin config migrate   Update config with new options")
        print("  rayovin config path      Show config file path")
        print("  rayovin config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``rayovin config`` UI so users
