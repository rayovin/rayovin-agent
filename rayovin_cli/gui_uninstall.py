"""
Rayovin Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``rayovin desktop`` / ``rayovin gui``)
     Built inside the agent checkout under ``$RAYOVIN_HOME/rayovin-agent/``:
       ...
       - ``$RAYOVIN_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Rayovin.app`` or ``~/Applications/Rayovin.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Rayovin`` (NSIS per-user)
...
the app name ("Rayovin"), separate from ``$RAYOVIN_HOME``:
  - macOS:   ``~/Library/Application Support/Rayovin``
  - Windows: ``%APPDATA%\\Rayovin``
  - Linux:   ``$XDG_CONFIG_HOME/Rayovin`` (default ``~/.config/Rayovin``)
...
``rayovin uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(rayovin_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return rayovin_home / "rayovin-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Rayovin"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Rayovin"
...
def source_built_gui_artifacts(rayovin_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``rayovin desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``rayovin-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(rayovin_home)
    ...
    return [
        ...
        rayovin_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Rayovin" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Rayovin.app"),
            home / "Applications" / "Rayovin.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Rayovin).
            local_base / "Programs" / "Rayovin",
            # Older / alternate layout some builds used.
            local_base / "rayovin-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Rayovin")
    else:
        ...
        paths += [
            data_base / "applications" / "rayovin.desktop",
            data_base / "applications" / "Rayovin.desktop",
        ]
    return paths


def agent_is_installed(rayovin_home: Path) -> bool:
    """Return True when a usable Python agent install exists under RAYOVIN_HOME.
    ...
    if (agent_root / "rayovin_cli").is_dir():
        return True
...
def gui_is_installed(rayovin_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(rayovin_home):
        if p.exists():
            return True
...
def gui_install_summary(rayovin_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = rayovin_home if rayovin_home is not None else get_rayovin_home()
...
def uninstall_gui(rayovin_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``rayovin-agent/rayovin_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$RAYOVIN_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove rayovin' or "
            "'sudo dnf remove rayovin'). AppImage builds are a single file you "
