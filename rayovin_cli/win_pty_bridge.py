"""Windows ConPTY bridge for the `rayovin dashboard` chat tab.

Drop-in counterpart to ``rayovin_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``rayovin_cli.web_server`` consumes: ``spawn``, ``read``,
