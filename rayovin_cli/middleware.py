"""Rayovin middleware contract helpers.
...
OBSERVER_SCHEMA_VERSION = "rayovin.observer.v1"
MIDDLEWARE_SCHEMA_VERSION = "rayovin.middleware.v1"
...
def apply_llm_request_middleware(
    ...
    **context: Any,
    ...
    provider kwargs before Rayovin sends them.
    """
...
def _invoke_middleware(kind: str, **kwargs: Any) -> List[Any]:
    from rayovin_cli.plugins import invoke_middleware

    return invoke_middleware(kind, **middleware_payload(**kwargs))


def _has_middleware(kind: str) -> bool:
    from rayovin_cli.plugins import has_middleware
