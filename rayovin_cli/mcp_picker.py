"""MCP picker — interactive `rayovin mcp picker` (also the default `rayovin mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``rayovin mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `rayovin plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from rayovin_cli.cli_output import prompt_yes_no
from rayovin_cli.curses_ui import curses_single_select
from rayovin_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Rayovin session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from rayovin_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `rayovin mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: rayovin mcp install <name>    Picker: rayovin mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Rayovin is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Rayovin — run `rayovin update` "
...
def show_catalog() -> None:
    """`rayovin mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`rayovin mcp picker` (and default `rayovin mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`rayovin mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `rayovin mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
