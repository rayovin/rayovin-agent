...
Used by ``rayovin profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``rayovin_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Rayovin Agent kanban board.
...
Rules:
  ...
  - Never write "Rayovin Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_rayovin_home())
        else:
