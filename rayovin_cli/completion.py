"""Shell completion script generation for rayovin CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_rayovin_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Rayovin Agent bash completion
# Add to ~/.bashrc:
#   eval "$(rayovin completion bash)"

_rayovin_profiles() {{
    local profiles_dir="$HOME/.rayovin/profiles"
...
_rayovin_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_rayovin_profiles)" -- "$cur"))
        return
    fi
...
complete -F _rayovin_completion rayovin
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef rayovin
# Rayovin Agent zsh completion
# Add to ~/.zshrc:
...
_rayovin_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.rayovin/profiles" ]]; then
        profiles+=($HOME/.rayovin/profiles/*(N/:t))
    fi
...
_rayovin() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_rayovin_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'rayovin command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Rayovin Agent fish completion",
        "# Add to your config:",
        "#   rayovin completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __rayovin_profiles",
        "    echo default",
        "    if test -d $HOME/.rayovin/profiles",
        "        for d in $HOME/.rayovin/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c rayovin -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c rayovin -f -s p -l profile"
        " -d 'Profile name' -xa '(__rayovin_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__rayovin_profiles)' -d 'Profile name'"
                )
