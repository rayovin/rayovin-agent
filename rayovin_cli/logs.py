"""``rayovin logs`` — view and filter Rayovin log files.
...
Usage examples::
    ...
    rayovin logs                    # last 50 lines of agent.log
    rayovin logs -f                 # follow agent.log in real time
    rayovin logs errors             # last 50 lines of errors.log
    rayovin logs gateway -n 100    # last 100 lines of gateway.log
    rayovin logs gui -f            # follow gui.log (dashboard/pty/ws)
    rayovin logs desktop -f        # follow desktop.log (Electron app boot/backend)
    rayovin logs --level WARNING    # only WARNING+ lines
    rayovin logs --session abc123   # filter by session ID substring
    rayovin logs --component tools  # only tool-related lines
    rayovin logs --since 1h         # lines from the last hour
    rayovin logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_rayovin_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Rayovin runs — try 'rayovin chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_rayovin_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_rayovin_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_rayovin_home()}/logs/")
        return

    print(f"Log files in {display_rayovin_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'rayovin chat' to generate logs)")
