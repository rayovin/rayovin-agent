"""`rayovin checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.rayovin/checkpoints/``.  Actions:

    rayovin checkpoints               # same as `status`
    rayovin checkpoints status        # total size, project count, breakdown
    rayovin checkpoints list          # per-project checkpoint counts + workdir
    rayovin checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    rayovin checkpoints clear [-f]    # nuke the entire base (asks first)
    rayovin checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    rayovin checkpoints
    rayovin checkpoints prune --retention-days 3 --max-size-mb 200
    rayovin checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: rayovin checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``rayovin checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `rayovin checkpoints` → status
