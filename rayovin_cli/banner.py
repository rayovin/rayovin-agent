...
Pure display functions with no RayovinCLI state dependency.
"""
import json
import logging
import os
...
from rayovin_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built rayovin — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NousResearch/rayovin-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/rayovin/rayovin-agent"
...
def _fetch_pypi_latest(package: str = "rayovin-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Rayovin update is available.

    Two paths: if ``RAYOVIN_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = rayovin_home / ".update_check"
    embedded_rev = os.environ.get("RAYOVIN_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # RAYOVIN_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `rayovin update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/rayovin/update/check`
    ...
    try:
        from rayovin_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $RAYOVIN_HOME/rayovin-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = rayovin_home / "rayovin-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Rayovin git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$RAYOVIN_HOME/rayovin-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``rayovin_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from rayovin_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/rayovin-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Rayovin checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/rayovin-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Rayovin Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else RAYOVIN_CADUCEUS
    except Exception:
        _bskin = None
        _hero = RAYOVIN_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]نبض آینده جنوب (Nabz-e-Ayandeh-e-Jonoob)[/]")
    else:
        ...
        ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
        left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]نبض آینده جنوب (Nabz-e-Ayandeh-e-Jonoob)[/]")

    if os.getenv("RAYOVIN_YOLO_MODE"):
    ...
    try:
        from rayovin_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from rayovin_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built rayovin; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else RAYOVIN_AGENT_LOGO
        console.print(_logo)
        console.print()
