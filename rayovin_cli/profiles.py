"""
Profile management for multiple isolated Rayovin instances.

Each profile is a fully independent RAYOVIN_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.rayovin/profiles/<name>/`` by default.

The "default" profile is ``~/.rayovin`` itself — backward compatible,
zero migration needed.

Usage::

    rayovin profile create coder          # fresh profile + bundled skills
    rayovin profile create coder --clone  # also copy config, .env, SOUL.md, skills
    rayovin profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    rayovin -p coder chat                 # or via flag
    rayovin profile use coder             # set as sticky default
    rayovin profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See rayovin_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.rayovin``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   rayovin-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `rayovin backup` archives
...
# Marker file written by `rayovin profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `rayovin
...
# `rayovin skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Rayovin infrastructure directories that only the default profile
         (``~/.rayovin``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_rayovin_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.rayovin) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "rayovin-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when RAYOVIN_HOME equals the
...
# into the export. The set below identifies the *known Rayovin profile
# artifacts* at the root of RAYOVIN_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``rayovin_constants``.
...
_RESERVED_NAMES = frozenset({
    "rayovin", "default", "test", "tmp", "root", "sudo",
})

# Rayovin subcommands that cannot be used as profile names/aliases
_RAYOVIN_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the rayovin root, NOT to the current RAYOVIN_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where RAYOVIN_HOME points outside
    ``~/.rayovin``, profiles live under ``RAYOVIN_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_rayovin_home() / "profiles"


def _get_default_rayovin_home() -> Path:
    """Return the default (pre-profile) RAYOVIN_HOME path.

    In standard deployments this is ``~/.rayovin``.
    In Docker/custom deployments where RAYOVIN_HOME is outside ``~/.rayovin``
    (e.g. ``/opt/data``), returns RAYOVIN_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_rayovin_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Rayovin installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its RAYOVIN_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_rayovin_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, rayovin subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _RAYOVIN_SUBCOMMANDS:
        return f"'{canon}' conflicts with a rayovin subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            rayovin_exe = shutil.which("rayovin") or "rayovin"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(rayovin_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Rayovin. If we
    ...
    try:
        ...
        from rayovin_cli.config import check_config_version, migrate_config
        ...
        # not be migrated. The next `rayovin doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``rayovin -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``rayovin profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``rayovin -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``rayovin profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``RAYOVIN_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Rayovin config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_rayovin_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, rayovin_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own RAYOVIN_HOME.
    ...
    The returned ``rayovin_home`` is the path to pass to
    ``set_rayovin_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_rayovin_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``rayovin update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.rayovin)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.rayovin/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `rayovin -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Rayovin profile.\n"
    ...
    if not soul_path.exists():
        try:
            from rayovin_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `rayovin update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`rayovin profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Rayovin (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `rayovin profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `rayovin -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches RAYOVIN_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``rayovin profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "RAYOVIN_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_rayovin_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Rayovin *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Rayovin backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``RAYOVIN_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `rayovin -p <canon> profile
    ...
    rayovin_markers = ("rayovin_cli.main", "rayovin-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Rayovin process: either an entrypoint marker in argv, or
            # a resolved executable named `rayovin`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_rayovin = (
                any(marker in joined for marker in rayovin_markers)
                or exe_name == "rayovin"
                or exe_name.startswith("rayovin")
            )
            if not is_rayovin:
                continue
            ...
            # ...or by RAYOVIN_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("RAYOVIN_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.rayovin).\n"
            "To remove everything, use: rayovin uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_rayovin_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``RAYOVIN_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from rayovin_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set RAYOVIN_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("RAYOVIN_HOME")
    try:
        os.environ["RAYOVIN_HOME"] = str(profile_dir)
        from rayovin_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["RAYOVIN_HOME"] = old_home
        elif "RAYOVIN_HOME" in os.environ:
            del os.environ["RAYOVIN_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.rayovin/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: rayovin profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from RAYOVIN_HOME.

    Returns ``"default"`` if RAYOVIN_HOME is not set or points to ``~/.rayovin``.
    Returns the profile name if RAYOVIN_HOME points into ``~/.rayovin/profiles/<name>``.
    Returns ``"custom"`` if RAYOVIN_HOME is set to an unrecognized path.
    """
    from rayovin_constants import get_rayovin_home
    rayovin_home = get_rayovin_home()
    resolved = rayovin_home.resolve()

    default_resolved = _get_default_rayovin_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      RAYOVIN_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Rayovin file the
      user may have lying alongside RAYOVIN_HOME (#58394).
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.rayovin itself — its parent is ~/ and its
        # directory name is ".rayovin", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: rayovin profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.rayovin itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.rayovin). "
            "Specify a different name: rayovin profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="rayovin_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"rayovin_{old_name}"
    legacy_old_host = f"rayovin.{old_name}"
    new_host = f"rayovin_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_rayovin_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("rayovin_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a RAYOVIN_HOME path string.

    Called early in the CLI entry point, before any rayovin modules
    are imported, to set the RAYOVIN_HOME environment variable.
    """
