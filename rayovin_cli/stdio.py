...
on Japanese locales, etc.).  Rayovin's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``rayovin_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Rayovin is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``RAYOVIN_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("RAYOVIN_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Rayovin-managed Git install directories so
    ...
    # already-running shells don't see the change — which means rayovin
    ...
    # install without Rayovin-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Rayovin's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Rayovin's
    ``rayovin config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Rayovin (User env var in Windows Settings, or
    export in a PowerShell profile) and Rayovin picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Rayovin-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\rayovin\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``rayovin`` from right after install) retain their old PATH.

    Any subprocess Rayovin spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Rayovin-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "rayovin", "git", "cmd"),
        os.path.join(local_appdata, "rayovin", "git", "bin"),
        os.path.join(local_appdata, "rayovin", "git", "usr", "bin"),
        # Rayovin venv Scripts directory — host of the rayovin.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes rayovin, but harmless to include.
        os.path.join(local_appdata, "rayovin", "rayovin-agent", "venv", "Scripts"),
