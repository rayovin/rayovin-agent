...
manual BotFather token copy-paste. Rayovin talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.rayovin.github.io/rayovin-agent"
...
DEFAULT_MANAGER_BOT = "RayovinSetupBot"

DEFAULT_BOT_NAME = "Rayovin Agent"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``rayovin_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``rayovin_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"rayovin_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Rayovin Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Rayovin Telegram onboarding service.")
