...
Reads the user's Nous OAuth state from ``~/.rayovin/auth.json`` through the
...
# Endpoints api.github.com/rayovin/rayovin-agent (future) actually serves. Anything else
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `rayovin auth add nous` first."
                )
    ...
    # to rayovin_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
