...
# xAI's public API is OpenAI-compatible for the endpoints Rayovin commonly
# uses. The Responses endpoint is included because Rayovin' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Rayovin-managed OAuth credentials."""

    auth_hint = "rayovin auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`rayovin auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`rayovin auth reset xai-oauth` or re-authenticate with "
                    "`rayovin auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `rayovin auth add xai-oauth --type oauth`."
            )
