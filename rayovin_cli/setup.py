"""
Interactive setup wizard for Rayovin Agent.
...
from rayovin_constants import get_optional_skills_dir
...
# display_rayovin_home imported lazily at call sites (stale-module safety during rayovin update)
...
from rayovin_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from rayovin_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Rayovin Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Rayovin using environment variables or config commands:")
    print_info("  rayovin config set model.provider custom")
    print_info("  rayovin config set model.base_url http://localhost:8080/v1")
    print_info("  rayovin config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'rayovin setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``RAYOVIN_NONINTERACTIVE=1`` (see ``rayovin_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("RAYOVIN_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``RAYOVIN_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from rayovin_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'rayovin setup')")


def _print_setup_summary(config: dict, rayovin_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'rayovin setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from rayovin_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `rayovin tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from rayovin_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'rayovin setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'rayovin setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'rayovin setup terminal'"))
    ...
    # Spotify (OAuth via rayovin auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'rayovin setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {rayovin_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('rayovin setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('rayovin setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('rayovin setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('rayovin setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('rayovin setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('rayovin config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('rayovin config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('rayovin config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('rayovin', Colors.GREEN)}              Start chatting")
    print(f"   {color('rayovin gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('rayovin doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `rayovin tools` and `rayovin setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``rayovin model``)
    ...
    provider added to ``rayovin model`` is automatically available here.
    ...
    # Delegate to the shared rayovin model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from rayovin_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: rayovin model")
    ...
    # on demand via `rayovin auth add`, `rayovin setup` vision, and
    # `rayovin setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from rayovin_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``rayovin model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from rayovin_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.rayovin/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Rayovin auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via rayovin setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'rayovin setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Rayovin runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `rayovin setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `rayovin setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into RAYOVIN_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("RAYOVIN_MAX_ITERATIONS")
    ...
    print_info("  Run `rayovin setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # RAYOVIN_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.rayovin/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from rayovin_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_rayovin_home(Path(get_rayovin_home()))
    except Exception:
        pass
...
def _profile_name_from_rayovin_home(rayovin_home) -> str | None:
    """Return the active profile name when RAYOVIN_HOME is a profile dir."""
    if rayovin_home.parent.name == "profiles":
        return rayovin_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Rayovin delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Rayovin to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from rayovin_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from rayovin_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Rayovin from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'rayovin setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     rayovin config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_rayovin_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: rayovin gateway install --system")
                print_info("  Or run in foreground:  rayovin gateway")
        else:
            from rayovin_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   rayovin gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   rayovin gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `rayovin setup tools` and `rayovin tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``rayovin_cli.auth`` — lists every supported
    ...
    """
    try:
        from rayovin_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from rayovin_cli.config
    ...
    if section_key == "model":
        ...
        from rayovin_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_rayovin.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_rayovin migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_rayovin", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Rayovin.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Rayovin to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Rayovin at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Rayovin at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Rayovin at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Rayovin at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Rayovin equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Rayovin config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Rayovin.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Rayovin's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(rayovin_home: Path) -> bool:
    ...
    print_info("Rayovin can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: rayovin claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=rayovin_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: rayovin claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Rayovin configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Rayovin config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Rayovin (use rayovin claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``rayovin setup --portal`` and ``rayovin portal``. This is the
    ...
    Rayovin session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``rayovin setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``rayovin model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``rayovin portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Rayovin Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `rayovin portal` == quick setup's Nous step.
    try:
        ...
        print_info("  You can retry later with `rayovin portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `rayovin portal`: %s", exc)
        print()
    ...
    print_info("  Run `rayovin portal info` to inspect routing.")
    print_info("  Run `rayovin` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      rayovin setup           — full or quick (auto-detected)
      rayovin setup model     — just model/provider
      rayovin setup tts       — just text-to-speech
      rayovin setup terminal  — just terminal backend
      rayovin setup gateway   — just messaging platforms
      rayovin setup tools     — just tool configuration
      rayovin setup agent     — just agent settings
    """
    from rayovin_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Rayovin Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Rayovin Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Rayovin Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, rayovin_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Rayovin configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'rayovin setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(rayovin_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Rayovin?",
            [
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, rayovin_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, rayovin_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {rayovin_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'rayovin config edit'")
    ...
    # Tune later with `rayovin setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, rayovin_home)


def _run_first_time_quick_setup(config: dict, rayovin_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``rayovin setup <section>``
    or switch providers with ``rayovin model``.
    """
    from rayovin_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'rayovin setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    rayovin setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  rayovin setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``rayovin tools`` (which
    ...
    try:
        ...
        from rayovin_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``rayovin setup agent`` /
    ``rayovin config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, rayovin_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `rayovin update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        rayovin tools")
        print_info("  Seed skills:         rayovin skills opt-in --sync")
        print_info("  Add MCP servers:     rayovin mcp add")
        print_info("  Enable plugins:      rayovin plugins")
        print_info("  Tune agent settings: rayovin setup agent")
        print()
        _print_setup_summary(config, rayovin_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, rayovin_home)


def _blank_slate_walkthrough(config: dict, rayovin_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`rayovin update` runs from re-injecting them. Opt back in any")
            print_info("time with `rayovin skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `rayovin tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `rayovin plugins list` / `rayovin plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `rayovin plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `rayovin mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `rayovin mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   rayovin tools")
    print_info("  Seed skills:         rayovin skills opt-in --sync")
    print_info("  Add MCP servers:     rayovin mcp add")
    print_info("  Tune agent settings: rayovin setup agent")
    print()
...
def _run_quick_setup(config: dict, rayovin_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'rayovin setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Rayovin to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'rayovin setup gateway'.")
