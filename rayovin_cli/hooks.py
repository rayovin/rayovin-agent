"""rayovin hooks — inspect and manage shell-script hooks.

Usage::

    rayovin hooks list
    rayovin hooks test <event> [--for-tool X] [--payload-file F]
    rayovin hooks revoke <command>
    rayovin hooks doctor

Consent records live under ``~/.rayovin/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.rayovin/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``rayovin hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: rayovin hooks {list|test|revoke|doctor}")
        print("Run 'rayovin hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.rayovin/config.yaml.")
        print("See `rayovin hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `rayovin hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `rayovin hooks test` and `rayovin hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Rayovin wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `rayovin hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `rayovin hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `rayovin hooks doctor`.")
