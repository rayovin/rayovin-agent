...
from rayovin_cli.dashboard_auth import list_session_providers
from rayovin_cli.dashboard_auth.audit import AuditEvent, audit_log
from rayovin_cli.dashboard_auth.base import (
...
from rayovin_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /rayovin``, the
    ``login_url`` is prefixed (``/rayovin/login?next=...``) so the
    ...
    """
    from rayovin_cli.dashboard_auth.prefix import prefix_from_request
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from rayovin_cli.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["rayovin.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``rayovin_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if session is None:
        ...
        from rayovin_cli.dashboard_auth.cookies import clear_session_cookies
        from rayovin_cli.dashboard_auth.prefix import prefix_from_request
