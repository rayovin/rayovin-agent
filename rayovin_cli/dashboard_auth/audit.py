...
Profile-aware location: ``$RAYOVIN_HOME/logs/dashboard-auth.log``.
...
from ``rayovin_constants`` or other rayovin_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$RAYOVIN_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``rayovin_constants.get_rayovin_home`` semantics: env var wins,
    else ``~/.rayovin``. A local copy avoids an import cycle with the
    middleware which lives below ``rayovin_cli``.
    """
    home = os.environ.get("RAYOVIN_HOME") or str(Path.home() / ".rayovin")
