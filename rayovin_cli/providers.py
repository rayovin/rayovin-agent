"""
Single source of truth for provider identity in Rayovin Agent.
...
2. **Rayovin overlays** — transport type, auth patterns, aggregator flags,
...
# -- Rayovin overlay ----------------------------------------------------------
# Rayovin-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class RayovinOverlay:
    """Rayovin-specific provider metadata layered on top of models.dev."""
...
RAYOVIN_OVERLAYS: Dict[str, RayovinOverlay] = {
    "moa": RayovinOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": RayovinOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-api": RayovinOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": RayovinOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": RayovinOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="RAYOVIN_QWEN_BASE_URL",
    ),
    "lmstudio": RayovinOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": RayovinOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": RayovinOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": RayovinOverlay(
    ...
    "kimi-for-coding": RayovinOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": RayovinOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": RayovinOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": RayovinOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": RayovinOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": RayovinOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": RayovinOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": RayovinOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": RayovinOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": RayovinOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "huggingface": RayovinOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": RayovinOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": RayovinOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": RayovinOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": RayovinOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
    "arcee": RayovinOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": RayovinOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": RayovinOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "rayovin", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Rayovin overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Rayovin overlay
    ...
    overlay = RAYOVIN_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + rayovin extra
    ...
    if overlay is not None:
        # Rayovin-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in RAYOVIN_OVERLAYS
    if provider == "bedrock":
