import type { RunExternalProcess } from '@rayovin/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`rayovin ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching rayovin: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`rayovin ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
