...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.RAYOVIN_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Rayovin on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via RAYOVIN_TUI_TRUECOLOR=1.
