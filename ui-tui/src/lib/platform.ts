 ...
 * rayovin-ink sets ``key.meta`` for plain Alt/Option on every platform
 ...
 * (``rayovin_cli/voice.py::normalize_voice_record_key_for_prompt_toolkit``)
 ...
 * modifier — but rayovin-ink reports Alt as ``key.meta`` on many
...
export const parseVoiceRecordKey = (raw: unknown): ParsedVoiceRecordKey => {
  ...
  // On macOS rayovin-ink reports Alt as ``key.meta``, which ``isActionMod``
...
export const isVoiceToggleKey = (
  ...
  switch (configured.mod) {
    case 'alt':
      ...
      // alt binding.
      //
      // Bare Escape on rayovin-ink can arrive as ``key.meta=true`` on some
    ...
    case 'ctrl':
      ...
      // ``key.meta`` is rayovin-ink's Alt signal and accepting it would
