...
const logDir = join(process.env.RAYOVIN_HOME?.trim() || join(homedir(), '.rayovin'), 'logs')
...
// a real ~/.rayovin (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('rayovin-tui: parent lifecycle log unavailable\n')
    }
  }
}
