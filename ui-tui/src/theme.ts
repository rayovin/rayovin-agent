...
// Mirrors @rayovin/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @rayovin/ink is bundled separately from packages/.
...
// Terminal. Explicit RAYOVIN_TUI_THEME / COLORFGBG signals above still win,
...
// `RAYOVIN_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `RAYOVIN_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `RAYOVIN_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `RAYOVIN_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Rayovin palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.RAYOVIN_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.RAYOVIN_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.RAYOVIN_TUI_BACKGROUND ?? '')
