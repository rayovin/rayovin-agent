...
React + Ink terminal UI for Rayovin. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `RAYOVIN_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `rayovin setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.rayovin/.rayovin_history` or under `RAYOVIN_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `rayovin setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/rayovin-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external rayovin setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.rayovin/tui-parent.log
    ...
    types/
      rayovin-ink.d.ts            type declarations for @rayovin/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent RayovinCLI subprocess for slash commands
```
