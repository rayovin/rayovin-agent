...
upstream prefix cache warm.  See ``rayovin-agent-dev``'s
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``RAYOVIN_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("RAYOVIN_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Rayovin never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want RAYOVIN_HOME persona while keeping
    ...
    # Pointer to the rayovin-agent skill + docs for user questions about Rayovin itself.
    stable_parts.append(RAYOVIN_AGENT_HELP_GUIDANCE)
    ...
    # RAYOVIN_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Rayovin, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Rayovin profile the agent is running
    # under so it doesn't conflate ~/.rayovin/skills/ (default profile) with
    # ~/.rayovin/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Rayovin profile: default. Other profiles (if any) live "
            "under ~/.rayovin/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Rayovin profile: {active_profile}. This session reads "
            f"and writes ~/.rayovin/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.rayovin/skills/, ~/.rayovin/plugins/, "
            f"~/.rayovin/cron/, ~/.rayovin/memories/ — those belong to a "
    ...
    if platform_key == "telegram" and _default_hint:
        try:
            from rayovin_cli.config import load_config_readonly
    ...
    if not agent.skip_context_files:
        ...
        # (developing Rayovin). Every other surface (desktop chat panel,
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Rayovin never rebuilds or reinjects parts of it
