...
from rayovin_constants import get_rayovin_home, get_skills_dir, is_wsl
...
_RAYOVIN_MD_NAMES = (".rayovin.md", "RAYOVIN.md")


def _find_rayovin_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.rayovin.md`` or ``RAYOVIN.md``.
    ...
    # could pick up a .rayovin.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _RAYOVIN_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Rayovin Agent, an intelligent AI assistant created by نبض آینده جنوب (Nabz-e-Ayandeh-e-Jonoob). "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.rayovin/kanban.db`. Your task id is in "
    "`$RAYOVIN_KANBAN_TASK`; your workspace is `$RAYOVIN_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $RAYOVIN_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $RAYOVIN_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${RAYOVIN_KANBAN_BRANCH:-wt/$RAYOVIN_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$RAYOVIN_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`rayovin profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `rayovin kanban <verb>` for board operations. Use "
...
#
# The rayovin-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to rayovin-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `rayovin computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Rayovin "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Rayovin terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Rayovin desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the Rayovin WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Rayovin itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Rayovin runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Rayovin itself is running. The host OS, home, and cwd "
                f"of the Rayovin process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Rayovin "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Rayovin
    ...
    extra = (os.getenv("RAYOVIN_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_rayovin_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("RAYOVIN_PLATFORM") or os.environ.get("RAYOVIN_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.rayovin/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Rayovin Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `rayovin-agent` skill "
            "first. It has the actual commands (e.g. `rayovin config set …`, `rayovin tools`, "
            "`rayovin setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: rayovin setup, rayovin setup tools, rayovin setup terminal, rayovin status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from RAYOVIN_HOME and return its content, or None.
    ...
    """
    try:
        from rayovin_cli.config import ensure_rayovin_home
        ensure_rayovin_home()
    except Exception as e:
        logger.debug("Could not ensure RAYOVIN_HOME before loading SOUL.md: %s", e)

    soul_path = get_rayovin_home() / "SOUL.md"
...
def _load_rayovin_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.rayovin.md / RAYOVIN.md — walk to git root."""
    rayovin_md_path = _find_rayovin_md(cwd_path)
    if not rayovin_md_path:
        return ""
    try:
        content = rayovin_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = rayovin_md_path.name
        try:
            rel = str(rayovin_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".rayovin.md", context_length=context_length,
            read_path=str(rayovin_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", rayovin_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .rayovin.md / RAYOVIN.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from RAYOVIN_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Rayovin install/source
    ...
    # explicitly configured cwd is honored verbatim — the Rayovin tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Rayovin in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Rayovin install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_rayovin_md(cwd_path, context_length)
    ...
    # SOUL.md from RAYOVIN_HOME only — skip when already loaded as identity
    if not skip_soul:
