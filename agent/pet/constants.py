...
row taxonomy is inferred from the atlas shape so Rayovin can render both legacy
...
# rows (1536x1872), while older Rayovin/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Rayovin' activity state names. They are not always identical to the
...
# Legacy Rayovin/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Rayovin activity names -> accepted row-name aliases in descending
