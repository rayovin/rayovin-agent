...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the RAYOVIN_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Rayovin runtime / deployment
    "RAYOVIN_HOME", "RAYOVIN_PROFILE", "RAYOVIN_GATEWAY_LOCK_DIR",
    "RAYOVIN_MAX_ITERATIONS", "RAYOVIN_MAX_TOKENS", "RAYOVIN_API_TIMEOUT",
    "RAYOVIN_REDACT_SECRETS", "RAYOVIN_NOUS_TIMEOUT_SECONDS",
    "_RAYOVIN_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "RAYOVIN_KANBAN_DB", "RAYOVIN_KANBAN_WORKSPACES_ROOT", "RAYOVIN_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "RAYOVIN_KANBAN_",
    "RAYOVIN_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(rayovin_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(rayovin_home) / ".env")
