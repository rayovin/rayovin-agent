...
Plugins built on Rayovin Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Rayovin
...
:class:`~rayovin_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Rayovin' main config
...
handles every provider, fallback chain, and per-task override Rayovin
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`rayovin_cli.plugins.PluginContext`
