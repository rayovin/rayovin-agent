...
``auth.json``.  They intentionally have no dependency on ``rayovin_cli.auth`` so
...
# Sources Rayovin owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Rayovin-owned OAuth/device-code state)
