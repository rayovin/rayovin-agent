...
Storage
-------
Bundles live in ``~/.rayovin/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``RAYOVIN_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``rayovin bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under RAYOVIN_HOME.

    Honors ``RAYOVIN_BUNDLES_DIR`` for tests; falls back to
    ``<RAYOVIN_HOME>/skill-bundles``.
    """
    override = os.environ.get("RAYOVIN_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_rayovin_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `rayovin bundles` CLI subcommand.
# ---------------------------------------------------------------------------
