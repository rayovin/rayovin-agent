...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `rayovin backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - rayovin_home (str): The active RAYOVIN_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.rayovin``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "rayovin").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'rayovin memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], rayovin_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'rayovin memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``rayovin_home`` is the active RAYOVIN_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE RAYOVIN_HOME.

        ``rayovin backup`` only walks RAYOVIN_HOME, so any provider state kept
        ...
        of the archive, and ``rayovin import`` restores them to their original
