"""Projects codex app-server events into Rayovin' messages list.

The translator that lets Rayovin' memory/skill review keep working under the
...
Rayovin' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Rayovin
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Rayovin' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Rayovin stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Rayovin already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Rayovin' messages store text only.
        text_parts: list[str] = []
