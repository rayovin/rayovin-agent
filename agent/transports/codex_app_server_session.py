...
Owns one Codex thread per Rayovin session. Drives `turn/start`, consumes
...
# Rayovin' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_RAYOVIN_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Rayovin/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Rayovin session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _RAYOVIN_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("RAYOVIN_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="rayovin",
            client_title="Rayovin Agent",
            client_version=_get_rayovin_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Rayovin' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Rayovin' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own rayovin-tools callback we
            # auto-accept — the user already approved Rayovin' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Rayovin approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Rayovin returns 'once', 'session', 'always', or 'deny'.
...
def _get_rayovin_version() -> str:
    """Best-effort Rayovin version string for codex's userAgent line."""
    try:
