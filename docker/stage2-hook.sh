...
#
# Wired into the image as /etc/cont-init.d/01-rayovin-setup by the
...
RAYOVIN_HOME="${RAYOVIN_HOME:-/opt/data}"
INSTALL_DIR="/opt/rayovin"

# Drop to rayovin via s6-setuidgid, but skip it when already non-root.
as_rayovin() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid rayovin "$@"; }
...
# starts non-root. The baked install tree under /opt/rayovin is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $RAYOVIN_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass RAYOVIN_UID/RAYOVIN_GID — or the PUID/PGID aliases — which the
...
# `id -u rayovin` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# rayovin UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u rayovin)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-rayovin UID).
...
and the baked /opt/rayovin install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e RAYOVIN_UID=\$(id -u) -e RAYOVIN_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the rayovin user to that UID/GID at boot and chowns the data
...
# --- Bootstrap RAYOVIN_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid rayovin mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom RAYOVIN_HOME paths whose parents only
# root can create (e.g. `RAYOVIN_HOME=/home/rayovin/.rayovin` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for RAYOVIN_UID/RAYOVIN_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  RAYOVIN_UID/
# RAYOVIN_GID still win when both are set.  See #15290, salvages #25872.
RAYOVIN_UID="${RAYOVIN_UID:-${PUID:-}}"
RAYOVIN_GID="${RAYOVIN_GID:-${PGID:-}}"

if [ -n "${RAYOVIN_UID:-}" ] && validate_uid_gid "$RAYOVIN_UID" && [ "$RAYOVIN_UID" != "$(id -u rayovin)" ]; then
    echo "[stage2] Changing rayovin UID to $RAYOVIN_UID"
    usermod -u "$RAYOVIN_UID" rayovin
fi
if [ -n "${RAYOVIN_GID:-}" ] && validate_uid_gid "$RAYOVIN_GID" && [ "$RAYOVIN_GID" != "$(id -g rayovin)" ]; then
    echo "[stage2] Changing rayovin GID to $RAYOVIN_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$RAYOVIN_GID" rayovin 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised rayovin user (UID 10000)
...
# rayovin process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes rayovin. Idempotent across container
...
#   - rayovin is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G rayovin 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] rayovin already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" rayovin 2>/dev/null; then
        echo "[stage2] Added rayovin to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group rayovin failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When RAYOVIN_UID is remapped or the top-level $RAYOVIN_HOME isn't owned by
# the runtime rayovin UID, restore ownership to rayovin — but ONLY for the
# directories rayovin actually writes to. The full $RAYOVIN_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of rayovin-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_rayovin_uid=$(id -u rayovin)
...
chown_rayovin_tree() {
    ...
    chown -R rayovin:rayovin "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$RAYOVIN_HOME" 2>/dev/null)" != "$actual_rayovin_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $RAYOVIN_HOME (targeted) to rayovin ($actual_rayovin_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $RAYOVIN_HOME: chown the directory itself (not its contents)
    # so rayovin can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$RAYOVIN_HOME"; then
        :
    else
        chown rayovin:rayovin "$RAYOVIN_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $RAYOVIN_HOME failed (rootless container?) — continuing"
    fi
    # Rayovin-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by rayovin (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$RAYOVIN_HOME/$sub" ]; then
            chown_rayovin_tree "$RAYOVIN_HOME/$sub"
        fi
    done
fi
...
# rayovin user. Hosted/container instances keep mutable state under
# $RAYOVIN_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# RAYOVIN_DISABLE_LAZY_INSTALLS=1. Keeping /opt/rayovin root-owned and
...
# $RAYOVIN_HOME/lazy-packages dir on the data volume (Dockerfile sets
# RAYOVIN_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to rayovin
...
# Always reset ownership of $RAYOVIN_HOME/profiles to rayovin on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> rayovin …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as rayovin and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$RAYOVIN_HOME/profiles" ]; then
    chown_rayovin_tree "$RAYOVIN_HOME/profiles"
fi

# Always reset ownership of $RAYOVIN_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged rayovin runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$RAYOVIN_HOME/cron" ]; then
    chown_rayovin_tree "$RAYOVIN_HOME/cron"
fi
...
# rayovin pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged rayovin gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $RAYOVIN_HOME is
...
# is negligible.
if [ -d "$RAYOVIN_HOME/platforms/pairing" ]; then
    chown_rayovin_tree "$RAYOVIN_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$RAYOVIN_HOME/pairing" ]; then
    chown_rayovin_tree "$RAYOVIN_HOME/pairing"
fi

# Reset ownership of rayovin-owned top-level state files on every boot.
# The targeted data-volume chown above only covers rayovin-owned
# *subdirectories*; loose state files living directly under $RAYOVIN_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> rayovin …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged rayovin runtime then hits
...
# sweep so host-owned files in a bind-mounted $RAYOVIN_HOME are never
...
# entries of rayovin_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$RAYOVIN_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$RAYOVIN_HOME/$f"; then
            :
        else
            chown rayovin:rayovin "$RAYOVIN_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the rayovin runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$RAYOVIN_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$RAYOVIN_HOME/config.yaml"; then
        :
    else
        chown rayovin:rayovin "$RAYOVIN_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$RAYOVIN_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as rayovin user ---
# Run as rayovin via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $RAYOVIN_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_rayovin mkdir -p \
    "$RAYOVIN_HOME/backups" \
    "$RAYOVIN_HOME/cron" \
    "$RAYOVIN_HOME/sessions" \
    "$RAYOVIN_HOME/logs" \
    "$RAYOVIN_HOME/logs/gateways" \
    "$RAYOVIN_HOME/hooks" \
    "$RAYOVIN_HOME/memories" \
    "$RAYOVIN_HOME/skills" \
    "$RAYOVIN_HOME/skins" \
    "$RAYOVIN_HOME/plans" \
    "$RAYOVIN_HOME/workspace" \
    "$RAYOVIN_HOME/home" \
    "$RAYOVIN_HOME/pairing" \
    "$RAYOVIN_HOME/platforms/pairing" \
    "$RAYOVIN_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/rayovin/.install_method (see Dockerfile), NOT written here into
# $RAYOVIN_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $RAYOVIN_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.rayovin:/opt/data) and sometimes shared with a
...
# 'rayovin update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $RAYOVIN_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/rayovin stamp, so deleting the data-dir copy is safe).
if [ -f "$RAYOVIN_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$RAYOVIN_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$RAYOVIN_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$RAYOVIN_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$RAYOVIN_HOME/$dest"; then
            :
        else
            as_rayovin cp "$INSTALL_DIR/$src" "$RAYOVIN_HOME/$dest"
        fi
    fi
}
...
if [ -f "$RAYOVIN_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$RAYOVIN_HOME/.env"; then
        :
    else
        chown rayovin:rayovin "$RAYOVIN_HOME/.env" 2>/dev/null || true
        chmod 600 "$RAYOVIN_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $RAYOVIN_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `rayovin update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set RAYOVIN_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$RAYOVIN_HOME/config.yaml" ]; then
    s6-setuidgid rayovin "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$RAYOVIN_HOME/auth.json" ] && [ -n "${RAYOVIN_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$RAYOVIN_HOME/auth.json"; then
        :
    else
        printf '%s' "$RAYOVIN_AUTH_JSON_BOOTSTRAP" > "$RAYOVIN_HOME/auth.json"
        chown rayovin:rayovin "$RAYOVIN_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$RAYOVIN_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via RAYOVIN_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$RAYOVIN_HOME/auth.json" ] && [ -n "${RAYOVIN_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$RAYOVIN_HOME/auth.json"; then
        :
    else
        s6-setuidgid rayovin "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$RAYOVIN_HOME/auth.json" \
...
# can set RAYOVIN_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# RAYOVIN_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$RAYOVIN_HOME/gateway_state.json" ] && \
        [ "${RAYOVIN_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$RAYOVIN_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$RAYOVIN_HOME/gateway_state.json"
        chown rayovin:rayovin "$RAYOVIN_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$RAYOVIN_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_rayovin "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/rayovin/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Rayovin spawns for the
...
# main-wrapper.sh propagates it into the supervised ``rayovin`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-rayovin, dashboard, etc.).
