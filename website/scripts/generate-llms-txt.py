#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Rayovin docs site.
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Rayovin as a Python Library", None),
        ("guides/use-mcp-with-rayovin", "Use MCP with Rayovin", None),
        ("guides/use-voice-mode-with-rayovin", "Use Voice Mode with Rayovin", None),
        ("guides/use-soul-with-rayovin", "Use SOUL.md with Rayovin", None),
        ("guides/build-a-rayovin-plugin", "Build a Rayovin Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Rayovin"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Rayovin Agent")
    lines.append("")
    lines.append(
        "> The intelligent Persian AI agent by نبض آینده جنوب built by نبض آینده جنوب (Nabz-e-Ayandeh-e-Jonoob). A terminal-native "
    ...
    lines.append(
        ...
        "rayovin-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/NousResearch/rayovin-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Rayovin Agent — Full Documentation\n",
        (
            "This file is the entire Rayovin Agent documentation concatenated for LLM "
