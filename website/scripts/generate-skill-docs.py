...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/rayovin-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    rayovin_meta = (fm.get("metadata") or {}).get("rayovin") or {}
    tags = rayovin_meta.get("tags") or []
    related = rayovin_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `rayovin skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Rayovin Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Rayovin ships with a large built-in skill library copied into `~/.rayovin/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Rayovin also syncs bundled skills on `rayovin update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.rayovin/skills/` tree, it is still shipped with Rayovin; restore it with `rayovin skills reset <name> --restore`.",
        "",
