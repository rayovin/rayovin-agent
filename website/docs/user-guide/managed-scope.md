...
`~/.rayovin/config.yaml`, `~/.rayovin/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/rayovin`:

```text
/etc/rayovin/
├── config.yaml     # managed config layer (wins over ~/.rayovin/config.yaml)
└── .env            # managed env layer (wins over ~/.rayovin/.env + shell)
```
...
The location can be relocated with the `RAYOVIN_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `RAYOVIN_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Rayovin.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export RAYOVIN_MANAGED_DIR=/opt/org/rayovin-policy
```

:::warning
A user who can set `RAYOVIN_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `rayovin doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/rayovin/config.yaml` (managed) | `/etc/rayovin/.env` (managed) |
| 2 | `~/.rayovin/config.yaml` (user) | `~/.rayovin/.env` (user) |
...
```bash
rayovin config        # shows a header naming the managed source + the pinned keys
rayovin doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Rayovin refuses and names the source:

```bash
$ rayovin config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/rayovin/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `rayovin config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/rayovin

# Pin some config values for every user on this machine
sudo tee /etc/rayovin/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/rayovin/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/rayovin
sudo chmod 0644 /etc/rayovin/config.yaml /etc/rayovin/.env
```

Changes take effect on the next Rayovin start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`rayovin doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Rayovin as `root`), managed scope is advisory.
