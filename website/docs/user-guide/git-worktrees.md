...
title: "Git Worktrees"
description: "Run multiple Rayovin agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Rayovin Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Rayovin so each session has a clean, isolated working directory.

## Why Use Worktrees with Rayovin?

Rayovin treats the **current working directory** as the project root:

- CLI: the directory where you run `rayovin` or `rayovin chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.rayovin/config.yaml`
...
git worktree add ../repo-feature feature/rayovin-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/rayovin-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Rayovin there:

```bash
cd ../repo-feature

# Start Rayovin in the worktree
rayovin
```
...
git worktree add ../repo-experiment-a feature/rayovin-a
git worktree add ../repo-experiment-b feature/rayovin-b
```
...
Each Rayovin process:

- Works on its own branch (`feature/rayovin-a` vs `feature/rayovin-b`).
...
- Rayovin checkpoint data under `~/.rayovin/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Rayovin experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/rayovin-checkpoints-docs`, `feature/rayovin-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Rayovin from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `rayovin -w` (Automatic Worktree Mode)

Rayovin has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `rayovin/rayovin-<hash>`).
...
```bash
rayovin -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `rayovin -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Rayovin session its own clean checkout.
