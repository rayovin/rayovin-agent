...
- Give it a name like "rayovin-agent"
- Select scopes:
...
- Give it a title like "rayovin-agent-&lt;machine-name>"
...
elif _rayovin_env="${RAYOVIN_HOME:-$HOME/.rayovin}/.env"; [ -f "$_rayovin_env" ] && grep -q "^GITHUB_TOKEN=" "$_rayovin_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_rayovin_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
