...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _rayovin_env="${RAYOVIN_HOME:-$HOME/.rayovin}/.env"; [ -f "$_rayovin_env" ] && grep -q "^GITHUB_TOKEN=" "$_rayovin_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_rayovin_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
