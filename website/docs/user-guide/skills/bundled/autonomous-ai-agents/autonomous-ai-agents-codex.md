...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`rayovin-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-rayovin-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Rayovin terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Rayovin itself, `model.provider: openai-codex` uses Rayovin-managed Codex
OAuth from `~/.rayovin/auth.json` after `rayovin auth add openai-codex`. For the
...
## Rayovin Gateway Caveat

When invoking the Codex CLI from a Rayovin gateway/service context (for example,
