...
| | |
|---|---|
| Source | Optional — install with `rayovin skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Rayovin Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Rayovin terminal calls (desktop app flow only)
...
### Service Account (recommended for Rayovin)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${RAYOVIN_HOME:-~/.rayovin}/.env` (the skill will prompt for this on first load).
...
## Rayovin Execution Pattern (desktop app flow)

Rayovin terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/rayovin-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/rayovin-op.sock"
