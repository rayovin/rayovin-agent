...
| | |
|---|---|
| Source | Optional — install with `rayovin skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Rayovin or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Rayovin. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.rayovin/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.rayovin/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Rayovin integration, either:

- configure the server in `~/.rayovin/config.yaml` using the `native-mcp` skill, or
...
### Rayovin cannot see the deployed server

The server-building part may be correct while the Rayovin config is not. Load the `native-mcp` skill and configure the server in `~/.rayovin/config.yaml`, then restart Rayovin.
