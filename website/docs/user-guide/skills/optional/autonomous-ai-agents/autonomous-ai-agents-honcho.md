...
description: "Configure and use Honcho memory with Rayovin -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Rayovin -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `rayovin skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Rayovin

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Rayovin profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/rayovin#running-honcho-locally-with-rayovin

### Verify

```bash
rayovin honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Rayovin profile's AI peer
...
Honcho models conversations as interactions between **peers**. Rayovin creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Rayovin instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Rayovin run |
| `global` | Single session across all directories |

Manual override: `rayovin honcho map my-project-name`
...
Each Rayovin profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block rayovin.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `rayovin.coder` host block in `honcho.json`
...
```bash
rayovin honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Rayovin when Honcho memory is active.
...
Config file: `$RAYOVIN_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `rayovin honcho setup`. Ensure `memory.provider: honcho` is in `~/.rayovin/config.yaml`.

### Memory not persisting across sessions
Check `rayovin honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `rayovin profile create <name> --clone`. For existing profiles: `rayovin honcho sync`.
...
| Command | Description |
|---------|-------------|
| `rayovin honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `rayovin honcho status` | Show resolved config, connection test, peer info for active profile |
| `rayovin honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `rayovin honcho disable` | Disable Honcho for the active profile |
| `rayovin honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `rayovin honcho peers` | Show peer identities across all profiles |
| `rayovin honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `rayovin honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `rayovin honcho sessions` | List known directory-to-session-name mappings |
| `rayovin honcho map <name>` | Map current working directory to a Honcho session name |
| `rayovin honcho identity` | Seed AI peer identity or show both peer representations |
| `rayovin honcho sync` | Create host blocks for all Rayovin profiles that don't have one yet |
| `rayovin honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Rayovin + Honcho |
| `rayovin memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `rayovin memory status` | Show active memory provider and config |
| `rayovin memory off` | Disable external memory provider |
