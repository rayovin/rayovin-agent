...
| | |
|---|---|
| Source | Optional — install with `rayovin skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${RAYOVIN_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${RAYOVIN_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `rayovin`)    |
