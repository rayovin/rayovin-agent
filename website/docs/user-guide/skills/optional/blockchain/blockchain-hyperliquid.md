...
| | |
|---|---|
| Source | Optional — install with `rayovin skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain/hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Rayovin Agent |
| License | MIT |
...
The script reads `${RAYOVIN_HOME:-~/.rayovin}/.env` for two optional defaults:
...
Helper script: `~/.rayovin/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${RAYOVIN_HOME:-~/.rayovin}/.env`.
