...
chat completions — use your Rayovin-managed provider subscription as their
...
| Use case | "Use Rayovin as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Rayovin stores
the refresh token in `~/.rayovin/auth.json` — the same place all Rayovin
...
Model:      Rayovin-4-70B    # or Rayovin-4.3-36B, Rayovin-4-405B
```
...
interface in `rayovin_cli/proxy/adapters/`.
...
If you see `not logged in`, run `rayovin portal`. If you see
...
supports; check `github.com/rayovin/rayovin-agent/models`.
...
[github.com/rayovin/rayovin-agent](https://github.com/rayovin/rayovin-agent).
...
`rayovin_cli/proxy/adapters/<provider>.py` and registering it in
