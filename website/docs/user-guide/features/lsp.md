...
Rayovin runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Rayovin
...
1. Rayovin captures a baseline of current diagnostics for the file.
...
…). Rayovin auto-detects the binary on PATH or in
`<RAYOVIN_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Rayovin at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<RAYOVIN_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`rayovin lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Rayovin installs both packages together when you
run `rayovin lsp install typescript` or auto-install fires on first
use.

## CLI

```
rayovin lsp status          # service state + per-server install status
rayovin lsp list            # registry, optionally --installed-only
rayovin lsp install <id>    # eagerly install one server
rayovin lsp install-all     # try every server with a known recipe
rayovin lsp restart         # tear down running clients
rayovin lsp which <id>      # print resolved binary path
```

`rayovin lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <RAYOVIN_HOME>/lsp/bin
...
When `install_strategy: auto`, Rayovin installs binaries into
`<RAYOVIN_HOME>/lsp/bin/`. NPM packages land in
`<RAYOVIN_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Rayovin-owned and is
...
Servers are kept alive for the life of the Rayovin process. There's
...
**`rayovin lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<RAYOVIN_HOME>/lsp/bin/`. Run
`rayovin lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `rayovin lsp status`**
...
When `rayovin lsp status` shows a `Backend warnings` section, install
...
`~/.rayovin/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.rayovin/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `rayovin lsp restart` to clear the set;
