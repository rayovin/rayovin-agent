...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Rayovin's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Rayovin instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
rayovin memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `rayovin` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Rayovin syncs them back at session init.
...
The `rayovin honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `rayovin memory setup honcho` (or run `rayovin memory setup` and pick it from the list); the `rayovin honcho` subcommand then appears on the next invocation.

```bash
rayovin memory setup honcho    # Configure Honcho directly (works before activation)
rayovin honcho status          # Connection status, config, and key settings
rayovin honcho setup           # Redirects to `rayovin memory setup` (post-activation alias)
rayovin honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
rayovin honcho peer            # Show or update peer names + dialectic reasoning level
rayovin honcho mode            # Show or set recall mode (hybrid/context/tools)
rayovin honcho tokens          # Show or set token budget for context and dialectic
rayovin honcho identity        # Seed or show the AI peer's Honcho identity
rayovin honcho sync            # Sync Honcho config to all existing profiles
rayovin honcho peers           # Show peer identities across all profiles
rayovin honcho sessions        # List known Honcho session mappings
rayovin honcho map             # Map current directory to a Honcho session name
rayovin honcho enable          # Enable Honcho for the active profile
rayovin honcho disable         # Disable Honcho for the active profile
rayovin honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `rayovin honcho`

If you previously used the standalone `rayovin honcho setup`:
...
No re-login or re-setup needed. Run `rayovin memory setup` and select "honcho" — the wizard detects your existing config.
