...
Rayovin Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
rayovin memory setup      # interactive picker + configuration
rayovin memory status     # check what's active
rayovin memory off        # disable external provider
```

You can also select the active memory provider via `rayovin plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.rayovin/config.yaml`:
...
When a memory provider is active, Rayovin automatically:
...
**Setup Wizard:**
```bash
rayovin memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `rayovin honcho setup` command still works (it now redirects to `rayovin memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$RAYOVIN_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$RAYOVIN_HOME/honcho.json` > `~/.rayovin/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/NousResearch/rayovin-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/rayovin).
...
:::tip Migrating from `rayovin honcho`
If you previously used `rayovin honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Rayovin profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Rayovin profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Rayovin profile. Host key `rayovin` → default; `rayovin.<profile>` for others. |
...
```bash
rayovin profile create coder --clone
```

`--clone` creates a `rayovin.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Rayovin profile, creates host blocks for any profile without one, inherits settings from the default `rayovin` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `rayovin memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/NousResearch/rayovin-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/rayovin).
...
rayovin memory setup    # select "openviking"
# Or manually:
rayovin config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.rayovin/.env
# Authenticated servers should use a user/admin API key:
echo "OPENVIKING_API_KEY=..." >> ~/.rayovin/.env
```
...
`OPENVIKING_AGENT` is Rayovin' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
rayovin memory setup    # select "mem0" → "Platform"
# Or manually:
rayovin config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.rayovin/.env
```

**Setup (OSS):**
```bash
rayovin memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
rayovin memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
rayovin memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
rayovin memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
rayovin memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.rayovin/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.rayovin/.env
```
...
**Config:** `$RAYOVIN_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.rayovin/.env`.
...
| `user_id` | `rayovin-user` | User identifier |
| `agent_id` | `rayovin` | Agent identifier |
...
**Switching modes:** Re-run `rayovin memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
rayovin memory setup    # select "hindsight"
# Or manually:
rayovin config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.rayovin/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p rayovin ui start`

**Config:** `$RAYOVIN_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `rayovin` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Rayovin Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/NousResearch/rayovin-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
rayovin memory setup    # select "holographic"
# Or manually:
rayovin config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.rayovin-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$RAYOVIN_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
rayovin memory setup    # select "retaindb"
# Or manually:
rayovin config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.rayovin/.env
```
...
# Then configure Rayovin
rayovin memory setup    # select "byterover"
# Or manually:
rayovin config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$RAYOVIN_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [API key](http://app.supermemory.ai/integrations?connect=rayovin) |
...
**Setup:**
```bash
rayovin memory setup    # select "supermemory"
# Or manually:
rayovin config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.rayovin/.env
```

**Config:** `$RAYOVIN_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `rayovin` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `rayovin-{identity}` → `rayovin-coder`) to isolate memories per Rayovin profile
...
| **Requires** | `pip install rayovin-memori` + `rayovin-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install rayovin-memori
rayovin-memori install
rayovin config set memory.provider memori
rayovin memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `rayovin-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$RAYOVIN_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$RAYOVIN_HOME/` so each profile has its own credentials
