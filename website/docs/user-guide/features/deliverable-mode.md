...
When Rayovin Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.rayovin/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.rayovin/config.yaml` (switchable per session
via `/personality`).
...
If you use Rayovin' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.rayovin/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Rayovin Agent's deliverable
