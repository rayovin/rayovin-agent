...
description: "Customize Rayovin Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Rayovin Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `RAYOVIN_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Rayovin is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Rayovin now seeds a default `SOUL.md` automatically in:

```text
~/.rayovin/SOUL.md
```

More precisely, it uses the current instance's `RAYOVIN_HOME`, so if you run Rayovin with a custom home directory, it will use:

```text
$RAYOVIN_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- Rayovin creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Rayovin loads `SOUL.md` only from `RAYOVIN_HOME`
- Rayovin does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Rayovin falls back to a built-in default identity
...
If Rayovin loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `RAYOVIN_HOME`, the personality belongs to the Rayovin instance itself.

That also makes it easier to teach users:
- "Edit `~/.rayovin/SOUL.md` to change Rayovin' default personality."
...
- how Rayovin should handle uncertainty, disagreement, or ambiguity
...
## What Rayovin injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Rayovin falls back to a built-in default identity ("You are Rayovin Agent, an intelligent AI assistant created by نبض آینده جنوب (Nabz-e-Ayandeh-e-Jonoob)..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Rayovin ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Rayovin, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Rayovin its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.rayovin/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.rayovin/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-rayovin)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Rayovin speaks
- `display.skin` and `/skin` affect how Rayovin looks in the terminal
