...
Connect Rayovin to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Rayovin supports two WeCom integration modes:
...
> Run `rayovin gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Rayovin decrypts the message, queues it for the agent
...
(Use `rayovin gateway start` only after `rayovin gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Rayovin and the token the
...
in `~/.rayovin/.env` values around `=` will also break signature checks. After
fixing, restart `rayovin gateway run`.
...
Check `rayovin gateway run` logs for the bound host/port. If the adapter bound to
