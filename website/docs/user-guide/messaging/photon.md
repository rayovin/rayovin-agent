...
Connect Rayovin to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Rayovin — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Rayovin runs it in a
...
rayovin photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Rayovin Agent` project on your account.
...
Runtime credentials are written to `~/.rayovin/.env`
...
(device token, dashboard project id) lives in `~/.rayovin/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Rayovin
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Rayovin replies with a pairing code. Approve it with:

```bash
rayovin pairing approve photon <CODE>
```

Use `rayovin pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.rayovin/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.rayovin/.env`):
...
By default Rayovin responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Rayovin` and
`@Rayovin agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Rayovin will reply.

## Status & troubleshooting

```bash
rayovin photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Rayovin uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run rayovin photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `rayovin photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `rayovin photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `rayovin photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Rayovin sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Rayovin wake words  | JSON list / comma / newline regex patterns for group mentions |
