...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Rayovin Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Rayovin to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.rayovin/.env`:
...
All standard Rayovin commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.rayovin/logs/gateway.log`
...
4. Consider enabling verbose logging: `RAYOVIN_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Rayovin automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
RAYOVIN_LOG_LEVEL=debug rayovin gateway
```
...
```bash
rayovin chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
