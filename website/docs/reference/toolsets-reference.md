...
description: "Reference for Rayovin core, composite, platform, and dynamic toolsets"
---
...
- **Platform** — A complete tool configuration for a specific deployment context (e.g., `rayovin-cli` is the default for interactive CLI sessions)
...
```bash
rayovin chat --toolsets web,file,terminal
rayovin chat --toolsets debugging        # composite — expands to file + terminal + web
rayovin chat --toolsets all              # everything
```
...
```yaml
toolsets:
  - rayovin-cli          # default for CLI
  # - rayovin-telegram   # override for Telegram gateway
```

### Interactive management

```bash
rayovin tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call Rayovin tools programmatically. |
...
| `discord` | `discord` | Core Discord text/embed/DM actions (gateway-only). Active on the `rayovin-discord` toolset. |
| `discord_admin` | `discord_admin` | Discord moderation (bans, role changes, channel management). Active on the `rayovin-discord` toolset; requires the bot to hold the relevant Discord permissions. |
...
| `feishu_drive` | `feishu_drive_add_comment`, `feishu_drive_list_comments`, `feishu_drive_list_comment_replies`, `feishu_drive_reply_comment` | Feishu/Lark drive comment operations. Scoped to the comment agent; not exposed on `rayovin-cli` or other messaging toolsets. |
...
| `x_search` | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool. Off by default; opt in via `rayovin tools`. Schema only registered when xAI credentials (SuperGrok OAuth or `XAI_API_KEY`) are configured. |
| `yuanbao` | `yb_query_group_info`, `yb_query_group_members`, `yb_search_sticker`, `yb_send_dm`, `yb_send_sticker` | Yuanbao DM/group actions and sticker search. Registered only on `rayovin-yuanbao`. |

## Platform Toolsets

Platform toolsets define the complete tool configuration for a deployment target. Most messaging platforms use the same set as `rayovin-cli`:

| Toolset | Differences from `rayovin-cli` |
|---------|-------------------------------|
| `rayovin-cli` | Full toolset — the default for interactive CLI sessions. Includes file, terminal, web, browser, memory, skills, vision, image_gen, todo, tts, delegation, code_execution, cronjob, session_search, and clarify, plus the `safe` (read-only) bundle. |
| `rayovin-acp` | Drops `clarify`, `cronjob`, `image_generate`, `text_to_speech`, and all four Home Assistant tools. Focused on coding tasks in IDE context. |
| `rayovin-api-server` | Drops `clarify` and `text_to_speech`. Keeps everything else — suitable for programmatic access where user interaction isn't possible. |
| `rayovin-cron` | Same as `rayovin-cli`. |
| `rayovin-telegram` | Same as `rayovin-cli`. |
| `rayovin-discord` | Adds `discord` and `discord_admin` on top of `rayovin-cli`. |
| `rayovin-slack` | Same as `rayovin-cli`. |
| `rayovin-whatsapp` | Same as `rayovin-cli`. |
| `rayovin-signal` | Same as `rayovin-cli`. |
| `rayovin-matrix` | Same as `rayovin-cli`. |
| `rayovin-mattermost` | Same as `rayovin-cli`. |
| `rayovin-email` | Same as `rayovin-cli`. |
| `rayovin-sms` | Same as `rayovin-cli`. |
| `rayovin-bluebubbles` | Same as `rayovin-cli`. |
| `rayovin-dingtalk` | Same as `rayovin-cli`. |
| `rayovin-feishu` | Adds the five `feishu_doc_*` / `feishu_drive_*` tools (only used by the document-comment handler, not the regular chat adapter). |
| `rayovin-qqbot` | Same as `rayovin-cli`. |
| `rayovin-wecom` | Same as `rayovin-cli`. |
| `rayovin-wecom-callback` | Same as `rayovin-cli`. |
| `rayovin-weixin` | Same as `rayovin-cli`. |
| `rayovin-yuanbao` | Adds the five `yb_*` tools (DM/group/sticker) on top of `rayovin-cli`. |
| `rayovin-homeassistant` | Same as `rayovin-cli` (the Home Assistant tools are already present by default and activate when `HASS_TOKEN` is set). |
| `rayovin-webhook` | Same as `rayovin-cli`. |
| `rayovin-gateway` | Internal gateway orchestrator toolset — union of every `rayovin-<platform>` toolset; used when the gateway needs to accept any message source. |
...
- **Workflow-gated** tools — the `kanban` toolset — are deliberately opt-in. `all`/`*` does **not** enable kanban; you must list `kanban` explicitly (or be a dispatcher-spawned worker with `RAYOVIN_KANBAN_TASK` set). Kanban tools mutate shared board state, so they stay off by default even under `all`.

## Relationship to `rayovin tools`

The `rayovin tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
