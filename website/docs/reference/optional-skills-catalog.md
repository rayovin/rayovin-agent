...
description: "Official optional skills shipped with rayovin-agent — install via rayovin skills install official/<category>/<skill>"
---

# Optional Skills Catalog

Optional skills ship with rayovin-agent under `optional-skills/` but are **not active by default**. Install them explicitly:

```bash
rayovin skills install official/<category>/<skill>
```

For example:

```bash
rayovin skills install official/blockchain/solana
rayovin skills install official/mlops/flash-attention
```
...
```bash
rayovin skills uninstall <skill-name>
```
...
| [**kanban-video-orchestrator**](/docs/user-guide/skills/optional/creative/creative-kanban-video-orchestrator) | Plan, set up, and monitor a multi-agent video production pipeline backed by Rayovin Kanban. Use when the user wants to make ANY video — narrative film, product/marketing, music video, explainer, ASCII/terminal art, abstract/generative loo... |
...
| [**rayovin-s6-container-supervision**](/docs/user-guide/skills/optional/devops/devops-rayovin-s6-container-supervision) | Modify, debug, or extend the s6-overlay supervision tree inside the Rayovin Agent Docker image — adding new services, debugging profile gateways, understanding the Architecture B main-program pattern. |
...
| Skill | Description |
|-------|-------------|
| [**agentmail**](/docs/user-guide/skills/optional/email/email-agentmail) | Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. rayovin-agent@agentmail.to). |
...
| Skill | Description |
|-------|-------------|
| [**openclaw-migration**](/docs/user-guide/skills/optional/migration/migration-openclaw-migration) | Migrate a user's OpenClaw customization footprint into Rayovin Agent. Imports Rayovin-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be mig... |
...
| [**telephony**](/docs/user-guide/skills/optional/productivity/productivity-telephony) | Give Rayovin phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi. |
