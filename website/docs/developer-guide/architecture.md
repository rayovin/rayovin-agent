...
sidebar_position: 1
title: "Architecture"
description: "Rayovin Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Rayovin Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ rayovin_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # RayovinCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── rayovin_state.py           # SQLite session/state database with FTS5
├── rayovin_constants.py       # RAYOVIN_HOME, profile-aware paths
...
│
├── rayovin_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `rayovin` subcommands (large file)
...
│   ├── skills_config.py      # rayovin skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # rayovin tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # rayovin gateway start/stop
│
...
```text
User input → RayovinCLI.process_input()
...
Three discovery sources: `~/.rayovin/plugins/` (user), `.rayovin/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `rayovin plugins` or `config.yaml`.
...
Exposes Rayovin as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`rayovin -p <name>`) gets its own RAYOVIN_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
