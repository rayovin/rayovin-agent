...
The ACP adapter wraps Rayovin' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
rayovin acp / rayovin-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.rayovin/.env
  -> configure stderr logging
  -> construct RayovinACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'rayovin-agent[acp]==<version>' rayovin-acp`, pointed at the `rayovin-agent` PyPI release.
...
- `allow_once` -> Rayovin `once`
- `allow_always` -> Rayovin `always`
- reject options -> Rayovin `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Rayovin tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["rayovin-acp"])
...
Instead it reuses Rayovin' runtime resolver:

- `acp_adapter/auth.py`
- `rayovin_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Rayovin provider/credentials. It also always advertises a terminal setup auth method (`rayovin-setup`, args `--setup`) so first-run registry clients can open Rayovin' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.rayovin/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `rayovin-acp` toolset definition
- `rayovin_cli/main.py` — `rayovin acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `rayovin-acp` script
