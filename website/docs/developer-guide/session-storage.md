...
Rayovin Agent uses a SQLite database (`~/.rayovin/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `rayovin_state.py`


## Architecture Overview

```
~/.rayovin/state.db (SQLite, WAL mode)
...
Multiple rayovin processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.rayovin/state.db
...
Default path: `~/.rayovin/state.db`

This is derived from `rayovin_constants.get_rayovin_home()` which resolves to
`~/.rayovin/` by default, or the value of `RAYOVIN_HOME` environment variable.
