...
description: "How Rayovin stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `rayovin_cli/cron.py` | CLI `rayovin cron` subcommands |
...
Jobs are stored in `~/.rayovin/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `rayovin cron` commands are run or during active CLI sessions.
...
```python
# ~/.rayovin/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.rayovin/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `rayovin cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `rayovin cron` CLI provides direct job management:

```bash
rayovin cron list                    # Show all jobs
rayovin cron create                  # Interactive job creation (alias: add)
rayovin cron edit <job_id>           # Edit job configuration
rayovin cron pause <job_id>          # Pause a running job
rayovin cron resume <job_id>         # Resume a paused job
rayovin cron run <job_id>            # Trigger immediate execution
rayovin cron remove <job_id>         # Delete a job
```
