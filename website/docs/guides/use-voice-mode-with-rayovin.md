---
sidebar_position: 8
title: "Use Voice Mode with Rayovin"
description: "A practical guide to setting up and using Rayovin voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Rayovin
...
- you want Rayovin sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Rayovin.
...
## Step 1: make sure normal Rayovin works first
...
Rayovin supports both local and cloud speech stacks.
...
### If you use `rayovin setup`

If you choose NeuTTS in the setup wizard, Rayovin checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Rayovin transcribes and responds
...
- asking Rayovin to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Rayovin loop.

## Tuning CLI behavior

### Silence threshold

If Rayovin starts/stops too aggressively, tune:
...
Rayovin stays a normal chat bot, but can speak replies.
...
- you want Rayovin to function like a portable research or ops assistant
...
Rayovin joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Rayovin detects speech boundaries
- transcripts are posted in the associated text channel
- Rayovin responds in text and audio
...
1. get text Rayovin working
2. install `rayovin-agent[voice]`
