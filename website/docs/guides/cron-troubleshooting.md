...
description: "Diagnose and fix common Rayovin cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`rayovin gateway` for foreground, or `rayovin gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `rayovin cron tick`.
...
```bash
date
rayovin cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.rayovin/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.rayovin/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.rayovin/.env` |
...
| `local` | Write access to `~/.rayovin/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `rayovin cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `rayovin skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `rayovin skills list`.
...
2. `~/.rayovin/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `rayovin cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Rayovin config directory). Verify:
```bash
ls ~/.rayovin/scripts/your-script.py   # Must exist
rayovin cron edit <job_id> --script ~/.rayovin/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `rayovin skills install <skill-name>`.
...
Jobs are stored in `~/.rayovin/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.rayovin/cron/jobs.json
chmod 600 ~/.rayovin/cron/jobs.json   # Your user should own it
```
...
```bash
rayovin cron list                    # Show all jobs, states, next_run times
rayovin cron run <job_id>            # Schedule for next tick (for testing)
rayovin cron edit <job_id>           # Fix configuration issues
rayovin logs                         # View recent Rayovin logs
rayovin skills list                  # Verify installed skills
```
...
1. Run the job with `rayovin cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.rayovin/logs/agent.log` for scheduler messages and `~/.rayovin/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/rayovin-agent](https://github.com/NousResearch/rayovin-agent) with:
