...
```bash
rayovin teams-pipeline validate
```
...
```bash
rayovin teams-pipeline token-health
rayovin teams-pipeline token-health --force-refresh
```
...
```bash
rayovin teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
rayovin teams-pipeline maintain-subscriptions
rayovin teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Rayovin cron (recommended if you already run the Rayovin gateway)

Rayovin ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.rayovin/scripts/`. First create the script:

```bash
mkdir -p ~/.rayovin/scripts
cat > ~/.rayovin/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec rayovin teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.rayovin/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
rayovin cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/rayovin-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Rayovin Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=rayovin
EnvironmentFile=/etc/rayovin/env
ExecStart=/usr/local/bin/rayovin teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/rayovin-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Rayovin Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now rayovin-teams-pipeline-maintain.timer
systemctl list-timers rayovin-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/rayovin teams-pipeline maintain-subscriptions >> /var/log/rayovin/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.rayovin/.env` at the top of a wrapper script that crontab calls.
...
```bash
rayovin teams-pipeline subscriptions   # should show expirationDateTime advanced
rayovin teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
rayovin teams-pipeline list
rayovin teams-pipeline list --status failed
rayovin teams-pipeline show <job-id>
```

### Replay a stored job

```bash
rayovin teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
rayovin teams-pipeline fetch --meeting-id <meeting-id>
rayovin teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `rayovin teams-pipeline maintain-subscriptions --dry-run`
- inspect `rayovin teams-pipeline list --status failed`
...
- run `rayovin teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `rayovin teams-pipeline run`
...
- [ ] `rayovin teams-pipeline validate` returns an OK snapshot
- [ ] `rayovin teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Rayovin cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
