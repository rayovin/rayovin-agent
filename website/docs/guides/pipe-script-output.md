...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `rayovin send`."
---

# Pipe Script Output to Messaging Platforms

`rayovin send` is a small, scriptable CLI that pushes a message to any
messaging platform Rayovin is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | rayovin send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `rayovin
...
rayovin send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
rayovin send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
rayovin send --list

# Filter by platform
rayovin send --list telegram
```
...
Any platform Rayovin ships adapters for works as a target:
...
`rayovin send` resolves the message body in this order:

1. **Positional argument** — `rayovin send --to telegram "hi"`
2. **`--file PATH`** — `rayovin send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | rayovin send --to telegram`

When stdin is a TTY (no pipe), Rayovin does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  rayovin send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `rayovin send` reuses your Rayovin config, the same script works on
any host where Rayovin is installed — no need to export bot tokens into
...
`rayovin send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  rayovin send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | rayovin send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.rayovin/bin/rayovin send \
...
```bash
./train.py --epochs 200 && \
  rayovin send --to telegram "training done" || \
  rayovin send --to telegram "training failed (exit $?)"
```
...
rayovin send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(rayovin send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `rayovin send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `rayovin send` calls
the platform's REST endpoint directly using credentials from
`~/.rayovin/.env` and `~/.rayovin/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `rayovin gateway start` and retry.
...
# Machine-readable
rayovin send --list --json
```

The listing is built from `~/.rayovin/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`rayovin gateway
...
| Approach | Multi-platform | Reuses Rayovin creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `rayovin send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`rayovin send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `rayovin send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `rayovin send` shares with cron delivery.
