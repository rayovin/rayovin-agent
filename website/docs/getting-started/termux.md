...
description: "Run Rayovin Agent directly on an Android phone with Termux"
---

# Rayovin on Android with Termux
...
Rayovin Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Rayovin from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- links `rayovin` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `rayovin` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/rayovin" "$PREFIX/bin/rayovin"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `rayovin` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.rayovin/.env`.
...
### `rayovin doctor` says ripgrep or Node is missing
