...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.rayovin/.env ] && grep -q "^GITHUB_TOKEN=" ~/.rayovin/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.rayovin/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Rayovin Agent\",
    \"comments\": [
...
```bash
source "${RAYOVIN_HOME:-$HOME/.rayovin}/skills/github/github-auth/scripts/gh-env.sh"
...
```bash
# 若无问题——批准
gh pr review $PR_NUMBER --approve --body "Reviewed by Rayovin Agent. Code looks clean — good test coverage, no security concerns."
