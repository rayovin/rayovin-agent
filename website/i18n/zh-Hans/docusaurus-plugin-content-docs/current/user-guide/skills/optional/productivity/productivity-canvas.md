...
| | |
|---|---|
| 来源 | 可选 — 通过 `rayovin skills install official/productivity/canvas` 安装 |
...
4. 为 token 命名（例如 "Rayovin Agent"），设置可选的过期时间，然后点击 **Generate Token**
5. 复制 token 并添加到 `~/.rayovin/.env`：
...
```bash
CANVAS="python $RAYOVIN_HOME/skills/productivity/canvas/scripts/canvas_api.py"
