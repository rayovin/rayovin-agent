...
# Do not write .pyc files at runtime: /opt/rayovin is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/rayovin/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when rayovin
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# rayovin process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main rayovin process, the dashboard,
...
RUN set -eu; \
    ...
    # 'Rayovin WebUI' catalog). The image has moved to s6-overlay /init\
...
# Non-root user for runtime; UID can be overridden via RAYOVIN_UID at runtime
RUN useradd -u 10000 -m -d /opt/data rayovin
...
#
# ui-tui/packages/rayovin-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/rayovin-ink/ ui-tui/packages/rayovin-ink/
# apps/shared/ is copied IN FULL because web/package.json references it as a
# `file:` workspace dependency (same pattern as rayovin-ink above).
...
# lock on the @rayovin/ink entry, tripped the TUI launcher's
...
# for the same reason: it lazy-installs into /opt/rayovin/.venv at first
...
# gives the non-root rayovin user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# ---------- Permissions ----------
# Link rayovin-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Wire the exec shim and install-method stamp.  Files under /opt/rayovin are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the rayovin user (go-w from the --chmod above).

USER root
RUN mkdir -p /opt/rayovin/bin && \
    cp /opt/rayovin/docker/rayovin-exec-shim.sh /opt/rayovin/bin/rayovin && \
    chmod 0755 /opt/rayovin/bin/rayovin && \
    printf 'docker\n' > /opt/rayovin/.install_method
# The ``.install_method`` stamp is baked next to the running code (the install
# tree), NOT into $RAYOVIN_HOME. $RAYOVIN_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install; stamping it at boot used to clobber that
# host install's marker and wrongly block its ``rayovin update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the rayovin user via
# `s6-setuidgid rayovin` in its run script. If RAYOVIN_UID is unset, services
# run as the default rayovin user (UID 10000).

# ---------- Bake build-time git revision ----------
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `rayovin dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the RAYOVIN_GIT_SHA build-arg to
# /opt/rayovin/.rayovin_build_sha at build time, and have
# rayovin_cli/build_info.py read it at runtime.  Both `rayovin dump` and
...
ARG RAYOVIN_GIT_SHA=
RUN if [ -n "${RAYOVIN_GIT_SHA}" ]; then \
        printf '%s\n' "${RAYOVIN_GIT_SHA}" > /opt/rayovin/.rayovin_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-rayovin + dashboard.
...
# `exec rayovin`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $RAYOVIN_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/rayovin/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-rayovin-setup && \
    chmod +x /etc/cont-init.d/01-rayovin-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV RAYOVIN_WEB_DIST=/opt/rayovin/rayovin_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/rayovin/ui-tui/dist/entry.js`) and skip the
...
ENV RAYOVIN_TUI_DIR=/opt/rayovin/ui-tui
ENV RAYOVIN_HOME=/opt/data
ENV RAYOVIN_WRITE_SAFE_ROOT=/opt/data
ENV RAYOVIN_DISABLE_LAZY_INSTALLS=1
# The published image seals /opt/rayovin (root-owned, read-only) so a runtime
...
# is seeded + chowned to the rayovin user by docker/stage2-hook.sh and lives
...
ENV RAYOVIN_LAZY_INSTALL_TARGET=/opt/data/lazy-packages

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> rayovin ...` they default to root, and any file the
# command writes under $RAYOVIN_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/rayovin/bin/rayovin, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid rayovin`
# when invoked as root. Non-root callers (supervised processes,
# `--user rayovin`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/rayovin/.venv/bin/rayovin). See the shim source for
# the opt-out env var (RAYOVIN_DOCKER_EXEC_AS_ROOT=1).

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> rayovin ...` and any
# subprocess that doesn't activate the venv first still find rayovin.
#
# /opt/rayovin/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/rayovin/bin:/opt/rayovin/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. rayovin
# subcommand vs. no-args), drops to the rayovin user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/rayovin/docker/main-wrapper.sh" ]
CMD [ ]
