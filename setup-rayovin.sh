...
# 5. Symlinks the 'rayovin' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Rayovin Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/rayovin-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink rayovin into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up rayovin command..."

RAYOVIN_BIN="$SCRIPT_DIR/venv/bin/rayovin"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$RAYOVIN_BIN" "$COMMAND_LINK_DIR/rayovin"
echo -e "${GREEN}✓${NC} Symlinked rayovin → $COMMAND_LINK_DISPLAY_DIR/rayovin"
...
# ============================================================================
# Seed bundled skills into ~/.rayovin/skills/
# ============================================================================

RAYOVIN_SKILLS_DIR="${RAYOVIN_HOME:-$HOME/.rayovin}/skills"
mkdir -p "$RAYOVIN_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.rayovin/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$RAYOVIN_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  rayovin status        # Check configuration"
if is_termux; then
    echo "  rayovin gateway       # Run gateway in foreground"
else
    echo "  rayovin gateway install # Install gateway service (messaging + cron)"
fi
echo "  rayovin cron list     # View scheduled jobs"
echo "  rayovin doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m rayovin_cli.main setup
fi
