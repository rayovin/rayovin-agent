"""Shared constants for Rayovin Agent.
...
_UNSET = object()
_RAYOVIN_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_RAYOVIN_HOME_OVERRIDE", default=_UNSET
)


def set_rayovin_home_override(path: str | Path | None) -> Token:
    """Set a context-local Rayovin home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _RAYOVIN_HOME_OVERRIDE.set(value)


def reset_rayovin_home_override(token: Token) -> None:
    """Restore the previous context-local Rayovin home override."""
    _RAYOVIN_HOME_OVERRIDE.reset(token)


def get_rayovin_home_override() -> str | None:
    """Return the active context-local Rayovin home override, if any."""
    override = _RAYOVIN_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_rayovin_home() -> Path:
    """Return the platform-native default Rayovin home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "rayovin"
    return Path.home() / ".rayovin"


def get_rayovin_home() -> Path:
    """Return the Rayovin home directory (default: platform-native path).

    Reads RAYOVIN_HOME env var, falls back to the platform-native default.
    This is the single source of truth — all other copies should import this.

    When ``RAYOVIN_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``RAYOVIN_HOME`` explicitly (see the systemd
    template in ``rayovin_cli/gateway.py`` and the kanban dispatcher in
    ``rayovin_cli/kanban_db.py``).  See https://github.com/NousResearch/rayovin-agent/issues/18594.
    """
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_rayovin_home()
        ...
        if active and active != "default":
            ...
            msg = (
                f"[RAYOVIN_HOME fallback] RAYOVIN_HOME is unset but active "
                f"profile is {active!r}. Falling back to {fallback_home}, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass RAYOVIN_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
    ...
    return _get_platform_default_rayovin_home()


def get_default_rayovin_root() -> Path:
    """Return the root Rayovin directory for profile-level operations.

    In standard deployments this is the platform-native Rayovin home
    (``~/.rayovin`` on POSIX, ``%LOCALAPPDATA%\\rayovin`` on native Windows).

    In Docker or custom deployments where ``RAYOVIN_HOME`` points outside
    ``~/.rayovin`` (e.g. ``/opt/data``), returns ``RAYOVIN_HOME`` directly
    — that IS the root.

    In profile mode where ``RAYOVIN_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.rayovin/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_rayovin_home()
    env_home = os.environ.get("RAYOVIN_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # RAYOVIN_HOME is under ~/.rayovin (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — RAYOVIN_HOME itself is the root
    return env_path
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Rayovin is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``RAYOVIN_OPTIONAL_SKILLS``.
    """
    override = os.getenv("RAYOVIN_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_rayovin_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``RAYOVIN_OPTIONAL_MCPS``.
    """
    override = os.getenv("RAYOVIN_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_rayovin_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``RAYOVIN_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<RAYOVIN_HOME>/skills`` last-resort
    """
    override = os.getenv("RAYOVIN_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_rayovin_home() / "skills"


def get_rayovin_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Rayovin subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to RAYOVIN_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to RAYOVIN_HOME (e.g. ``"image_cache"``).
...
def iter_rayovin_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Rayovin-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\rayovin\\node``. POSIX installs use
    ``$RAYOVIN_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_rayovin_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with rayovinManagedNodePathEntries() in
...
_RAYOVIN_NODE_TARGET_MAJOR = int(os.environ.get("RAYOVIN_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Rayovin-managed Node trees live under ``$RAYOVIN_HOME/node`` (or a profile's
    ``RAYOVIN_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_rayovin_node_executable``
    used to trust file presence alone, so ``rayovin update`` would pick that
...
def rayovin_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Rayovin-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_rayovin_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%RAYOVIN_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    home = get_rayovin_home()
    index_url = f"https://nodejs.org/dist/latest-v{_RAYOVIN_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_RAYOVIN_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def heal_rayovin_managed_node() -> bool:
    """Redownload Rayovin-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not rayovin_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "RAYOVIN_HOME": str(get_rayovin_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def find_rayovin_node_executable(command: str) -> str | None:
    """Return a Rayovin-managed Node/npm executable path, healing broken trees."""
    names = _candidate_node_command_names(command)
    broken_present = False
    for directory in iter_rayovin_node_dirs():
        for name in names:
            candidate = directory / name
            if candidate.is_file() and (
                sys.platform == "win32" or os.access(candidate, os.X_OK)
                ...
                if node_tool_runnable(resolved):
                    return resolved
                broken_present = True
    if broken_present and heal_rayovin_managed_node():
        for directory in iter_rayovin_node_dirs():
            for name in names:
                candidate = directory / name
                if candidate.is_file() and (
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Rayovin-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Rayovin-managed installs.

    This is for Rayovin-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_rayovin_node_executable(command)
    if managed:
        return managed
    if rayovin_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_rayovin_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Rayovin-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_rayovin_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``rayovin update`` — leaving a **dangling symlink** that ``which`` still
...
def display_rayovin_home() -> str:
    """Return a user-friendly display string for the current RAYOVIN_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.rayovin``
        profile:  ``~/.rayovin/profiles/coder``
        custom:   ``/opt/rayovin-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.rayovin``.  For code that needs a real ``Path``, use
    :func:`get_rayovin_home` instead.
    """
    home = get_rayovin_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``RAYOVIN_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/NousResearch/rayovin-agent/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{RAYOVIN_HOME}/home`` when the profile-home directory exists."""
    rayovin_home = get_rayovin_home_override() or (env or {}).get("RAYOVIN_HOME") or os.getenv("RAYOVIN_HOME")
    if not rayovin_home:
        return None
    profile_home = os.path.join(rayovin_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("RAYOVIN_REAL_HOME") or os.getenv("RAYOVIN_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Rayovin profile HOME.

    ``RAYOVIN_HOME`` scopes Rayovin state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={RAYOVIN_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{RAYOVIN_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{RAYOVIN_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Rayovin' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["RAYOVIN_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Rayovin itself runs inside WSL.
...
def is_container() -> bool:
    ...
    See: NousResearch/rayovin-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under RAYOVIN_HOME.

    Replaces the ``get_rayovin_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, rayovin_logging.py, rayovin_time.py, etc.).
    """
    return get_rayovin_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under RAYOVIN_HOME."""
    return get_rayovin_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under RAYOVIN_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_rayovin_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._rayovin_ipv4_patched = True  # type: ignore[attr-defined]
