...
<url>``) is spawned as a direct child of the Rayovin process. Rayovin's own
...
Rayovin process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Rayovin startup's opt-in
