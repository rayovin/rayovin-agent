...
One ``CDPSupervisor`` runs per Rayovin ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Rayovin logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "rayovin-dialog-bridge.invalid"
...
(() => {
  if (window.__rayovinDialogBridgeInstalled) return;
  window.__rayovinDialogBridgeInstalled = true;
  const ENDPOINT = "http://rayovin-dialog-bridge.invalid/";
