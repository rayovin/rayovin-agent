...
This module provides the glue:
    - ``RayovinTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Rayovin Agent"
"""
...
from rayovin_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__rayovin_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses RAYOVIN_HOME so each profile gets its own OAuth tokens.
    Layout: ``RAYOVIN_HOME/mcp-tokens/``
    """
    try:
        from rayovin_constants import get_rayovin_home
        base = Path(get_rayovin_home())
    except ImportError:
        base = Path(os.environ.get("RAYOVIN_HOME", str(Path.home() / ".rayovin")))
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``rayovin mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `rayovin mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# RayovinTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class RayovinTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        RAYOVIN_HOME/mcp-tokens/<server_name>.json         -- tokens
        RAYOVIN_HOME/mcp-tokens/<server_name>.client.json   -- client info
        RAYOVIN_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Rayovin records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Rayovin.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://rayovin.github.io/rayovin-agent/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `rayovin mcp login <server>` later to "
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = RayovinTokenStorage(server_name)
    storage.remove()
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Rayovin Agent")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`rayovin mcp login {server_name}` interactively first to complete "
