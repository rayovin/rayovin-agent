...
`rayovin skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.rayovin.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.rayovin.blueprint`` is a mapping containing
    ...
    rayovin = meta.get("rayovin") if isinstance(meta, dict) else None
    blueprint = rayovin.get("blueprint") if isinstance(rayovin, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.rayovin.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.rayovin.blueprint`` block) they can hand
    to ``rayovin skills publish`` to share. ``body`` is the plain-language
