...
transport, discovers their tools, and registers them into the rayovin-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.rayovin/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.rayovin/logs/mcp-stderr.log), tagged with the
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Rayovin' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            rayovin_home = os.path.expanduser(
                os.getenv(
                    "RAYOVIN_HOME", os.path.join(os.path.expanduser("~"), ".rayovin")
                )
            )
            candidates = [
                os.path.join(rayovin_home, "node", "bin", resolved_command),
                ...
                # image (which the Rayovin Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Rayovin MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Rayovin gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Rayovin document cache; returns a marker with
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Rayovin' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (RAYOVIN_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit RAYOVIN_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into Rayovin' logging (agent.log via rayovin_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'rayovin-agent[mcp]'\n"
                "or (full install):\n"
                "  pip install 'rayovin-agent[all]'"
            )
        ...
        # ungraceful exit of this Rayovin process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Rayovin' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Rayovin is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `rayovin mcp login {server_name}` (or delete the tokens "
            f"file under ~/.rayovin/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local RAYOVIN_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_rayovin_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_rayovin_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local RAYOVIN_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_rayovin_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Rayovin config file.
    ...
    ``os.environ`` (which includes ``~/.rayovin/.env`` loaded at startup).
    """
    try:
        from rayovin_cli.config import load_config
        from utils import env_var_enabled as _env_enabled

        if _env_enabled("RAYOVIN_SAFE_MODE"):
            return {}
        config = load_config()
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Rayovin' MEDIA: tag convention and out to messaging
            ...
            # Rayovin' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Rayovin's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Rayovin uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Rayovin registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``rayovin tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Rayovin registry.
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple rayovin
