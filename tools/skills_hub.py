#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Rayovin Skills Hub.
...
Used by rayovin_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _rayovin_home() -> Path:
    return get_rayovin_home()


def _skills_dir() -> Path:
    forced = _override("SKILLS_DIR")
    return Path(forced) if forced is not None else _rayovin_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            rayovin_meta = metadata.get("rayovin", {})
            if isinstance(rayovin_meta, dict):
                tags = rayovin_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            rayovin_meta = metadata.get("rayovin", {})
            if isinstance(rayovin_meta, dict):
                raw_tags = rayovin_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by نبض آینده جنوب (Nabz-e-Ayandeh-e-Jonoob)) but not activated
    ...
    OFFICIAL_REPO = "NousResearch/rayovin-agent"
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                rayovin_meta = meta_block.get("rayovin", {})
                if isinstance(rayovin_meta, dict):
                    tags = rayovin_meta.get("tags", [])
...
def _rayovin_index_cache_file() -> Path:
    return _index_cache_dir() / "rayovin-index.json"


def _load_rayovin_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for RAYOVIN_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    rayovin_index_cache_file = _rayovin_index_cache_file()
    if rayovin_index_cache_file.exists():
        try:
            age = time.time() - rayovin_index_cache_file.stat().st_mtime
            if age < RAYOVIN_INDEX_TTL:
                return json.loads(rayovin_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Rayovin index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Rayovin index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Rayovin index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        rayovin_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        rayovin_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class RayovinIndexSource(SkillSource):
    """Skill source backed by the centralized Rayovin Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_rayovin_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "rayovin-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "rayovin-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        RayovinIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "rayovin-index"
