# Rayovin Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: rayovin-achievements plugin
...
### `GET /api/plugins/rayovin-achievements/achievements`
...
### `POST /api/plugins/rayovin-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/rayovin-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/rayovin-achievements/overview`
...
- Persist plugin data under:
  - `~/.rayovin/plugins/rayovin-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.rayovin/plugins/rayovin-achievements/scan_checkpoint.json`
...
- `~/.rayovin/plugins/rayovin-achievements/`
