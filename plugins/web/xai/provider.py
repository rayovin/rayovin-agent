...
``{title, url, description, position}`` rows every other Rayovin web
provider produces.
...
prefers Rayovin-managed xAI Grok OAuth (via ``rayovin auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.rayovin/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``rayovin tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `rayovin auth` to sign in with "
