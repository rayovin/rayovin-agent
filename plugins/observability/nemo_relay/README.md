...
Optional Rayovin observability plugin that maps Rayovin observer hooks to
...
not replace Rayovin Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Rayovin emit NeMo Relay lifecycle events for
the work Rayovin already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Rayovin Agent can:

- Preserve Rayovin execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
rayovin plugins enable observability/nemo_relay
```

The `RAYOVIN_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `RAYOVIN_HOME` that the
agent run will use:

```bash
env RAYOVIN_HOME=/tmp/rayovin-nemo-relay-test \
  rayovin plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`RAYOVIN_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`RAYOVIN_HOME` is the Rayovin profile/config home used by both
`rayovin plugins enable ...` and the later `rayovin chat ...` run. If unset,
Rayovin uses the user's default home, usually `~/.rayovin`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export RAYOVIN_HOME=/tmp/rayovin-nemo-relay-test
rayovin plugins enable observability/nemo_relay
rayovin chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `rayovin` command you run is built from the
...
uv run rayovin plugins enable observability/nemo_relay
uv run rayovin chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/rayovin_agent-*.whl
...
The plugin can configure exporters directly from `RAYOVIN_NEMO_RELAY_*`
...
```bash
export RAYOVIN_NEMO_RELAY_ATOF_ENABLED=1
export RAYOVIN_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export RAYOVIN_NEMO_RELAY_ATIF_ENABLED=1
export RAYOVIN_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `RAYOVIN_NEMO_RELAY_ATOF_FILENAME`
- `RAYOVIN_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `RAYOVIN_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `RAYOVIN_NEMO_RELAY_ATIF_AGENT_NAME`
- `RAYOVIN_NEMO_RELAY_ATIF_AGENT_VERSION`
- `RAYOVIN_NEMO_RELAY_ATIF_MODEL_NAME`
- `RAYOVIN_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Rayovin at it:

```bash
export RAYOVIN_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Rayovin Agent"
agent_version = "local"
```

When `RAYOVIN_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`RAYOVIN_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`RAYOVIN_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Rayovin does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Rayovin keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Rayovin routes LLM and tool
...
Rayovin feature-detects the dynamic-plugin activation API available in NeMo Relay
0.6 and later. Configure native or worker plugins with Rayovin-owned
...
with this Rayovin-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. Rayovin rejects
...
Rayovin activates these plugins before registering its managed LLM and tool
...
activation API, Rayovin logs an actionable warning and continues with the
...
For the full generic Rayovin middleware contract, see
...
export RAYOVIN_HOME=/tmp/rayovin-nemo-relay-docs/rayovin-home
mkdir -p "$RAYOVIN_HOME"

cat > "$RAYOVIN_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Rayovin session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export RAYOVIN_NEMO_RELAY_ATOF_ENABLED=1
export RAYOVIN_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/rayovin-nemo-relay-docs/subagent/atof
export RAYOVIN_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export RAYOVIN_NEMO_RELAY_ATOF_MODE=overwrite
export RAYOVIN_NEMO_RELAY_ATIF_ENABLED=1
export RAYOVIN_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/rayovin-nemo-relay-docs/subagent/atif
export RAYOVIN_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export RAYOVIN_NEMO_RELAY_ATIF_AGENT_NAME='Rayovin Agent E2E'
export RAYOVIN_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export RAYOVIN_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"rayovin.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Rayovin Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Rayovin dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/rayovin-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/rayovin-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/rayovin-nemo-relay-docs/workdir/beta.txt
cd /tmp/rayovin-nemo-relay-docs/workdir

export RAYOVIN_NEMO_RELAY_ATOF_ENABLED=1
export RAYOVIN_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/rayovin-nemo-relay-docs/parallel/atof
export RAYOVIN_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export RAYOVIN_NEMO_RELAY_ATOF_MODE=overwrite
export RAYOVIN_NEMO_RELAY_ATIF_ENABLED=1
export RAYOVIN_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/rayovin-nemo-relay-docs/parallel/atif
export RAYOVIN_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export RAYOVIN_NEMO_RELAY_ATIF_AGENT_NAME='Rayovin Agent E2E'
export RAYOVIN_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Rayovin sessions map to `agent` scopes.
- Rayovin API request hooks map to `llm` scope start/end events.
- Rayovin tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Rayovin execution middleware to hand
...
Enable it for Rayovin:

```bash
export RAYOVIN_NEMO_RELAY_PLUGINS_TOML=/tmp/rayovin-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Rayovin routes execution
through these boundaries:

```text
Rayovin provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Rayovin provider adapter next_call(...)

Rayovin tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Rayovin tool dispatcher next_call(...)
```
...
middleware for a local Rayovin run. This path requires a NeMo Relay runtime that
supports `[components.config.tool_parallelism]`, as provided by the supported
0.x release range beginning with 0.5.

```bash
export RAYOVIN_HOME=/tmp/rayovin-middleware-test/rayovin-home
mkdir -p "$RAYOVIN_HOME" /tmp/rayovin-middleware-test/nemo-relay
...
cat > /tmp/rayovin-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/rayovin-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/rayovin-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Rayovin Middleware E2E"
agent_version = "local"
...
export RAYOVIN_NEMO_RELAY_PLUGINS_TOML=/tmp/rayovin-middleware-test/nemo-relay/plugins.toml
