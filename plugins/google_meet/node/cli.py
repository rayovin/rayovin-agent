"""`rayovin meet node ...` subcommand tree.

Wired into the existing ``rayovin meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``rayovin meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="rayovin-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``rayovin meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             rayovin meet node approve <name> ws://<host>:{args.port} {token}")
        try:
