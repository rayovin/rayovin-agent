...
session-scoped state file under ``$RAYOVIN_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $RAYOVIN_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_rayovin_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this rayovin install, leave it first —
    ...
    env = os.environ.copy()
    env["RAYOVIN_MEET_URL"] = url
    env["RAYOVIN_MEET_OUT_DIR"] = str(out)
    env["RAYOVIN_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["RAYOVIN_MEET_HEADED"] = "1"
    if auth_state:
        env["RAYOVIN_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["RAYOVIN_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if RAYOVIN_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["RAYOVIN_MEET_MODE"] = mode
    if realtime_model:
        env["RAYOVIN_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["RAYOVIN_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["RAYOVIN_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["RAYOVIN_MEET_REALTIME_KEY"] = realtime_api_key
