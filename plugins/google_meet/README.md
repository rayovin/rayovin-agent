...
Let the rayovin agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where rayovin runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `rayovin meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `rayovin meet` CLI |
...
| `cli.py` | `rayovin meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$RAYOVIN_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `rayovin meet node {run,list,approve,remove,status,ping}` |
...
rayovin meet install                                      # pip + Chromium
rayovin meet setup                                        # preflight
rayovin meet auth                                         # optional
rayovin meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
rayovin meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.rayovin/.env
rayovin meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
rayovin meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
rayovin meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.rayovin/.env
rayovin meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, rayovin will **not** switch your system audio input automatically — the
...
rayovin plugins enable google_meet
rayovin meet node run --display-name my-mac --host 0.0.0.0 --port 18789
