...
Auto-tracks and cleans up ephemeral files created during Rayovin Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$RAYOVIN_HOME` and `/tmp/rayovin-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `RAYOVIN_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `RAYOVIN_HOME` or `/tmp/rayovin-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$RAYOVIN_HOME/disk-cleanup/` is itself excluded
- `$RAYOVIN_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
