"""disk_cleanup — ephemeral file cleanup for Rayovin Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under RAYOVIN_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly RAYOVIN_HOME and /tmp/rayovin-*
Never touches: ~/.rayovin/logs/ or any system directory.
"""
...
try:
    ...
    def get_rayovin_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("RAYOVIN_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".rayovin").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$RAYOVIN_HOME/logs/``."""
    return get_rayovin_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$RAYOVIN_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under RAYOVIN_HOME or ``/tmp/rayovin-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    rayovin_home = get_rayovin_home()
    try:
        path.resolve().relative_to(rayovin_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/rayovin-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("rayovin-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "rayovin-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $RAYOVIN_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so RAYOVIN_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        rayovin_home = get_rayovin_home()
        for parent in ("cron", "cronjobs"):
            base = rayovin_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside RAYOVIN_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under RAYOVIN_HOME, but never recurse into known
    # durable state trees.  Some installs place the Rayovin checkout, venv,
    # and desktop build under RAYOVIN_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    rayovin_home = get_rayovin_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in rayovin_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    rayovin_home = get_rayovin_home()
    try:
        rel = path.resolve().relative_to(rayovin_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "rayovin-agent",
        }:
            return None
        ...
        # Path isn't under RAYOVIN_HOME (e.g. /tmp/rayovin-*) — fall through.
        pass
