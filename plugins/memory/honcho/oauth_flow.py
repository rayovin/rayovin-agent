...
``rayovin://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Rayovin OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "rayovin-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``rayovin-desktop`` / ``rayovin-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Rayovin.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local RAYOVIN_HOME override can't reach.
