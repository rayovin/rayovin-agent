...
```bash
rayovin memory setup    # select "mem0"
```
...
Behavioral settings live in `$RAYOVIN_HOME/mem0.json` (set them via `rayovin memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.rayovin/.env`.
...
| `user_id` | `rayovin-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Rayovin session and call `mem0_search` — it connects to your server.
...
```bash
rayovin memory setup mem0 --mode oss \
...
```bash
rayovin memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$RAYOVIN_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.rayovin/mem0_qdrant"}}
  }
}
```
...
```bash
rayovin memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$RAYOVIN_HOME/mem0.json`).
