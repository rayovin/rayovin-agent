...
def _save_mem0_json(rayovin_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(rayovin_home) / "mem0.json"
    existing = {}
...
def _setup_platform(rayovin_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "rayovin-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "rayovin"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.rayovin/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.rayovin/.env (or unset it) or Rayovin will keep "
    ...
    provider.save_config(provider_config, rayovin_home)

    if env_writes:
        _write_env(Path(rayovin_home) / ".env", env_writes)
...
def _setup_selfhosted(rayovin_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "rayovin-user"
    )
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "rayovin")
...
def _setup_oss(rayovin_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(rayovin_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "rayovin-user")
    ...
    if env_writes:
        _write_env(Path(rayovin_home) / ".env", env_writes)
    _save_mem0_json(rayovin_home, {"mode": "oss", "user_id": user_id, "agent_id": "rayovin", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, rayovin_home: str) -> str:
...
_PGVECTOR_CONTAINER = "rayovin-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "rayovin"
...
def _setup_oss_interactive(rayovin_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], rayovin_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], rayovin_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'rayovin-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "rayovin-user")

    agent_id = input("  Agent ID [rayovin]: ").strip()
    agent_id = agent_id or "rayovin"
    ...
    if env_writes:
        _write_env(Path(rayovin_home) / ".env", env_writes)
    _save_mem0_json(rayovin_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(rayovin_home: str, config: dict) -> None:
    """Entry point called by rayovin memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(rayovin_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(rayovin_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(rayovin_home, config, flags)
        return
