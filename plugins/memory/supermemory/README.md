...
- Supermemory API key from [app.supermemory.ai/integrations?connect=rayovin](http://app.supermemory.ai/integrations?connect=rayovin)
...
Config file: `$RAYOVIN_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `rayovin` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `rayovin-{identity}` → `rayovin-coder`). |
...
All Supermemory API calls send `x-sm-source: rayovin`, and document writes stamp
`metadata.sm_source: rayovin`. This is a **functional routing key, not telemetry**:
it groups Rayovin-written memories into a dedicated "Rayovin" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Rayovin profile:

```json
{
  "container_tag": "rayovin-{identity}"
}
```

For a profile named `coder`, this resolves to `rayovin-coder`. The default profile resolves to `rayovin-default`. Without `{identity}`, all profiles share the same container.
