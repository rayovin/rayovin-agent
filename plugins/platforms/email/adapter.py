"""
Email platform adapter for the Rayovin gateway.

Allows users to interact with Rayovin by sending emails.
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `rayovin config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `rayovin gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Rayovin Agent")
        ...
        msg_id = f"<rayovin-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# rayovin_cli/gateway.py, and the _send_email dispatch in
