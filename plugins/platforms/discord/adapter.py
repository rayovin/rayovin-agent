...
def _discord_ready_timeout_seconds() -> float:
    ...
    raw = os.getenv("RAYOVIN_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()
    if raw:
        try:
            return max(0.0, float(raw))
        except ValueError:
            logger.warning(
                "Ignoring invalid RAYOVIN_GATEWAY_PLATFORM_CONNECT_TIMEOUT=%r",
                raw,
            )
    return 30.0
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.rayovin/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("RAYOVIN_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("RAYOVIN_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        self._liveness_interval_seconds = env_float(
            "RAYOVIN_DISCORD_LIVENESS_INTERVAL_SECONDS", 60.0
        )
        self._liveness_failure_threshold = env_int(
            "RAYOVIN_DISCORD_LIVENESS_FAILURE_THRESHOLD", 3
        )
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Rayovin gateway
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        disconnect budget (``RAYOVIN_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT``, default
    ...
    def _text_batch_flush_deadline_seconds(self) -> float:
        ...
        raw = os.getenv("RAYOVIN_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "").strip()
        if raw:
            try:
                parsed = float(raw)
                if parsed > 0:
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_rayovin_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Rayovin manages."""
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit Rayovin pairing grant."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Rayovin session")
        ...
        @tree.command(name="status", description="Show Rayovin session status")
        ...
        @tree.command(name="stop", description="Stop the running Rayovin agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.rayovin/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Rayovin Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Rayovin gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Rayovin session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Rayovin in the thread",
        ...
        # rayovin_cli/commands.py automatically appear as Discord slash
        ...
        dropped_over_cap = 0
        try:
            from rayovin_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from rayovin_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a Rayovin skill",
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from rayovin_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Rayovin session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Rayovin: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Rayovin has an LLM-generated session title and can safely rename
        ...
        thread_name = content[:80] if content else "Rayovin"
        if len(content) > 80:
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        for attempt in range(2):
            try:
                ...
                try:
                    setattr(thread, "_rayovin_auto_thread_initial_name", thread_name)
                except Exception:
                    pass
                return thread
            except Exception as direct_error:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by Rayovin: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def rename_thread(
        ...
        *,
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="Rayovin semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(
        ...
        reason = "Rayovin session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Rayovin handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want Rayovin to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
            if smart_denied:
    ...
    async def send_clarify(
        ...
        try:
            ...
            embed = discord.Embed(
                title="❓ Rayovin needs your input",
                description=body,
                color=discord.Color.orange(),
            )
            ...
            content = self._self_contained_prompt_content(
                "❓ **Rayovin needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``rayovin update --gateway``
    ...
    async def _handle_message(self, message: DiscordMessage, role_authorized: bool = False) -> None:
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ Rayovin could not create a Discord thread for "
        ...
        source = self.build_source(
            ...
            auto_thread_initial_name=(
                getattr(auto_threaded_channel, "_rayovin_auto_thread_initial_name", None)
...
def _component_check_auth(
    ...
    the pairing store (``rayovin pairing approve``).
    ...
    # so users approved via ``rayovin pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``rayovin update`` prompts.
...
# is not in this process (e.g. ``rayovin cron`` running standalone) and no live
...
def interactive_setup() -> None:
    ...
    from rayovin_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    if lis is not None and not os.getenv("RAYOVIN_DISCORD_LIVENESS_INTERVAL_SECONDS"):
        os.environ["RAYOVIN_DISCORD_LIVENESS_INTERVAL_SECONDS"] = str(lis)
    lft = discord_cfg.get("liveness_failure_threshold")
    if lft is not None and not os.getenv("RAYOVIN_DISCORD_LIVENESS_FAILURE_THRESHOLD"):
        os.environ["RAYOVIN_DISCORD_LIVENESS_FAILURE_THRESHOLD"] = str(lft)
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``rayovin_cli.gateway.get_env_value`` at call time (not via
    ...
    """
    import rayovin_cli.gateway as gateway_mod
...
def register(ctx) -> None:
    """Plugin entry point — called by the Rayovin plugin system."""
    ctx.register_platform(
        ...
        install_hint="pip install 'rayovin-agent[messaging]'",
        # Interactive setup wizard — replaces the central
        # rayovin_cli/setup.py::_setup_discord function.  Same shape as Teams.
