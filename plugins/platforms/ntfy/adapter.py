"""ntfy platform adapter (Rayovin plugin).
...
a Rayovin dependency.

This adapter ships as a Rayovin platform plugin under
``plugins/platforms/ntfy/``. The Rayovin plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "rayovin-in"              # subscribe topic (incoming)
          publish_topic: "rayovin-out"     # optional — defaults to topic
...
_ECHO_TAG = "rayovin-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``rayovin cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Rayovin dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `rayovin gateway status` without
