"""SimpleX Chat platform adapter (Rayovin plugin).
...
This adapter ships as a Rayovin platform plugin under
``plugins/platforms/simplex/``. The Rayovin plugin loader scans the
...
Optional environment variables:
    ...
    RAYOVIN_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``rayovin setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Rayovin — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("RAYOVIN_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``rayovin setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.rayovin/.env`` via
    ``rayovin_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "rayovin_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.rayovin/.env"
        )
        return
