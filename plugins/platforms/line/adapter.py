"""
LINE Messaging API platform adapter for Rayovin Agent.
...
to Rayovin Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``RAYOVIN_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            rayovin_home = Path(get_rayovin_home()).resolve()
        except Exception:
            rayovin_home = Path.home().joinpath(".rayovin").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``rayovin status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``rayovin status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``rayovin setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.rayovin/.env`` via ``rayovin_cli.config``.
    """
    print()
    ...
    try:
        from rayovin_cli.config import get_env_var, set_env_var
    except ImportError:
        print("rayovin_cli.config not available; set LINE_* vars manually in ~/.rayovin/.env")
        return
