...
# ``model_tools`` import time, so ``rayovin status``, ``rayovin chat``, etc.
...
# ``rayovin_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Rayovin is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a rayovin MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "rayovin-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${RAYOVIN_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _rayovin_home = _get_rayovin_home()
        except (ModuleNotFoundError, ImportError):
            _rayovin_home = _Path.home() / ".rayovin"
        self._thread_count_store = _ThreadCountStore(
            _rayovin_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Rayovin is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("RAYOVIN_HOME", str(_Path.home() / ".rayovin"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${RAYOVIN_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a rayovin MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Rayovin is thinking…' marker message.
        ...
        "Rayovin is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Rayovin is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Rayovin is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Rayovin is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Rayovin is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``rayovin setup``.

    The setup wizard at ``rayovin_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.rayovin/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. rayovin-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.rayovin/.env")
    print_info("Restart the gateway: rayovin gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``rayovin cron`` running as a
    separate process from ``rayovin gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Rayovin plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'rayovin-agent[google_chat]'",
        ...
        # the "Rayovin is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Rayovin is thinking…' marker message "
