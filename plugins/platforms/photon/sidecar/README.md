...
Small Node helper that bridges Rayovin Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Rayovin is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Rayovin-Sidecar-Token`)
...
The Rayovin plugin's `rayovin photon setup` command runs `npm install`
here automatically.
