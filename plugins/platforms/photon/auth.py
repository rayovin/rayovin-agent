...
Credential storage mirrors every other Rayovin channel:

    * runtime SDK creds  -> ``~/.rayovin/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.rayovin/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a rayovin dependency
...
# registers Rayovin as its own client_id.
...
# Default name of the project Rayovin provisions for the operator.
DEFAULT_PROJECT_NAME = "Rayovin Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of rayovin-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.rayovin/auth.json`` honouring the active Rayovin profile."""
    try:
        from rayovin_constants import get_rayovin_home
        return Path(get_rayovin_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.rayovin")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.rayovin/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.rayovin/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.rayovin/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: rayovin_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `rayovin photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `rayovin photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `rayovin photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `rayovin photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `rayovin photon setup`)"
