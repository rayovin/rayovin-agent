...
This plugin connects Rayovin Agent to iMessage (and other Spectrum
...
TypeScript-only, Rayovin runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Rayovin-Sidecar-Token`.
...
There is no separate `login` command; like every other Rayovin channel,
...
Run `rayovin photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.rayovin/.env` (the same place every other
...
Management metadata lives in `~/.rayovin/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `rayovin photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `rayovin photon status`, a DM and a group roundtrip,
