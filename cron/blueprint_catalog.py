...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``rayovin://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``rayovin://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"rayovin://blueprint/{quote(blueprint.key)}{qs}"
