...
Rayovin supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.rayovin/config.yaml`:
...
Or set the env var: `RAYOVIN_KANBAN_DISPATCH_IN_GATEWAY=false`
