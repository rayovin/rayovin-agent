...
Rayovin middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Rayovin'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Rayovin calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Rayovin retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Rayovin guardrails,
...
- `telemetry_schema_version`: currently `rayovin.observer.v1`
- `middleware_schema_version`: currently `rayovin.middleware.v1`
...
Rayovin stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Rayovin runs
them as a nested chain in registration order. Middleware failures are fail-open:
Rayovin logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Rayovin applies middleware in this order:
...
For each tool call, Rayovin applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Rayovin pre-execution path against the effective arguments:
...
```bash
rayovin plugins enable <plugin-name>
```

For isolated local testing, use one `RAYOVIN_HOME` for plugin enablement and the
agent run:

```bash
export RAYOVIN_HOME=/tmp/rayovin-middleware-test
mkdir -p "$RAYOVIN_HOME"
rayovin plugins enable <plugin-name>
rayovin chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run rayovin plugins enable <plugin-name>
uv run rayovin chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["rayovin_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/rayovin-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Rayovin expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Rayovin treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Rayovin preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Rayovin does not convert downstream
