...
`rayovin config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.rayovin/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `RAYOVIN_HOME`, secret scope, and
