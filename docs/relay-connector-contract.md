...
This document is the formal interface between the **Rayovin gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `rayovin gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`rayovin gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `rayovin` CLI.
