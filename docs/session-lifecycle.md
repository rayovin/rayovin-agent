...
The canonical transcript store is SQLite via `SessionDB` (from `rayovin_state`). The
...
Counts consecutive restarts via a JSON file (`{RAYOVIN_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `rayovin update`, `rayovin gateway restart`,
or `/restart`.
