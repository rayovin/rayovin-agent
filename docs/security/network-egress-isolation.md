...
When running Rayovin inside Docker, the default `network_mode: host` gives the
...
The Rayovin [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ rayovin-agent │   │ rayovin-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   RAYOVIN_UID=$(id -u) RAYOVIN_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=rayovin,rayovin-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://rayovin-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Rayovin trust model and vulnerability reporting
