...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``rayovin setup`` display).
    ...
    # built-ins).  Used by ``rayovin gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `rayovin`
        # invocation -- including plain `rayovin chat`, which never touches any
        ...
        # `rayovin setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``rayovin setup``/``gateway status``, channel
