...
# Audio file extensions Rayovin recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Rayovin' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Rayovin-created Telegram private-chat
...
from rayovin_constants import get_default_rayovin_root, get_rayovin_dir, get_rayovin_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.rayovin/.env manually."
)
...
IMAGE_CACHE_DIR = get_rayovin_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_rayovin_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_rayovin_dir(new_subpath, old_name)
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; RayovinAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
AUDIO_CACHE_DIR = get_rayovin_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_rayovin_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_rayovin_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_rayovin_dir("cache/screenshots", "browser_screenshots")
...
_RAYOVIN_ROOT = get_default_rayovin_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "RAYOVIN_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "RAYOVIN_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "RAYOVIN_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "RAYOVIN_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _RAYOVIN_HOME / "image_cache",
    _RAYOVIN_HOME / "audio_cache",
    _RAYOVIN_HOME / "video_cache",
    _RAYOVIN_HOME / "document_cache",
    _RAYOVIN_HOME / "browser_screenshots",
    ...
    _RAYOVIN_HOME / "cache" / "images",
    _RAYOVIN_HOME / "cache" / "audio",
    _RAYOVIN_HOME / "cache" / "videos",
    _RAYOVIN_HOME / "cache" / "documents",
    _RAYOVIN_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Rayovin root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* RAYOVIN_HOME's cache, so a gateway running at
    the root (e.g. ``RAYOVIN_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when RAYOVIN_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _RAYOVIN_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("RAYOVIN_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _RAYOVIN_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Rayovin profile and shared Rayovin root both contain control
    ...
    # RAYOVIN_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.rayovin
    ...
    for rayovin_root in (_RAYOVIN_HOME, _RAYOVIN_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(rayovin_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Rayovin
    secrets (``~/.rayovin/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``RAYOVIN_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Rayovin-managed cache, under an operator-allowlisted root
    (``RAYOVIN_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Rayovin root (~/.rayovin/.env,
    ...
    # ``MEDIA:~/.rayovin/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?rayovin\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+rayovin[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``RAYOVIN_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Rayovin commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("RAYOVIN_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "RAYOVIN_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "RAYOVIN_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Rayovin sends
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Rayovin home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Rayovin home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.rayovin/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from rayovin_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          RAYOVIN_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          RAYOVIN_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          RAYOVIN_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("RAYOVIN_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("RAYOVIN_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("RAYOVIN_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _rayovin_run_generation is set on
    ...
    def build_source(
        ...
        per-profile RAYOVIN_HOME isolation.
        """
