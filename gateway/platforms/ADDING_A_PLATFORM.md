...
There are two ways to add a platform to the Rayovin gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.rayovin/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Rayovin code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `rayovin gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `rayovin_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Rayovin core.
...
And add it to the `rayovin-gateway` composite:

```python
"rayovin-gateway": {
    "includes": [..., "rayovin-your-platform"]
}
```
...
## 12. Status Display (`rayovin_cli/status.py`)
...
## 13. Gateway Setup Wizard (`rayovin_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ rayovin_cli/ toolsets.py \
  --include="*.py" -l | sort -u
