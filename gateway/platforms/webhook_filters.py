...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.rayovin to the active profile home."""
    ...
    if raw == "~/.rayovin":
        return rayovin_home
    if raw.startswith("~/.rayovin/"):
        return rayovin_home / raw.removeprefix("~/.rayovin/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return rayovin_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under RAYOVIN_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from rayovin_constants import get_rayovin_home

    scripts_root = (get_rayovin_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.rayovin" or raw_text.startswith("~/.rayovin/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__rayovin_ignore__") is True
        ):
            return False, None
