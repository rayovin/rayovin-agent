...
interface RayovinConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function RayovinConsoleModal({ open, onClose }: RayovinConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("rayovin> ");
  const inputPromptRef = useRef("rayovin> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "rayovin> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Rayovin Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="rayovin-console-title"
    >
      <div
        className={cn(
          themedBody,
