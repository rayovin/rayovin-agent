...
VERSION_FILE = REPO_ROOT / "rayovin_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "kavi@local.rayovin": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level RAYOVIN_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (rayovin journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "rayovin.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.rayovin/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "moonsong@rayovin.local": "Tranquil-Flow",  # PR #52623 salvage (auxiliary Anthropic base_url host validation; #52608)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows rayovin-shim resolution + prefer --update on recovery; #52378)
    ...
    "jake@github.com/rayovin/rayovin-agent": "simpolism",
    ...
    "rayovin@marian.local": "Schrotti77",
    ...
    "teknium@github.com/rayovin/rayovin-agent": "teknium1",
    ...
    "274096618+rayovin-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "shannon@github.com/rayovin/rayovin-agent": "shannonsands",
    ...
    "rayovin@github.com/rayovin/rayovin-agent": "NousResearch",
    ...
    "ben@github.com/rayovin/rayovin-agent": "benbarclay",
    ...
    "emozilla@github.com/rayovin/rayovin-agent": "emozilla",
    ...
    "jonny@github.com/rayovin/rayovin-agent": "yoniebans",
    "jake@github.com/rayovin/rayovin-agent": "simpolism",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on rayovin mcp add)
    ...
    "rayovin@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "teknium@rayovin-agent": "teknium1",
    ...
    "github@nadyarayovin.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (RAYOVIN_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (RAYOVIN_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: rayovin-eval community link)
    ...
    "rayovinagent26@gmail.com": "rayovinagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+rayovinagent26@users.noreply.github.com": "rayovinagent26",
    ...
    "clement@github.com/rayovin/rayovin-agent": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "vigo@rayovin": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.rayovin@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Rayovin runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level RAYOVIN_HOME state files)
    ...
    "annguyen@github.com/rayovin/rayovin-agent": "annguyenNous",  # PR #52844 co-author
    ...
    "zhchl@rayovin-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Rayovin
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"rayovin-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "rayovin@github.com/rayovin/rayovin-agent"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/rayovin-agent",
    ...
    lines.append(f"# Rayovin Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Rayovin Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Rayovin Agent Release Tool")
    ...
    print(f"{'='*60}")
    print("  Rayovin Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Rayovin Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Rayovin Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Rayovin Agent v{new_version} ({calver_date})' "
