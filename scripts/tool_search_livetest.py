#!/usr/bin/env python3
"""Live test harness for Rayovin Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any rayovin imports.
ORIGINAL_HOME = os.environ.get("RAYOVIN_HOME")
ORIGINAL_AUTH = Path.home() / ".rayovin" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.rayovin/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.rayovin/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="rayovin_ts_live_"))
    rayovin_home = home_dir / ".rayovin"
    rayovin_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, rayovin_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".rayovin" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, rayovin_home / ".env")
        ...
        load_rayovin_dotenv(rayovin_home=str(Path.home() / ".rayovin"))
    ...
    (rayovin_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return rayovin_home
...
def reset_module_state():
    """Drop cached modules so the new RAYOVIN_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "rayovin_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["RAYOVIN_HOME"] = str(home)
...
def main():
    ...
    # Restore original RAYOVIN_HOME
    if ORIGINAL_HOME is not None:
        os.environ["RAYOVIN_HOME"] = ORIGINAL_HOME
    else:
